...
public:
  ...
  vec_type LLH2XYZ(const LonLatAlt& llh) const {
    const double cp = std::cos(llh.phi()),    sp = std::sin(llh.phi());
    const double cl = std::cos(llh.lambda()), sl = std::sin(llh.lambda());
    const double nu = rc_normal(llh);
  ...
  LonLatAlt XYZ2LLH(vec_type x) const {
    const double rho      = TNT::norm(x.subarray(0,1));
    const double z_by_rho = x(2)/rho;
    const double lambda   = 2.0*std::atan2(x(1), x(0)+rho);
    double alt[2] = { 0,0 };
  ...
  mat_type dXYZdENU(const LonLatAlt& llh) const {
    const double cp = std::cos(llh.phi()),    sp = std::sin(llh.phi());
    const double cl = std::cos(llh.lambda()), sl = std::sin(llh.lambda());
...
protected:
  ...
  double rc_normal(const LonLatAlt& llh) const {
  ...
  double rc_meridional(const LonLatAlt &llh) const {
    const double sp(std::sin(llh.phi()));
