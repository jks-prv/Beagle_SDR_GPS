...
static const char* edata_with_file_ext(char **o_uri, bool free_o_uri, bool *free_uri, const char *prefix_list[],
    bool cache_check, size_t *size, u4_t *mtime, bool *is_gzip)
{
	...
	//printf("edata_with_file_ext %s %s\n", *o_uri, cache_check? "CACHE_CHECK":"REQUEST");
	*free_uri = FALSE;
	if (prefix_list == NULL) prefix_list = no_prefix;

    for (char **pre = (char **) prefix_list; *pre != NULL && !edata_data; pre++) {
        for (int i = 0; i < ARRAY_LEN(suffix) && !edata_data; i++) {
            asprintf(&uri2, "%s%s%s", *pre, *o_uri, suffix[i]);
            if (*free_uri) free(uri);
            uri = uri2;
            *free_uri = TRUE;
            edata_data = edata(uri, cache_check, size, mtime);
            if (edata_data) *is_gzip = kiwi_str_ends_with(uri, ".gz");
	        //printf("edata_with_file_ext TRY %s %p\n", uri, edata_data);
	        // NB: "&& !edata_data" in both for loops will cause double break if edata_data becomes non-NULL
        }
    }
    
    //printf("edata_with_file_ext RTN %p %s %s %s\n", edata_data, uri, cache_check? "CACHE_CHECK":"REQUEST", *is_gzip? "GZIP":"");
...
int web_request(struct mg_connection *mc, enum mg_event evt) {
    ...
    bool is_gzip = FALSE;
    edata_data = edata_with_file_ext(&uri, free_uri, &free_uri, NULL, evt == MG_CACHE_INFO, &edata_size, &mtime, &is_gzip);
    ...
    if (!edata_data && !has_prefix) {
        ...
        edata_data = edata_with_file_ext(&uri, FALSE, &free_uri, prefix_kiwi, evt == MG_CACHE_INFO, &edata_size, &mtime, &is_gzip);
    }
    ...
    if (!edata_data && is_extension) {
        ...
        edata_data = edata_with_file_ext(&uri, FALSE, &free_uri, prefix_ext, evt == MG_CACHE_INFO, &edata_size, &mtime, &is_gzip);
    }
    ...
    if (evt == MG_CACHE_INFO) {
        ...
        web_printf("%-15s %s headers, is_sdr_hu=%d %s\n", "sending", hdr_type, is_sdr_hu, is_gzip? "GZIP":"");
        //printf("%-15s %s headers, is_sdr_hu=%d %s %s\n", "sending", hdr_type, is_sdr_hu, is_gzip? "GZIP":"", uri);
