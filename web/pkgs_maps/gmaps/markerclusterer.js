...
// KiwiSDR
// single-click: zoom-in +1
// double-click: zoom to bounding box
...
ClusterIcon.prototype.triggerClusterClick = function(event) {
    ...
    if (markerClusterer.click_pending === true) return;
    markerClusterer.click_pending = true;

    markerClusterer.click_timeout = setTimeout(function(map, cluster) {
        markerClusterer.click_pending = false;

        if (markerClusterer.isZoomOnClick()) {
           // Zoom +1 into the cluster.
           map.panTo(cluster.getCenter());
           map.setZoom(map.getZoom() + 1);
        }
    }, 300, this.map_, this.cluster_);
};

/**
 * Triggers the clusterdblclick event and zoom's if the option is set.
 *
 * @param {google.maps.MouseEvent} event The event to propagate
 */
ClusterIcon.prototype.triggerClusterDblClick = function(event) {
    ...
    kiwi_clearTimeout(markerClusterer.click_timeout);   // keep single click event from hapenning
    markerClusterer.click_pending = false;

    if (markerClusterer.isZoomOnClick()) {
        // Zoom into the cluster.
        this.map_.fitBounds(this.cluster_.getBounds());
        cancelEvent(event);     // stop event propagation
    }
};
...
function defaultClusterOnAdd(clusterIcon) {
    ...
    google.maps.event.addDomListener(clusterIcon.div_, 'dblclick', function(event) {
        // Only perform double click when not preceded by a drag
        if (!isDragging) {
            clusterIcon.triggerClusterDblClick(event);
        }
    });
