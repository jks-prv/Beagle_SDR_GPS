...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Wed Jul  5 09:23:03 2023) */
...
function kiwi_output_msg(id,id_scroll,p){var i,j;var dbg=(0&&dbgUs);if(dbgUs)kiwi.d.p=p;if(1&&dbg){console.log('$kiwi_output_msg id='+id+' init='+p.init+' isAltBuf='+p.isAltBuf+' s='+p.s);console.log('$ '+kiwi_JSON(p));}
...
return{s:s,a:a};};var resize_init=function(init){var r,c;removeAllLines(parent_el);p.screen=[];p.color=[];p.dirty=[];p.els=[];for(var r=1;r<=p.nrows;r++){p.screen[r]=[];p.color[r]=[];try{for(var c=1;c<=p.cols;c++){p.screen[r][c]=' ';p.color[r][c]={fg:null,bg:null};}}catch(ex){if(dbg){console.log('--------');console.log('r='+r+' c='+c);console.log(p.screen);console.log(ex);}}
p.dirty[r]=false;p.els[r]=appendEmptyLine(parent_el);p.els[r].innerHTML='&nbsp;';}
p.margin_set=false;p.r=p.c=p.r_cursor=p.c_cursor=1;if(init){if(isAdmin())
console_is_char_oriented(true);p.isAltBuf=true;p.r_cursor=p.c_cursor=0;}};var s;if(isNoArg(p.no_decode)||p.no_decode!=true){try{s=kiwi_decodeURIComponent('kiwi_output_msg',p.s);}catch(ex){console.log('decodeURIComponent FAIL:');console.log(p.s);s=p.s;}
if(s==null)s='(kiwi_decodeURIComponent():null)';}else{s=p.s;}
if(p.init!=true){p.el=appendEmptyLine(parent_el);p.cols=p.cols||80;p.NONE=0;p.ESC=1;p.CSI=2;p.NON_CSI=3;p.NOSHIFT_MODE=true;p.esc={s:'',state:p.NONE};p.sgr={span:0,bright:0,fg:null,bg:null};p.return_pending=false;p.must_scroll_down=false;p.traceEvery=false;p.resized=false;p.ccol=1;p.line=[];p.line_sgr=[];p.line_html=[];p.inc=1;p.nrows=p.rows;p.margin_top=1;p.margin_bottom=p.nrows;p.show_cursor=p.show_cursor||false;p.r_cursor=p.c_cursor=0;p.insertMode=true;p.eol_wrap=false;resize_init(true);p.isAltBuf=false;p.altbuf_via_cursor_visible=false;p.altbuf_via_cup=false;p.alt_save='';p.rend_seq=0;p.metronome_running=false;p.sched_time=0;p.init=true;}
if(p.resized&&p.isAltBuf){if(dbg)console.log('console resized nrows: '+p.nrows+' => '+p.rows+' #############################################################################################');p.nrows=p.rows;resize_init();p.margin_top=1;p.margin_bottom=p.nrows;p.r_cursor=p.c_cursor=0;render();p.resized=false;}
...
if(enter_altbuf){if(dbg)console.log('$ENTER alt buf via '+(p.altbuf_via_cup?'cup':'cursor_visible')+' ====================================');p.alt_save=parent_el.innerHTML.replace(/<pre><span class="cl-admin-console-cursor">.*<\/span><\/pre>$/,'');if(dbg)console.log(kiwi_JSON(p.alt_save));resize_init();p.isAltBuf=true;if(dbg)console.log('1049: '+p.r+' '+p.c+' '+p.r_cursor+' '+p.c_cursor);}else
...
if(c=='r'&&p.isAltBuf){if(dbg)console.log('margin set: PREV p.nrows='+p.nrows);p.margin_top=n1;p.margin_bottom=n2;p.margin_set=true;p.nrows=n2;result='set margins, top='+n1+', bottom/nrows='+n2;}else
