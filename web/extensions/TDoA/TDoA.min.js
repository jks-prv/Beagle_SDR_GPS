...
switch(param[0]){case"ready":tdoa.a=(decodeURIComponent(param[1])).split(',');tdoa.a[0]=enc(tdoa.a[0]);tdoa.a[1]=enc(tdoa.a[1]);tdoa.params=ext_param();if(tdoa.params&&tdoa.params.includes('gmap:')){tdoa.leaflet=false;tdoa.gmap_param=true;}
...
function tdoa_reset_spiderfied(){if(tdoa.spiderfied)tdoa.spiderfied.unspiderfy();tdoa.spiderfied=false;}
...
function tdoa_update_link(){var url=kiwi_url_origin()+'/?f='+ext_get_freq_kHz()+'iqz'+ext_get_zoom();var pb=ext_get_passband();url+='&pbw='+(pb.high*2).toFixed(0)+'&ext=tdoa';var m=tdoa.cur_map;if(!m)return;var c=m.getCenter();url+=',lat:'+tdoa_lat(c).toFixed(2)+',lon:'+tdoa_lon(c).toFixed(2)+',z:'+m.getZoom();if(!tdoa.leaflet&&m.getMapTypeId()!='satellite')url+=',map:1';tdoa.field.forEach(function(f,i){if(f.good){url+=','+w3_get_value('tdoa.id_field-'+i);}});var r=w3_get_value('id-tdoa-known-location');if(r&&r!=''){r=r.replace(/\s+/g,' ').split(/[ ,]/g);if(r.length>=2){url+=','+r[2].replace(/[^0-9A-Za-z]/g,'');}}
...
function tdoa_marker_click(marker){if(tdoa.click_pending===true)return;tdoa.click_pending=true;tdoa.click_timeout=setTimeout(function(marker){tdoa.click_pending=false;marker.kiwi_mkr_2_ref_or_host.click(marker);},300,marker);}
...
function tdoa_host_marker_click(mkr){var h=mkr.kiwi_mkr_2_ref_or_host;for(var j=0;j<tdoa.tfields;j++){if(w3_get_value('tdoa.id_field-'+j)==h.call)return;}
...
function tdoa_open_window(host){var url='http://'+host.hp+'/?f='+ext_get_freq_kHz()+ext_get_mode()+'z'+ext_get_zoom();var pb=ext_get_passband();url+='&pbw='+(pb.high*2).toFixed(0);console.log('tdoa_open_window '+url);var win=window.open(url,'_blank');if(win)win.focus();}
function tdoa_listen_cb(path,val,first){var field_idx=+val;var f=tdoa.field[field_idx];if(f&&f.mkr&&f.mkr.kiwi_mkr_2_ref_or_host){tdoa_open_window(f.mkr.kiwi_mkr_2_ref_or_host);}}
...
tdoa.known_location_idx=r.idx;r.selected=true;console.log('ref_click: select ref '+r.id);tdoa_rebuild_refs();if(r.f)
...
function tdoa_style_marker(marker,idx,name,type,map){marker.bindTooltip(name,{className:'id-tdoa-'+type+' id-tdoa-'+type+'-'+idx,permanent:true,direction:'right'});marker.on('add',function(ev){var rh=ev.sourceTarget.kiwi_mkr_2_ref_or_host;w3_iterate_classname('id-tdoa-'+type+'-'+rh.idx,function(el){if(el){if(rh.type_host){if(rh.selected)w3_color(el,'black','yellow');else w3_color(el,'white','blue');}
el.title=rh.title;el.kiwi_mkr_2_ref_or_host=rh;el.addEventListener('click',function(ev){tdoa_marker_click(ev.target);});el.addEventListener('dblclick',function(ev){tdoa_marker_dblclick(ev.target);});el.addEventListener('mouseenter',function(ev){if(!rh.selected)w3_color(el,'black','yellow')});el.addEventListener('mouseleave',function(ev){if(!rh.selected)rh.type_host?w3_color(el,'white','blue'):w3_color(el,'black','lime');});}});});if(map){marker.addTo(map);tdoa.map_layers.push(marker);}}
...
h.dither_idx=h0.dither_idx++;tdoa.hosts_dither.push(h);console.log(h0.id+' '+h.id+' spacing='+spacing);}}
if(h0.id==h.id){console.log('TDoA duplicate ID: #'+h.i+' '+h.id+' #'+h0.i+' '+h0.id);if(h0.dup==undefined)h0.dup=0;h0.dup++;h.id+='/'+h0.dup;}}
h.id_lcase=h.id.toLowerCase();var mkr=tdoa_place_host_marker(h,tdoa.kiwi_map);if(mkr)markers.push(mkr);h.mkr=mkr;}
var lat,lon,zoom,maptype,init_submit;if(tdoa.params){var p=tdoa.params.split(',');tdoa.params=null;for(var i=0,len=p.length;i<len;i++){var a=p[i];console.log('TDoA: param <'+a+'>');if(a.includes(':')){if(a.startsWith('lat:')){lat=parseFloat(a.substring(4));}else
if(a.startsWith('lon:')||a.startsWith('lng:')){lon=parseFloat(a.substring(4));}else
if(a.startsWith('z:')){zoom=parseInt(a.substring(2));}else
if(a.startsWith('zoom:')){zoom=parseInt(a.substring(5));}else
...
if(a.startsWith('help:')){extint_help_click();}else
...
if(a.startsWith('devl:')){tdoa.devl=true;}}else{a=a.toLowerCase();var match=false;tdoa.refs.forEach(function(r){if(!match&&r.id_lcase&&r.id_lcase.includes(a)){console.log('TDoA refs match: '+a);r.selected=true;tdoa_ref_marker_click(r.mkr);match=true;}});match=false;tdoa.hosts.forEach(function(h){if(!match&&h.id_lcase&&h.id_lcase.includes(a)){console.log('TDoA hosts match: '+a);h.selected=true;tdoa_host_marker_click(h.mkr);match=true;}});}}}
tdoa_rebuild_hosts();tdoa_rebuild_refs();tdoa_pan_zoom(tdoa.kiwi_map,(lat&&lon)?[lat,lon]:null,zoom?zoom:-1);if(!tdoa.leaflet&&maptype==1)tdoa.kiwi_map.setMapTypeId('roadmap');tdoa.state=tdoa.READY_SAMPLE;if(init_submit==true)
tdoa.init_submit_timeout=setTimeout(tdoa_submit_button_cb2,6000);}
function TDoA_environment_changed(changed){if(changed.resize){var el=w3_el('id-tdoa-data');if(!el)return;var left=Math.max(0,(window.innerWidth-tdoa.w_data-time_display_width())/ 2);el.style.left=px(left);return;}
tdoa_update_link();}
...
function tdoa_set_icon(name,field_idx,icon,size,color,cb,cb_param){field_idx=(field_idx>=0)?field_idx:'';w3_innerHTML('id-tdoa-'+name+'-icon-c'+field_idx,(icon=='')?'':w3_icon('id-tdoa-'+name+'-icon'+field_idx,icon,size,color,cb,cb_param));}
...
function tdoa_rerun_clear(){w3_hide('id-tdoa-rerun-button');w3_hide('id-tdoa-download-KML');w3_hide('id-tdoa-download-MAT');tdoa.rerun=0;}
...
function tdoa_host_click_status_cb(obj,field_idx){var err;var f=tdoa.field[field_idx];if(obj.response){var arr=obj.response.split('\n');var offline=auth=users=users_max=fixes_min=null;for(var i=0;i<arr.length;i++){var a=arr[i];if(a.startsWith('offline='))offline=a.split('=')[1];if(a.startsWith('auth='))auth=a.split('=')[1];if(a.startsWith('users='))users=a.split('=')[1];if(a.startsWith('users_max='))users_max=a.split('=')[1];if(a.startsWith('fixes_min='))fixes_min=a.split('=')[1];}
...
if(fixes_min==0){err='no recent GPS timestamps';}else{if(users_max==null)return;if(users<users_max){tdoa_set_icon('sample',field_idx,'fa-refresh',20,'lime');tdoa_set_icon('listen',field_idx,'fa-volume-up',20,'lime','tdoa_listen_cb',field_idx);var s=fixes_min?(fixes_min+' GPS fixes/min'):'channel available';w3_innerHTML('id-tdoa-sample-status-'+field_idx,s);f.host.selected=true;tdoa_rebuild_hosts();if(f.mkr)tdoa_change_marker_style(f.mkr,'red','white','cl-tdoa-gmap-selected-label');tdoa_update_link();return;}
...
function tdoa_submit_button_cb2(){var i,j;tdoa.ngood=0;tdoa.field.forEach(function(f,i){if(f.good&&!tdoa.rerun)f.use=true;if(f.good&&f.use)tdoa.ngood++;});if(tdoa.ngood<2){tdoa_submit_state(tdoa.ERROR,'at least 2 available hosts needed');tdoa.rerun=0;return;}
if(tdoa.cur_map.getZoom()<3){tdoa_submit_state(tdoa.ERROR,'must be zoomed in further');tdoa.rerun=0;return;}
...
if(list_i)h_v+=',',p_v+=',',id_v+=',';h=a[0];li.h=h;h_v+=h;p=a[1];li.p=p;p_v+=p;id=w3_get_value('tdoa.id_field-'+i);if(id==''){id='id_'+i;w3_set_value('tdoa.id_field-'+i,id);}
id=id.replace(/[^0-9A-Za-z\-\/]/g,'');li.call=id;id=id.replace(/\//g,'-');li.id=id;id_v+=id;li.field_idx=i;list_i++;}
if(list_i<2){tdoa_submit_state(tdoa.ERROR,'at least 2 available hosts needed');tdoa.rerun=0;return;}
var s=h_v+p_v+id_v;s+='&f='+(ext_get_passband_center_freq()/1e3).toFixed(2);var pb=ext_get_passband();s+='&w='+pb.high.toFixed(0);if(tdoa.cur_map==null)tdoa.cur_map=tdoa.kiwi_map;var b=tdoa.cur_map.getBounds();var ne=b.getNorthEast();var sw=b.getSouthWest();var lat_n=Math.round(tdoa_lat(ne)+1);var lat_s=Math.round(tdoa_lat(sw)-1);var lon_e=Math.round(tdoa_lon(ne)+1);var lon_w=Math.round(tdoa_lon(sw)-1);s+="&pi=struct('lat_range',\\["+lat_s+","+lat_n+"\\],'lon_range',\\["+lon_w+","+lon_e+"\\]";var r=w3_get_value('id-tdoa-known-location');if(r&&r!=''){r=r.replace(/\s+/g,' ').split(/[ ,]/g);if(r.length>=3){var lat=parseFloat(r[0]),lon=parseFloat(r[1]);if(!isNaN(lat)&&!isNaN(lon)){var name=r[2].replace(/[^0-9A-Za-z]/g,'');s+=",'known_location',struct('coord',\\["+lat+","+lon+"\\],'name',"+sq(name)+")";}}}
if(!tdoa.old_algorithm)s+=",'new',true";if(tdoa.kml)s+=",'kml',1";s+=')';if(tdoa.rerun)s+='&rerun='+tdoa.response.key;if(tdoa.devl)s+='&devl=1';tdoa.last_menu_select=undefined;tdoa.response={};tdoa.response.seq=0;kiwi_ajax_progress(tdoa.url_base+'php/tdoa.php'+tdoa.a[0]+s,function(json){tdoa_protocol_response_cb(json);},0,0,function(json){tdoa_protocol_response_cb(json);},0);if(!tdoa.rerun){tdoa.field.forEach(function(f,i){if(f.good){w3_innerHTML('id-tdoa-sample-icon-c'+i,kiwi_pie('id-tdoa-pie',tdoa.pie_size,'#eeeeee','deepSkyBlue'));tdoa_set_icon('download',i,'');w3_innerHTML('id-tdoa-sample-status-'+i,'sampling started');}});tdoa_submit_state(tdoa.RUNNING,'sampling started');}else{tdoa_submit_state(tdoa.RUNNING,'rerun using same samples');tdoa.rerun=0;}
...
kiwi_ajax(tdoa.url_files+tdoa.response.key+'/status.json',function(j){var okay=0;var info=undefined;var err='unknown status returned';var per_file,status,message;if(j.AJAX_error){if(j.AJAX_error=='status'){err='status file missing';okay=1;}else{var nj=j.response.replace(/\n/g,'');j=kiwi_JSON_parse('tdoa_submit_status_new_cb',nj);if(j){okay=0;}else{err='status JSON parse error';okay=2;}}}
...
if(status){err=status;if(message)console.log(message);okay=3;}else{okay=0;}}
if(okay==0){try{per_file=j.input.per_file;}catch(ex){per_file=undefined;}
if(per_file){per_file.forEach(function(pf,i){if(pf.status=='BAD'){var name=pf.name.toLowerCase();tdoa.field.forEach(function(f,i){if(f.good&&f.host.id_lcase==name){tdoa_set_icon('sample',i,'fa-times-circle',20,'red');w3_innerHTML('id-tdoa-sample-status-'+i,'unused, poor data quality');}});}});}}
...
obj=kiwi_JSON_parse('tdoa_protocol_response_cb',json);if(!obj)return;}else
obj=json;if(tdoa.response.seq>0&&obj.key&&obj.key!=tdoa.response.key){return;}
var v;if(tdoa.response.seq==0&&obj.key!=undefined){tdoa.response.key=obj.key;tdoa.response.seq++;}
if(tdoa.response.seq==1&&obj.files!=undefined){tdoa.response.files=obj.files;tdoa.response.seq++;}
if(tdoa.response.seq==2&&obj.status0!=undefined){v=obj.status0;tdoa_sample_status_cb(v);tdoa.response.seq++;}
if(tdoa.response.seq==3&&obj.done!=undefined){v=obj.done;tdoa_submit_status_new_cb(v);tdoa.response.seq++;}}
...
if(tdoa.leaflet){tdoa.heatmap[midx]=L.imageOverlay(tdoa.url_files+tdoa.response.key+'/'+tdoa.results[midx].ids+'_for_map.png',[[j.imgBounds.north,j.imgBounds.west],[j.imgBounds.south,j.imgBounds.east]],{opacity:0.5});if(tdoa.heatmap_visible){tdoa.heatmap[midx].addTo(tdoa.result_map);tdoa.map_layers.push(tdoa.heatmap[midx]);}
...
var show_mkrs=[];if(idx==tdoa.TDOA_MAP_NO_HOSTS||idx==tdoa.TDOA_MAP_WITH_HOSTS||idx==tdoa.COMBINED_MAP){ms=tdoa.SINGLE_MAPS;me=tdoa.SINGLE_MAPS+npairs;}else{ms=idx;me=ms+1;}
for(var mi=ms;mi<me;mi++){var list1=tdoa.results[mi].list1;var list2=tdoa.results[mi].list2;show_mkrs[list1]=tdoa.list[list1].field_idx;show_mkrs[list2]=tdoa.list[list2].field_idx;}
for(var i=0;i<show_mkrs.length;i++){var field_idx=show_mkrs[i];if(field_idx==undefined)continue;var f=tdoa.field[field_idx];if(f.mkr&&f.host){if(idx==tdoa.TDOA_MAP_NO_HOSTS){var mkr=f.host.mkr;if(tdoa.leaflet){mkr.unbindTooltip();mkr.remove();mkr.off();}else mkr.setMap(null);}else{tdoa_place_host_marker(f.host,tdoa.result_map);}}}
...
s='<img'+' src="'+tdoa.url_files+tdoa.response.key+'/'+tdoa.results[idx].file+'.png"'+' alt="Image not available"'+'/>';w3_innerHTML('id-tdoa-png',w3_div('w3-text-yellow',s));w3_show('id-tdoa-png');tdoa.cur_map=tdoa.kiwi_map;}
...
function tdoa_clear_results_cb(path,idx,first){if(!(+idx))return;tdoa.result_refs.forEach(function(ref,i){if(tdoa.result_refs.length<=1)return;var m=tdoa.result_mkrs.shift();tdoa.result_refs.shift();if(m==null)return;console.log(m);if(tdoa.leaflet){m.unbindTooltip();m.remove();m.off();}else m.setMap(null);});}
...
function tdoa_refs_visible_cb(path,checked,first){if(first)return;tdoa.ref_ids.forEach(function(id){w3_checkbox_set('tdoa.refs_'+id,checked?true:false);});tdoa.show_refs=checked;tdoa_rebuild_refs();}
...
function tdoa_rebuild_hosts(opts){if(!tdoa.hosts)return;if(tdoa.cur_host_markers){tdoa.hosts.forEach(function(h){if(tdoa.leaflet){h.mkr.unbindTooltip();h.mkr.remove();h.mkr.off();}else h.mkr.setMap(null);});if(tdoa.leaflet&&tdoa.hosts_clusterer)tdoa.hosts_clusterer.clearLayers();else tdoa.hosts_clusterer.clearMarkers()}
var show_hosts=(opts==undefined)?tdoa.show_hosts:opts.show;tdoa.cur_host_markers=[];for(var i=0;i<tdoa.hosts.length;i++){var h=tdoa.hosts[i];if(h.mkr){if(h.selected){if(tdoa.leaflet)tdoa_style_marker(h.mkr,h.idx,h.id,'host',tdoa.kiwi_map);else h.mkr.setMap(tdoa.kiwi_map);}else{if(show_hosts){if(tdoa.leaflet)tdoa_style_marker(h.mkr,h.idx,h.id,'host');tdoa.cur_host_markers.push(h.mkr);}}}}
...
tdoa.spiderfied.unspiderfy();tdoa.spiderfy_deferred=a.layer;}else{a.layer.spiderfy();tdoa.spiderfied=a.layer;}});mc.on('unspiderfied',function(a){if(tdoa.spiderfy_deferred){tdoa.spiderfy_deferred.spiderfy();tdoa.spiderfied=tdoa.spiderfy_deferred;tdoa.spiderfy_deferred=false;}else{tdoa.spiderfied=false;}});}
...
function tdoa_rebuild_refs(opts){if(!tdoa.refs)return;if(tdoa.cur_ref_markers){tdoa.refs_markers.forEach(function(m){if(tdoa.leaflet){m.unbindTooltip();m.remove();m.off();}else m.setMap(null);});if(tdoa.leaflet&&tdoa.refs_clusterer)tdoa.refs_clusterer.clearLayers();else tdoa.refs_clusterer.clearMarkers()}
var ids=[];tdoa.ref_ids.forEach(function(id){if(w3_checkbox_get('tdoa.refs_'+id))
ids.push(id);});tdoa.cur_ref_markers=[];var show_refs=(opts==undefined)?tdoa.show_refs:opts.show;if(show_refs){for(var i=tdoa.FIRST_REF;i<tdoa.refs.length-1;i++){var r=tdoa.refs[i];var done=false;ids.forEach(function(id){if(r.r.includes(id)&&!done){if(r.selected){if(tdoa.leaflet)
