...
/* web/extensions/TDoA/TDoA.min.js (web/extensions/TDoA/TDoA.js = Sat Feb 10 14:27:29 2024) */
...
el.title=rh.title;el.kiwi_mkr_2_ref_or_host=rh;if(!rh.have_event_listeners[i]){rh.have_event_listeners[i]=true;el.addEventListener('click',function(ev){tdoa_marker_click(ev.target,ev);});el.addEventListener('dblclick',function(ev){tdoa_marker_dblclick(ev.target);});el.addEventListener('mouseenter',function(ev){console.log('tooltip mouseenter');console.log(ev);if(!rh.selected)w3_color(el,'black','yellow');tdoa_clear_unspiderfy_timeout();});el.addEventListener('mouseleave',function(ev){console.log('tooltip mouseleave');if(!rh.selected)rh.type_host?w3_color(el,'white','blue'):w3_color(el,'black','lime');tdoa_set_unspiderfy_timeout();});}}});});if(map){marker.addTo(map);tdoa.map_layers.push(marker);}}
...
function tdoa_clear_results_cb(path,idx,first){if(!(+idx))return;tdoa.result_refs.forEach(function(ref,i){if(tdoa.result_refs.length<=1)return;var mkr=tdoa.result_mkrs.shift();tdoa.result_refs.shift();if(mkr==null)return;console.log(mkr);tdoa_marker_remove(mkr);});}
...
function tdoa_preview_click(mkr,ev){if(tdoa.wf_ws!=null){tdoa.wf_mkr=mkr;tdoa.wf_ws.close();}else{console.log('TDoA aper SAVE '+((wf.aper==kiwi.aper_e.auto)?'AUTO':'MAN'));tdoa.aper_save=wf.aper;tdoa_wf_preview(mkr);}}
...
waterfall_add_queue2(what,ws,firstChars);tdoa.preview_lines++;if(tdoa.preview_lines==4){wf_aper_cb('wf.aper',kiwi.aper_e.man);setTimeout(wf_autoscale_cb,250);}
else
if(kiwi_gc_wf)what=null;}
function tdoa_waterfall_close(){kiwi_clearTimeout(tdoa.preview_timeo);if(tdoa.wf_ws!=null&&tdoa.wf_mkr!=null){var mkr=tdoa.wf_mkr;tdoa.wf_mkr=null;tdoa_wf_preview(mkr);}else{tdoa.wf_ws=tdoa.wf_mkr=null;kiwi.wf_preview_mode=false;console.log('TDoA aper RESTORE '+((tdoa.aper_save==kiwi.aper_e.auto)?'AUTO':'MAN'));wf_aper_cb('wf.aper',tdoa.aper_save);if(tdoa.aper_save==kiwi.aper_e.man){setTimeout(wf_autoscale_cb,500);}
if(!tdoa.wf_conn_bad){w3_clearInnerHTML('id-tdoa-submit-status');}
...
w3_innerHTML('id-tdoa-submit-status','Waterfall preview: '+h.id_snr+' ('+tdoa.wf_url+')');tdoa.wf_ws=open_websocket('W/F',function(){tdoa.preview_lines=0;tdoa.wf_ws.send("SET auth t=kiwi");tdoa.wf_ws.send("SERVER DE CLIENT openwebrx.js W/F");tdoa.wf_ws.send("SET ident_user=TDoA_preview");tdoa.wf_ws.send("SET send_dB=1");tdoa.wf_ws.send('SET zoom='+zoom_level+' start='+x_bin);tdoa.wf_ws.send("SET maxdb=0 mindb=-100");tdoa.wf_ws.send("SET wf_speed=3");tdoa.preview_timeo=setTimeout(function(){if(tdoa.wf_ws)tdoa.wf_ws.close();},10000);},null,null,tdoa_waterfall_add_queue,on_ws_error,tdoa_waterfall_close,{url:tdoa.wf_url,new_ts:true,qs:'',all_msg_cb:tdoa_all_msg_cb});tdoa.wf_conn_bad=false;kiwi.wf_preview_mode=true;}
