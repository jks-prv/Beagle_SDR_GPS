...
/* web/extensions/TDoA/TDoA.min.js (web/extensions/TDoA/TDoA.js = Thu Jun 15 10:00:00 2023) */
...
var lat,lon,zoom,maptype,init_submit;if(tdoa.params){var p=tdoa.params.split(',');tdoa.params=null;for(var i=0,len=p.length;i<len;i++){var a=p[i];console.log('TDoA: param <'+a+'>');var r;if(a.includes(':')){if(a.startsWith('lat:')){lat=parseFloat(a.substring(4));}else
...
w3_text('|color:orange','lat:<i>num</i> lon:<i>num</i> z|zoom:<i>num</i> sample:<i>secs</i> (samp/ref station list) all: hosts:0 submit:')+'<br> List of sampling stations and/or reference station IDs. Case-insensitive and can be abbreviated '+'(e.g. "dcf" matches "DCF77", "cyp2" matches "OTHR/CYP2") <br>'+'sample:<i>secs</i> (one of the time values from the "sample" menu) <br>'+'all: (check "show all results" checkbox) &nbsp; hosts:0 (uncheck "Kiwi hosts" checkbox)<br>'+'submit: (start TDoA process)<br>'+'Example: <i>ext=tdoa,lat:35,lon:35,z:6,cyp2,iu8cri,ur5vib,kuwait,all:,submit:</i> <br>'+''));confirmation_show_content(s,610,350);w3_el('id-confirmation-container').style.height='100%';}
return true;}
function TDoA_focus(){tdoa.optbar=ext_get_optbar();ext_set_optbar('optbar-off');tdoa.pie_size=10;tdoa.pie_cnt=0;tdoa.pie_max=tdoa.sample_time;tdoa.pie_interval=setInterval(function(){if(!w3_el('id-tdoa-pie')){tdoa.pie_cnt=0;return;}
