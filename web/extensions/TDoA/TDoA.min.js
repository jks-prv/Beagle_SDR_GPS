...
/* web/extensions/TDoA/TDoA.min.js (web/extensions/TDoA/TDoA.js = Sat Feb 10 18:00:52 2024) */
...
function tdoa_preview_click(mkr,ev){if(tdoa.wf_ws!=null){tdoa.wf_mkr=mkr;tdoa.wf_ws.close();}else{var auto=(wf.aper==kiwi.aper_e.auto);tdoa.aper_save=wf.aper;tdoa.maxdb_save=auto?wf.save_maxdb:maxdb;tdoa.mindb_un_save=auto?wf.save_mindb_un:mindb_un;tdoa_wf_preview(mkr);}}
...
waterfall_add_queue2(what,ws,firstChars);tdoa.preview_lines++;if(tdoa.preview_lines==2){wf_aper_cb('wf.aper',kiwi.aper_e.man);setTimeout(wf_autoscale_cb,1);}
else
if(kiwi_gc_wf)what=null;}
function tdoa_waterfall_close(){kiwi_clearTimeout(tdoa.preview_timeo);if(tdoa.wf_ws!=null&&tdoa.wf_mkr!=null){var mkr=tdoa.wf_mkr;tdoa.wf_mkr=null;tdoa_wf_preview(mkr);}else{tdoa.wf_ws=tdoa.wf_mkr=null;kiwi.wf_preview_mode=false;var auto=(wf.aper==kiwi.aper_e.auto);wf.save_maxdb=maxdb=tdoa.maxdb_save;wf.save_mindb_un=mindb_un=tdoa.mindb_un_save;wf_aper_cb('wf.aper',tdoa.aper_save);if(!tdoa.wf_conn_bad){w3_clearInnerHTML('id-tdoa-submit-status');}
