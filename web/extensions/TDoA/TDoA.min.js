...
/* web/extensions/TDoA/TDoA.min.js (web/extensions/TDoA/TDoA.js = Thu Feb  8 06:44:34 2024) */
...
return true;}
function TDoA_focus(){tdoa.optbar=ext_get_optbar();zoom_center=0.6;ext_set_optbar('optbar-off','init');tdoa.pie_size=10;tdoa.pie_cnt=0;tdoa.pie_max=tdoa.sample_time;tdoa.pie_interval=setInterval(function(){if(!w3_el('id-tdoa-pie')){tdoa.pie_cnt=0;return;}
...
break;case'monitor':tdoa.wf_conn_bad=true;break;case'wf_setup':if(tdoa.wf_up)break;tdoa.wf_up=true;waterfall_add_line(wf_canvas_actual_line+1);var c=wf_cur_canvas;var x=freq_to_pixel(freq_passband_center())-c.ctx.measureText(tdoa.wf_id_snr).width / 2;waterfall_add_text(wf_canvas_actual_line,x,14,tdoa.wf_id_snr,'Arial',13,'white',4);break;case'maxdb':case'mindb':kiwi_msg(msg_a,ws);break;default:break;}
...
if(tdoa.wf_up){wf_send('SET zoom='+zoom_level+' start='+x_bin);waterfall_add_line(wf_canvas_actual_line+1);var c=wf_cur_canvas;var id='Back to this Kiwi';var x=freq_to_pixel(freq_passband_center())-c.ctx.measureText(id).width / 2;waterfall_add_text(wf_canvas_actual_line,x,14,id,'Arial',13,'white',4);}}
tdoa.wf_up=false;}
function tdoa_wf_preview(mkr){var h=mkr.kiwi_mkr_2_ref_or_host;tdoa.wf_url=h.h+':'+h.p;tdoa.wf_id_snr=h.id_snr
w3_innerHTML('id-tdoa-submit-status','Waterfall preview: '+h.id_snr+' ('+tdoa.wf_url+')');tdoa.wf_ws=open_websocket('W/F',function(){tdoa.wf_ws.send("SET auth t=kiwi");tdoa.wf_ws.send("SERVER DE CLIENT openwebrx.js W/F");tdoa.wf_ws.send("SET ident_user=TDoA_preview");tdoa.wf_ws.send("SET send_dB=1");tdoa.wf_ws.send('SET zoom='+zoom_level+' start='+x_bin);tdoa.wf_ws.send("SET maxdb=0 mindb=-100");tdoa.wf_ws.send("SET wf_speed=3");wf_aper_cb('wf.aper',0);setTimeout(wf_autoscale_cb,500);tdoa.preview_timeo=setTimeout(function(){if(tdoa.wf_ws)tdoa.wf_ws.close();},10000);},null,null,tdoa_waterfall_add_queue,on_ws_error,tdoa_waterfall_close,{url:tdoa.wf_url,new_ts:true,qs:'',all_msg_cb:tdoa_all_msg_cb});tdoa.wf_conn_bad=false;kiwi.wf_preview_mode=true;}
