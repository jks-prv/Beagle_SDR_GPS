...
/* web/extensions/TDoA/TDoA.min.js (web/extensions/TDoA/TDoA.js = Sun Feb  4 20:21:46 2024) */
...
el.title=rh.title;el.kiwi_mkr_2_ref_or_host=rh;el.addEventListener('click',function(ev){tdoa_marker_click(ev.target,ev);});el.addEventListener('dblclick',function(ev){tdoa_marker_dblclick(ev.target);});el.addEventListener('mouseenter',function(ev){if(!rh.selected)w3_color(el,'black','yellow');tdoa_reset_spiderfied();});el.addEventListener('mouseleave',function(ev){if(!rh.selected)rh.type_host?w3_color(el,'white','blue'):w3_color(el,'black','lime');});}});});if(map){marker.addTo(map);tdoa.map_layers.push(marker);}}
...
if(h.mkr){h.id_snr=h.id;if(h.snr>0)h.id_snr+=' '+h.snr;if(h.selected){if(tdoa.leaflet)tdoa_style_marker(h.mkr,h.idx,h.id_snr,'host',tdoa.kiwi_map);else h.mkr.setMap(tdoa.kiwi_map);}else{if(show_hosts){if(tdoa.leaflet)tdoa_style_marker(h.mkr,h.idx,h.id_snr,'host');tdoa.cur_host_markers.push(h.mkr);}}}}
...
function TDoA_blur(){tdoa_waterfall_close();tdoa_clear_all_hosts();ext_set_data_height();if(ext_get_optbar()=='optbar-off'&&tdoa.optbar!='optbar-off')
...
function tdoa_preview_click(mkr,ev){if(tdoa.wf_ws!=null){tdoa.wf_mkr=mkr;tdoa.wf_ws.close();}else{tdoa_wf_preview(mkr);}}
...
break;case'monitor':tdoa.wf_conn_bad=true;break;case'wf_setup':if(tdoa.wf_up)break;tdoa.wf_up=true;waterfall_add_line(wf_canvas_actual_line+1);var c=wf_cur_canvas;var x=(c.width-c.ctx.measureText(tdoa.wf_id_snr).width)/2;waterfall_add_text(wf_canvas_actual_line,x,14,tdoa.wf_id_snr,'Arial',13,'white',4);break;case'maxdb':case'mindb':kiwi_msg(msg_a,ws);break;default:break;}
...
function tdoa_waterfall_close(){kiwi_clearTimeout(tdoa.preview_timeo);if(tdoa.wf_ws!=null&&tdoa.wf_mkr!=null){var mkr=tdoa.wf_mkr;tdoa.wf_mkr=null;tdoa_wf_preview(mkr);}else{tdoa.wf_ws=tdoa.wf_mkr=null;kiwi.wf_preview_mode=false;setTimeout(wf_autoscale_cb,500);if(!tdoa.wf_conn_bad){w3_clearInnerHTML('id-tdoa-submit-status');}
if(tdoa.wf_up){wf_send("SET zoom=10 start="+x_bin);waterfall_add_line(wf_canvas_actual_line+1);var c=wf_cur_canvas;var id='Back to this Kiwi';var x=(c.width-c.ctx.measureText(id).width)/2;waterfall_add_text(wf_canvas_actual_line,x,14,id,'Arial',13,'white',4);}}
tdoa.wf_up=false;}
function tdoa_wf_preview(mkr){var h=mkr.kiwi_mkr_2_ref_or_host;tdoa.wf_url=h.h+':'+h.p;tdoa.wf_id_snr=h.id_snr
w3_innerHTML('id-tdoa-submit-status','Waterfall preview: '+h.id_snr+' ('+tdoa.wf_url+')');tdoa.wf_ws=open_websocket('W/F',function(){tdoa.wf_ws.send("SET auth t=kiwi");tdoa.wf_ws.send("SERVER DE CLIENT openwebrx.js W/F");tdoa.wf_ws.send("SET ident_user=TDoA_preview");tdoa.wf_ws.send("SET send_dB=1");tdoa.wf_ws.send("SET zoom=10 start="+x_bin);tdoa.wf_ws.send("SET maxdb=0 mindb=-100");tdoa.wf_ws.send("SET wf_speed=3");wf_aper_cb('wf.aper',0);setTimeout(wf_autoscale_cb,500);},null,null,tdoa_waterfall_add_queue,on_ws_error,tdoa_waterfall_close,{url:tdoa.wf_url,new_ts:true,qs:'',all_msg_cb:tdoa_all_msg_cb});tdoa.wf_conn_bad=false;tdoa.preview_timeo=setTimeout(function(){if(tdoa.wf_ws)tdoa.wf_ws.close();},10000);kiwi.wf_preview_mode=true;}
