...
var lat,lon,zoom,maptype,init_submit;console.log(tdoa.params);if(tdoa.params){var p=tdoa.params.split(',');tdoa.params=null;for(var i=0,len=p.length;i<len;i++){var a=p[i];console.log('TDoA: param <'+a+'>');var r;if(a.includes(':')){if(a.startsWith('lat:')){lat=parseFloat(a.substring(4));}else
...
kiwi_ajax(tdoa.url_files+tdoa.response.key+'/status.json',function(j){console.log('tdoa_submit_status_new_cb');console.log(j);var okay=0;var info=undefined;var err='unknown status returned';var per_file,status,message;if(j.AJAX_error){if(j.AJAX_error=='status'){err='status file missing';okay=1;}else{var nj=j.response.replace(/\n/g,'');j=kiwi_JSON_parse('tdoa_submit_status_new_cb',nj);if(j){okay=0;}else{err='status JSON parse error';okay=2;}}}
...
var show_hosts=(opts==undefined)?tdoa.show_hosts:opts.show;tdoa.cur_host_markers=[];for(var i=0;i<tdoa.hosts.length;i++){var h=tdoa.hosts[i];var v=+h.v;if(tdoa.hosts_sel&&((tdoa.hosts_sel==1&&v>1.646)||(tdoa.hosts_sel==2&&v<1.653))){continue;}
if(h.mkr){var id=h.id;if(h.snr>0)id+=' '+h.snr;if(h.selected){if(tdoa.leaflet)tdoa_style_marker(h.mkr,h.idx,id,'host',tdoa.kiwi_map);else h.mkr.setMap(tdoa.kiwi_map);}else{if(show_hosts){if(tdoa.leaflet)tdoa_style_marker(h.mkr,h.idx,id,'host');tdoa.cur_host_markers.push(h.mkr);}}}}
...
return true;}
function TDoA_focus(){tdoa.optbar=ext_get_optbar();ext_set_optbar('optbar-off','init');tdoa.pie_size=10;tdoa.pie_cnt=0;tdoa.pie_max=tdoa.sample_time;tdoa.pie_interval=setInterval(function(){if(!w3_el('id-tdoa-pie')){tdoa.pie_cnt=0;return;}
kiwi_draw_pie('id-tdoa-pie',tdoa.pie_size,tdoa.pie_cnt / tdoa.pie_max);tdoa.pie_cnt=Math.min(tdoa.pie_cnt+1,tdoa.pie_max);},1000);}
function TDoA_blur(){ext_set_data_height();if(ext_get_optbar()=='optbar-off'&&tdoa.optbar!='optbar-off')
