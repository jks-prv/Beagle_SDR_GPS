...
function prefs_check()
{
   ...
   setTimeout(prefs_check, 1000);
}

function prefs_load(cb)
{
   if (kiwi.xdLocalStorage_ready) {
      var id = ident_user? ident_user : '_no_user_';
      xdLocalStorageHA.getItem('prefs.'+ id, function(d) {
      ...
      setTimeout(prefs_load, 250, cb);
   }
}

function prefs_save(cb)
{
	var id = ident_user? ident_user : '_no_user_';
	prefs.id = id;
	var val = JSON.stringify(prefs.pobj);
	xdLocalStorageHA.setItem('prefs.'+ id, val, function() {
		console.log('prefs_save key=prefs.'+ id);
		console.log(val);
		if (cb) cb();
	});
}

function prefs_focus()
{
	console.log('prefs_focus');
}

function prefs_blur()
{
	console.log('prefs_blur');
}

function prefs_help(show)
{
   if (show) {
      var s = 
         w3_text('w3-medium w3-bold w3-text-aqua', 'User preferences help') +
