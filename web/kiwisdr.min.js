...
/* web/extensions/ext.min.js (web/extensions/ext.js = Tue Mar 12 19:18:02 2024) */
...
function ext_hasCredential(conn_type,cb,cb_param,ws){if(conn_type=='mfg')conn_type='admin';var pwd;if(conn_type=='admin'){if(kiwi.admin_save_pwd){pwd=kiwi_storeGet(conn_type,'');}else{pwd='';}
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Sun Mar 10 15:52:35 2024) */
...
var prot=(kiwi_url_param(['p','prot','protected'],true,false)&&conn_kiwi);if(prot)s1='You have requested a password protected channel<br>';var user_login=w3_innerHTML('id-kiwi-user-login').trim();var s=isNonEmptyString(user_login)?(user_login+'<br><br>'):'KiwiSDR: software-defined receiver<br>';s+=s1+try_again+
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Sun Mar 10 17:20:38 2024) */
...
console.log('kiwi_open_or_reload_page: '+url);if(w3_opt(obj,'tab')){window.open(url,'_blank');}else{window.location.href=url;}}
...
var sendmail=function(to,subject){var s="mailto:"+enc(decodeURIComponent(to))+(isDefined(subject)?('?subject='+subject):'');kiwi_open_or_reload_page({url:s});}
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Tue Mar 12 19:32:25 2024) */
...
pb_adj_cf.style.left=px(from_px+env_adj);pb_adj_cf.style.width=px(Math.max(0,to_px-from_px-2*env_adj));pb_adj_lo.style.left=px(from_px-env_slope-env_adj);pb_adj_lo.style.width=px(env_slope+2*env_adj);pb_adj_hi.style.left=px(to_px-env_adj);pb_adj_hi.style.width=px(env_slope+2*env_adj);from_px-=(env_att_w+env_bounding_line_w);to_px+=(env_att_w+env_bounding_line_w);scale_ctx.lineWidth=3;scale_ctx.strokeStyle=color;scale_ctx.fillStyle=color;var drag_ranges={envelope_on_screen:false,line_on_screen:false,allow_pb_adj:false};if(!(to_px<0||from_px>window.innerWidth)){drag_ranges.beginning={x1:from_px-env_adj,x2:from_px+env_slope+env_adj};drag_ranges.ending={x1:to_px-env_slope-env_adj,x2:to_px+env_adj};drag_ranges.whole_envelope={x1:from_px,x2:to_px};owrx.pbx1=from_px;owrx.pbx2=to_px;drag_ranges.envelope_on_screen=true;drag_ranges.allow_pb_adj=((to_px-from_px)>=50);if(!drag_ranges.allow_pb_adj)
...
if(x1>0&&x2<sw1&&wfext.spb_on){pbctx.fillStyle=wfext.spb_color;pbctx.fillRect(x1,0,x2-x1,sh);}
