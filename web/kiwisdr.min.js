...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Mon Dec 19 09:56:50 2022) */
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Mon Dec 19 14:22:02 2022) */
...
if(isDefined(s_a[i]))
s+=s_a[i];dx_div.innerHTML=s;dx.last_ident='';if(!dx.font){var el=w3_el('0-id-dx-label');if(el){dx.font=css_style(el,'font-size')+' '+css_style(el,'font-family');console_log_lbl('dx.font=<'+dx.font+'>');}}
...
return s;};for(i=1;i<arr.length;i++){dx_idx=i-1;obj=arr[i];var el;var ident=obj.i;var freq=obj.f;if(eibi){if(ident==dx.last_ident){console_log_lbl('2222 dx_idx='+dx_idx+' last_dx_idx='+dx.last_dx_idx+' '+obj.f+' SAME AS LAST '+ident);el=w3_el(dx.last_dx_idx+'-id-dx-label');el.title+='\n'+dx_title(obj);continue;}}
dx.last_dx_idx=dx_idx;dx.last_ident=ident;if(eibi)console_log_lbl('2222 dx_idx='+dx_idx+' '+obj.f+' OK '+ident);var notes=(isDefined(obj.n))?obj.n:'';el=w3_el(dx_idx+'-id-dx-label');if(!el)continue;var _ident=kiwi_decodeURIComponent('dx_ident2',ident).trim();if(_ident=='')_ident='&nbsp;&nbsp;';el.innerHTML=_ident.replace(/\\n/g,'<br>');var idx=dx_type2idx(obj.fl&dx.DX_TYPE);var ex=(eibi&&dx.eibi_ext[idx]!='')?'\nshift-click to open extension':'';if(eibi)console_log_lbl(obj.i+' '+idx+' '+dx.eibi_ext[idx]+' '+ex);if(eibi){el.title=dx.eibi_svc_s[idx]+' // home country: '+obj.c+ex+'\n'+dx_title(obj);}else{var title=kiwi_decodeURIComponent('dx_notes',notes);title=title.replace(/<br>/g,'\n');title=title.replace(/\\n/g,'\n');var dow=obj.fl&dx.DX_DOW;if(dow!=dx.DX_DOW||(obj.b!=0||!(obj.b==0&&obj.e==2400)))
