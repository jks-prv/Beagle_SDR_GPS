...
/* web/extensions/ext.min.js (web/extensions/ext.js = Wed Jun 14 09:23:40 2023) */
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Tue Jun 13 14:37:08 2023) */
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Wed Jun 14 09:44:54 2023) */
...
function w3_ext_param(s,param){var rv={match:false,full_match:false};var pu=param.split(':');var pl=param.toLowerCase().split(':');if(s.startsWith(pl[0])){rv.match=true;if(pl.length>1){rv.has_value=true;rv.num=parseFloat(pl[1]);rv.string=pl[1];rv.string_case=pu[1];}else{rv.has_value=false;rv.num=0;rv.string='';}}
if(s==pl[0])rv.full_match=true;return rv;}
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Wed Jun 14 11:50:59 2023) */
...
console.log('DX URL DONE db='+dx.db+' open='+open+' eibi_types_mask='+dx.eibi_types_mask.toHex(8)+' help='+dx.help);dx_schedule_update();},null,500);}else{dx.db=kiwi_storeGet('last_db',cfg.dx_default_db);}
...
function dx_color_init(){if(dx.db!=dx.DB_EiBi){var _dxcfg=dx_cfg_db();for(var i=0;i<_dxcfg.dx_type.length;i++){var o=_dxcfg.dx_type[i];var color=(o.color=='')?'white':o.color;var colorObj=w3color(color);if(colorObj.valid){if(dx.db==dx.DB_STORED)
...
w3_text('w3-margin-TB-8 w3-text-css-yellow','The community database is read-only and downloaded periodically. <br>'+
...
if(dx.help){setTimeout(function(){extint_help_click_now();},1000);dx.help=false;}},function(){},true);ext_set_controls_width_height(550,290);}
function dx_database_cb(path,idx,first,opt,from_shortcut){if(first)return;dx.list=[];console_log_dbgUs('DX DB-SWITCHED db='+dx.db+' opt='+kiwi_JSON(opt));if(w3_opt(opt,'toggle')){dx.filter_tod[dx.db]^=1;dx_time_dow_cb(dx.db.toString()+'-filter-tod',dx.filter_tod[dx.db]);}else{dx.db=+idx;if(w3_opt(opt,'open')){dx_show_edit_panel(null,-1,from_shortcut);}else{if(ext_panel_displayed('dx'))
dx_close_edit_panel();}}
