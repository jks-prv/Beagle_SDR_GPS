...
/* web/extensions/ext.min.js (web/extensions/ext.js = Wed Mar 13 13:00:22 2024) */
...
function ext_get_optbar(){var optbar=readCookie('last_optbar');return optbar;}
function ext_set_optbar(optbar,cb_param){if(extint.optbars[optbar]){writeCookie('last_optbar',optbar);w3_click_nav('id-navbar-optbar',optbar,'optbar',cb_param);}}
function ext_hasCredential(conn_type,cb,cb_param,ws){if(conn_type=='mfg')conn_type='admin';var pwd;if(conn_type=='admin'){if(kiwi.admin_save_pwd){pwd=kiwi_storeGet(conn_type,'');}else{pwd='';kiwi_storeSet('admin',pwd);}
deleteCookie('admin');deleteCookie('admin-pwd');}else{pwd=readCookie(conn_type);}
pwd=pwd?decodeURIComponent(pwd):'';extint.pwd_cb=cb;extint.pwd_cb_param=cb_param;ext_valpwd(conn_type,pwd,ws);}
function ext_valpwd(conn_type,pwd,ws){if(conn_type=='mfg')conn_type='admin';pwd=encodeURIComponent(pwd);if(conn_type=='admin'){if(kiwi.admin_save_pwd){kiwi_storeSet('admin',pwd);}}else{writeCookie(conn_type,pwd);}
extint.conn_type=conn_type;pwd=(pwd!='')?pwd:'#';var ipl=null;var iplimit_cookie=readCookie('iplimit');var iplimit_pwd=kiwi_url_param(['pwd','password'],'','');if(iplimit_pwd!=''){ipl=iplimit_pwd;}else
...
w3_select_value(el,value);var menu=el.childNodes;var name=menu[value+1].innerHTML.toLowerCase();var idx;extint_enum_names(function(i,value,id,id_en){if(name.startsWith(id.toLowerCase())){idx=i;}});if(name=='ant_switch'){ant_switch_view();}else
if(name=='waterfall'){waterfall_view();}else
if(name=='noise_blank'){noise_blank_view();}else
if(name=='noise_filter'){noise_filter_view();}else{extint_blur_prev(0);w3_call(extint.hide_func);extint.current_ext_name=extint.ext_names[idx];if(extint.first_ext_load){extint_connect_server();extint.first_ext_load=false;}else{ext_send('SET ext_is_locked_status');}}}
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Wed Mar 13 10:58:30 2024) */
...
((param[1]!='')?(' Reason: '+kiwi_decodeURIComponent('reason_disabled',param[1])):'');break;case"sample_rate":extint.srate=parseFloat(param[1]);break;case'pref_import_ch':kiwi.prefs_import_ch=+param[1];break;case'pref_import':prefs_import_cb(param[1],kiwi.prefs_import_ch);break;case'adc_clk_nom':extint.adc_clock_nom_Hz=+param[1];break;case'notify_msg':var s=kiwi_decodeURIComponent('notify_msg',param[1]);console.log('notify_msg: '+s);if(confirmation.displayed)break;s=w3_div('',s);confirmation_show_content(s,425,50);setTimeout(confirmation_panel_close,3000);break;case'kiwi_kick':var s=param[1]+((cfg.reason_kicked!='')?('<br>Reason: '+cfg.reason_kicked):'');s=kiwi_decodeURIComponent('kiwi_kick',s);if(confirmation.displayed)break;s=w3_div('',s);confirmation_show_content(s,425,75,null,'red');break;case'snr_stats':var p=param[1].split(',');kiwi_snr_stats(p[0],p[1]);break;default:if(param[0].startsWith('antsw_')){if(ant_switch_msg(param))
break;}
rtn=false;break;}
return rtn;}
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Tue Mar 12 09:30:37 2024) */
...
if(!website)console.log('Safari='+kiwi_isSafari()+' Firefox='+kiwi_isFirefox()+' Chrome='+kiwi_isChrome()+' Opera='+kiwi_isOpera());if(!website){kiwi_load_js_dir('pkgs/js/',['sprintf/sprintf.js','SHA256.js','coloris/coloris.js'],function(){kiwi_load2();});}else{kiwi_load2();}}}
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Tue Mar 12 11:20:02 2024) */
...
if(el.id.toLowerCase().startsWith(next_id_nav)){next_id=el.id.split('id-nav-')[1];next_el=el;found++;}});if(cur_id)
...
function w3_json_set_cfg_cb(path,val,first){console.log('w3_json_set_cfg_cb: path='+path+' '+typeof(val)+' "'+val+'" first='+first);var save=isArg(first)?(first?false:true):true;ext_set_cfg_param(path,val,save);}
...
/* web/kiwi/waterfall.min.js (web/kiwi/waterfall.js = Wed Mar 13 12:26:47 2024) */
...
function wfext_spb_color_cb(path,val,first,cbp){w3_string_cb(path,val);kiwi_storeSet('last_spb_color',val);wfext.spb_color_seq++;}
function wfext_spb_on_cb(path,checked,first){if(first)return;w3_bool_cb(path,checked,first);kiwi_storeSet('last_spb_on',checked?1:0);wfext.spb_color_seq++;}
...
/* web/kiwi/noise_blank.min.js (web/kiwi/noise_blank.js = Fri Mar  1 06:47:28 2024) */
...
function nb_algo_cb(path,idx,first,from){if(first)return;idx=+idx;w3_select_value(path,idx,{all:1});noise_blank.algo=idx;kiwi_storeSet('last_nb_algo',idx.toString());noise_blank_send();noise_blank_controls_refresh();}
...
/* web/kiwi/noise_filter.min.js (web/kiwi/noise_filter.js = Fri Mar  1 06:47:34 2024) */
...
function nr_algo_cb(path,idx,first,from){if(first)return;idx=+idx;w3_select_value(path,idx,{all:1});noise_filter.algo=idx;kiwi_storeSet('last_nr_algo',idx.toString());if(from=='m'&&!noise_filter.denoise&&!noise_filter.autonotch&&(noise_filter.algo==noise_filter.NR_ORIG||noise_filter.algo==noise_filter.NR_WDSP)){noise_filter.denoise=1;}
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Tue Mar 12 19:22:07 2024) */
var ant_sw={isConfigured:false,update_seq:0,denymixing:0,thunderstorm:0,ver:0,backend:-1,backend_s:null,backends_s:null,n_ant:8,ip_or_url:null,exantennas:0,notify_timeout:null,url_ant:null,url_deselected:false,url_idx:0,desc_lc:[],last_offset:-1,last_high_side:-1,EVERYONE:0,LOCAL_CONN:1,LOCAL_OR_PWD:2,denyswitching:0,deny_s:['everyone','local connections only','local connections or user password only'],denied_because_multiuser:false};function ant_switch_view(){keyboard_shortcut_nav('rf');var Hrf;w3_do_when_cond(function(){Hrf=w3_el('id-optbar-rf').clientHeight;return(Hrf!=0);},function(){var Hopt=kiwi.OPTBAR_CONTENT_HEIGHT;var Hant=w3_el('id-optbar-rf-antsw').clientHeight+8;pct=w3_clamp(kiwi_round(1-(Hant-Hopt)/(Hrf-Hopt),2),0,1);w3_scrollTo('id-optbar-content',pct);},null,200);var p=ext_param();if(isNonEmptyString(p)){console.log('ant_switch_view: EXT param = <'+p+'>');ant_sw.url_ant=p.split(',');ant_switch_prompt_query();}}
...
break;case"antsw_AntennasAre":ant_switch_process_reply(param[1]);break;case"antsw_AntennaDenySwitching":ant_sw.denyswitching=(param[1]==0)?ant_sw.EVERYONE:ant_sw.LOCAL_CONN;ant_sw.denied_because_multiuser=(param[1]==2);ant_switch_showpermissions();break;case"antsw_AntennaDenyMixing":if(param[1]==1)ant_sw.denymixing=1;else ant_sw.denymixing=0;ant_switch_showpermissions();break;case"antsw_Thunderstorm":if(param[1]==1){ant_sw.thunderstorm=1;ant_sw.denyswitching=ant_sw.LOCAL_CONN;}else{ant_sw.thunderstorm=0;}
ant_switch_showpermissions();break;default:console.log('ant_switch_msg: UNKNOWN CMD '+param[0]);rv=false;break;}
return rv;}
function ant_switch_buttons_setup(){var antdesc=[];var tmp;for(tmp=1;tmp<=ant_sw.n_ant;tmp++){console.log('### ant_switch_buttons_setup '+tmp);antdesc[tmp]=ext_get_cfg_param_string('ant_switch.ant'+tmp+'desc','',EXT_NO_SAVE);}
console.log('ant_switch: Antenna configuration ant_sw.n_ant='+ant_sw.n_ant);var buttons_html='';var n_ant=0;for(tmp=1;tmp<=ant_sw.n_ant;tmp++){if(antdesc[tmp]==undefined||antdesc[tmp]==null||antdesc[tmp]==''){antdesc[tmp]='';}else{buttons_html+=w3_div('w3-valign w3-margin-T-8',w3_button('id-antsw-btn w3-padding-smaller w3-round-6px','Antenna '+tmp,'ant_switch_select_antenna_cb',tmp),w3_div('w3-margin-L-8',antdesc[tmp]));n_ant++;}
ant_sw.desc_lc[tmp]=antdesc[tmp].toLowerCase();console.log('ant_switch: Antenna '+tmp+': '+antdesc[tmp]);if(!ant_sw.isConfigured){if(isNonEmptyString(antdesc[tmp]))
ant_sw.isConfigured=true;}}
w3_innerHTML('id-antsw-user',buttons_html);console.log('ant_switch_buttons_setup DONE');}
function ant_switch_user_init(){ant_sw.update_seq=ext_get_cfg_param('ant_switch.update_seq',0,EXT_NO_SAVE);ant_sw.denyswitching=ext_get_cfg_param('ant_switch.denyswitching',ant_sw.EVERYONE,EXT_NO_SAVE);ant_sw.denymixing=ext_get_cfg_param('ant_switch.denymixing','',EXT_NO_SAVE)?1:0;ant_sw.thunderstorm=ext_get_cfg_param('ant_switch.thunderstorm','',EXT_NO_SAVE)?1:0;var controls_html=w3_div('id-antsw-controls w3-margin-right w3-text-white',w3_inline('w3-gap-12/',w3_div('w3-text-aqua','<b>Antenna switch</b>'),w3_div('w3-text-white','by Kari Karvonen'),w3_button('id-antsw-help-btn w3-right w3-green w3-small w3-padding-small||onclick="ant_switch_help()"','help')),w3_div('w3-margin-LR-16',w3_div('id-antsw-display-selected w3-margin-T-4'),w3_div('id-antsw-display-permissions'),w3_div('id-antsw-user w3-margin-B-8')));w3_innerHTML('id-optbar-rf-antsw',controls_html);console.log('SET antsw_init');ext_send('SET antsw_init');ant_switch_buttons_setup();ant_switch_prompt_query();var p=kiwi_url_param('ant');console.log('$p='+p);if(isArg(p)){if(isNonEmptyString(p)){console.log('ant_switch_user_init: URL param = <'+p+'>');ant_sw.url_ant=p.split(',');}
ant_switch_view();}}
function ant_switch_blur(){}
function ant_switch_display_update(ant){w3_innerHTML('id-antsw-display-selected',ant);}
function ant_switch_select_antenna(ant){console.log('ant_switch: switching to antenna '+ant);ext_send('SET antsw_SetAntenna='+ant);ant_switch_prompt_query();}
...
function ant_switch_process_reply(ant_selected_antenna){var need_to_inform=false;if(cfg.ant_switch.update_seq!=ant_sw.update_seq){console.log('ant_switch_process_reply: update_seq='+cfg.ant_switch.update_seq+'|'+ant_sw.update_seq);ant_switch_buttons_setup();ant_sw.update_seq=cfg.ant_switch.update_seq;}
ant_sw.denyswitching=ext_get_cfg_param('ant_switch.denyswitching',ant_sw.EVERYONE,EXT_NO_SAVE);if(!ant_sw.isConfigured){ant_switch_display_update('Antenna switch is not configured.');return;}
...
var selected_antennas_list=ant_selected_antenna.split(',');var re=/^Antenna ([1]?[0-9]+)/i;w3_els('id-antsw-btn',function(el,i){if(!el.textContent.match(re))return;w3_unhighlight(el);var antN=el.textContent.parseIntEnd();if(!isArray(selected_antennas_list))return;if(selected_antennas_list.indexOf(antN.toString())<0)return;w3_highlight(el);if(ant_sw.denymixing&&selected_antennas_list.length==1){var s='ant_switch.ant'+antN+'offset';var offset=ext_get_cfg_param(s,'',EXT_NO_SAVE);offset=+offset;if(!isNumber(offset))offset=0;if(offset!=ant_sw.last_offset){ext_send('SET antsw_freq_offset='+offset);ant_sw.last_offset=offset;}
...
function ant_switch_lock_buttons(lock){w3_els('id-antsw-btn',function(el,i){var re=/^Antenna ([1]?[0-9]+)/i;if(el.textContent.match(re)){w3_disable(el,lock);}
var re=/^Ground all$/i;if(el.textContent.match(re)){w3_disable(el,lock);}});}
function ant_switch_showpermissions(){if(!ant_sw.isConfigured){w3_innerHTML('id-antsw-display-permissions','');return;}
if(ant_sw.denyswitching==ant_sw.LOCAL_CONN){ant_switch_lock_buttons(true);var reason=ant_sw.denied_because_multiuser?'Multiple users online.':'';w3_innerHTML('id-antsw-display-permissions','Switching denied: '+reason);}else{ant_switch_lock_buttons(false);if(ant_sw.denymixing){w3_innerHTML('id-antsw-display-permissions','Switching is allowed. Mixing not allowed.');}else{w3_innerHTML('id-antsw-display-permissions','Switching and mixing are allowed.');}}
if(ant_sw.thunderstorm==1){ant_switch_lock_buttons(true);w3_innerHTML('id-antsw-display-permissions',w3_text('w3-text-css-yellow','Thunderstorm. Switching is denied.'));}}
...
w3_select_set_if_includes('ant_sw.backend',ant_sw.backend_s);return true;case"antsw_ver":console.log('antsw_ver='+param[1]);ant_sw.ver=param[1];w3_innerHTML('id-antsw-ver','backend script v'+ant_sw.ver);return true;case"antsw_nch":console.log('antsw_nch='+param[1]);ant_switch_config_html2(+param[1]);return true;case"antsw_ip_or_url":console.log('antsw_ip_or_url='+param[1]);ant_sw.ip_or_url=param[1];w3_set_value('id-ant_sw.ip_or_url',ant_sw.ip_or_url);return true;}
return false;}
...
function ant_switch_users_notify_change(){console.log('ant_switch_users_notify_change');kiwi_clearTimeout(ant_sw.notify_timeout);ant_sw.notify_timeout=setTimeout(function(){ant_sw.update_seq++;ext_set_cfg_param('ant_switch.update_seq',ant_sw.update_seq,EXT_SAVE);setTimeout(function(){ext_send('ADM antsw_notify_users');},1);console.log('ant_sw reload_cfg');},1000);}
...
function ant_denyswitch_cb(path,val,first){console.log('ant_denyswitch_cb path='+path+' val='+val+'('+w3_switch_s(val)+') first='+first);w3_int_set_cfg_cb(path,val);ant_switch_users_notify_change();}
...
function ant_switch_config_html2(n_ch){if(n_ch)ant_sw.n_ant=n_ch;console.log('ant_switch_config_html2 n_ch='+n_ch);var s='';for(var i=1;i<=ant_sw.n_ant;i++){s+=w3_inline_percent('w3-margin-T-16 w3-valign-center/',w3_input_get('','Antenna '+i+' description','ant_switch.ant'+i+'desc','ant_switch_desc_cb',''),50,'&nbsp;',3,w3_checkbox_get_param('//w3-label-inline','Default<br>antenna','ant_switch.ant'+i+'default','admin_bool_cb',false),7,'&nbsp;',3,w3_checkbox_get_param('w3-restart//w3-label-inline','High-side<br>injection','ant_switch.ant'+i+'high_side','admin_bool_cb',false),10,'&nbsp;',1,w3_input_get('w3-restart//','Frequency scale offset (kHz)','ant_switch.ant'+i+'offset','w3_int_set_cfg_cb',0));}
w3_innerHTML('id-antsw-list',s);}
function ant_switch_config_html(){ext_send('ADM antsw_GetBackends');ext_send('ADM antsw_GetInfo');ant_sw.update_seq=ext_get_cfg_param('ant_switch.update_seq',0,EXT_NO_SAVE);ant_sw.denyswitching=ext_get_cfg_param('ant_switch.denyswitching',ant_sw.EVERYONE,EXT_NO_SAVE);if(ant_sw.denyswitching=='')ant_sw.denyswitching=ant_sw.EVERYONE;ext_admin_config('ant_switch','Antenna switch',w3_div('id-antsw w3-text-teal','<b>Antenna switch configuration</b>'+'<hr>'+
w3_div('',w3_inline('w3-gap-32/',w3_div('id-antsw-backends'),w3_div('id-antsw-ver'),w3_input('/w3-label-inline w3-label-not-bold w3-nowrap/w3-padding-small||size=40','IP address or URL:','ant_sw.ip_or_url',ant_sw.ip_or_url,'ant_switch_set_ip_or_url_cb')),w3_div('','<br>If antenna switching is denied then users cannot switch antennas. <br>'+'Admin can always switch antennas, either from a user connection on the local network, or from the <br>'+'admin console tab using the script: <x1>/root/Beagle_SDR_GPS/pkgs/ant_switch/ant-switch-frontend</x1> <br><br>'+'The last menu option allows anyone connecting using a password to switch antennas <br>'+'(i.e. time limit exemption password on the admin page control tab, not the user login password). <br>'+'Other connections made without passwords are denied.'),w3_select('w3-width-auto w3-label-inline w3-margin-T-8|color:red','Allow antenna switching by:','','ant_switch.denyswitching',ant_sw.denyswitching,ant_sw.deny_s,'ant_denyswitch_cb'),w3_div('w3-margin-T-16','If antenna mixing is denied then users can select only one antenna at time.'),w3_switch_label_get_param('w3-label-inline w3-label-left w3-margin-T-8','Deny antenna mixing?','Yes','No','ant_switch.denymixing',true,false,'ant_switch_cb'),w3_div('w3-margin-T-16','If multiuser is denied then antenna switching is disabled when more than one user is online.'),w3_switch_label_get_param('w3-label-inline w3-label-left w3-margin-T-8','Deny multiuser switching?','Yes','No','ant_switch.denymultiuser',true,false,'ant_switch_cb'),w3_div('w3-margin-T-16','If thunderstorm mode is activated, all antennas and forced to ground and switching is disabled.'),w3_switch_label_get_param('w3-label-inline w3-label-left w3-margin-T-8','Enable thunderstorm mode?','Yes','No','ant_switch.thunderstorm',true,false,'ant_switch_cb'),w3_div('w3-margin-T-16','The <x1>Default antenna</x1> checkboxes below define which antenna(s) are selected initially. <br>'+'And also when no users are connected if the switch below is set to <x1>Yes</x1>. <br>'+'"Users" includes all connections including FT8/WSPR autorun and kiwirecorder (e.g. wsprdaemon).'),w3_switch_label_get_param('w3-label-inline w3-label-left w3-margin-T-8','Switch to default antenna(s) when no users connected?','Yes','No','ant_switch.default_when_no_users',true,true,'admin_radio_YN_cb'),w3_div('','<hr><b>Antenna buttons configuration</b><br>'),w3_col_percent('w3-margin-T-16/','Leave antenna description field empty if you want to hide antenna button from users. <br>'+'For two-line descriptions use break sequence &lt;br&gt; between lines.',74,'Overrides frequency scale offset value on<br>'+'config tab when any antenna selected. <br>'+'No effect if antenna mixing enabled.'),w3_div('',w3_div('id-antsw-list'),w3_col_percent('w3-margin-T-16/',w3_input_get('','Antenna switch failure or unknown status decription','ant_switch.ant0desc','w3_string_set_cfg_cb',''),70)))));}





/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Thu Mar 14 17:51:49 2024) */
...
function init_top_bar(){w3_el('id-top-bar').addEventListener('click',freqset_select,false);var rx_loc=kiwi_decodeURIComponent('RX_LOC',cfg.index_html_params.RX_LOC);w3_innerHTML('id-rx-desc',rx_loc+' | Grid '+
...
afft.o_re=ooura32.vectorArrayFactory(afft.offt);afft.o_im=ooura32.vectorArrayFactory(afft.offt);afft.scale=10.0*2.0 /(afft.size*afft.size*afft.size*afft.CUTESDR_MAX_VAL*afft.CUTESDR_MAX_VAL);if(!afft.init&&wf.aper!=kiwi.APER_AUTO&&wf_mm==''){wf.need_autoscale=16;}
...
function wf_aper_cb(path,idx,first){if(first)return;idx=+idx;wf.aper=w3_clamp(idx,0,kiwi.aper_s.length-1,0);w3_select_value(path,idx);writeCookie('last_aper',idx);var auto=(wf.aper==kiwi.APER_AUTO);w3_innerHTML('id-wf-sliders',auto?wf.floor_ceil_sliders:wf.max_min_sliders);w3_color('id-button-wf-autoscale',null,auto?w3_selection_green:'');if(auto){wf.save_maxdb=maxdb;wf.save_mindb_un=mindb_un;if(wf.audioFFT_active)
