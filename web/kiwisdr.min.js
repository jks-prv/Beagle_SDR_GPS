...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Thu Jan  5 14:22:50 2023) */
...
s='mute';if(q[s])muted_initially=parseInt(q[s]);s='wf';if(q[s])wf_rate=q[s];s='wfm';if(q[s])wf_mm=q[s];s='wfa';if(q[s])wf_auto=parseInt(q[s]);s='cmap';if(q[s])wf.cmap_override=w3_clamp(parseInt(q[s]),0,kiwi.cmap_s.length-1,0);s='sqrt';if(q[s])wf.sqrt=w3_clamp(parseInt(q[s]),0,4,0);s='wfts';if(q[s])wf.url_tstamp=w3_clamp(parseInt(q[s]),2,60*60,2);s='peak';if(q[s])peak_initially=parseInt(q[s]);s='no_geo';if(q[s])no_geoloc=true;s='keys';if(q[s])shortcut.keys=isNonEmptyString(q[s])?q[s].split(''):null;s='user';if(q[s])user_url=q[s];s='u';if(q[s])user_url=q[s];s='m';if(q[s])force_mobile=true;s='mobile';if(q[s])force_mobile=true;s='mem';if(q[s])owrx.override_fmem=q[s];s='sqth';if(q[s])squelch_threshold=parseFloat(q[s]);s='click';if(q[s])nb_click=true;s='nocache';if(q[s]){param_nocache=true;nocache=parseInt(q[s]);}
...
for(i=0;i<=dx.f_same.length;i++){var idx=dx.f_same[i];j=i%(n+1);dx.post_render[idx]={top:dx_label_top+(j*spacing),ltop:dx_label_top,x:x+j*(spacing-1),z:z+i};if(j==n){x+=gap;}}};if(dbg_lbl){console.log('######## dx_label_cb db='+dx.db+' arr.len='+arr.length);console.log(arr);}
...
dx.last_idx=dx_idx;dx.last_freq=freq;dx.last_ident=ident;console_log_lbl('1111 dx_idx='+dx_idx+' '+freq+' OK filtered='+(filtered?1:0)+' '+ident);var gid=obj.g;var f_base=freq-kiwi.freq_offset_kHz;var moff=obj.o;var notes=isDefined(obj.n)?obj.n:'';var params=isDefined(obj.p)?obj.p:'';if(eibi&&(flags&dx.DX_HAS_EXT))params=dx.eibi_ext[dx_type2idx(type)];var f_baseHz=f_base*1000;var f_base_label_Hz=f_baseHz+passband_offset_dxlabel(kiwi.modes_l[flags&dx.DX_MODE]);var x=scale_px_from_freq(f_base_label_Hz,g_range)-1;var cmkr_x=0;var carrier=f_baseHz-moff;if(moff)cmkr_x=scale_px_from_freq(carrier,g_range);if(eibi&&f_base_label_Hz==dx.last_f_base){dx.f_same.push(dx_idx);same_x=x;if(lock_z==0)lock_z=dx_z;}else{if(eibi){if(dx.f_same.length>2)optimize_label_layout(same_x,lock_z,dx.last_f_base/1e3);dx.f_same=[];dx.f_same.push(dx_idx);lock_z=0;}}
var top=dx_label_top+(gap*(dx_idx&1));dx.post_render[dx_idx]={top:top,ltop:top,x:x};dx.last_f_base=f_base_label_Hz;var color;if(eibi){color=filtered?dx.eibi_colors_light[color_idx]:dx.eibi_colors[color_idx];}else{var c=cfg.dx_type[color_idx].color;if(c=='')c='white';color=filtered?dx.stored_colors_light[color_idx]:c;}
...
+' m='+kiwi.modes_u[flags&dx.DX_MODE]+' <'+ident+'> <'+notes+'>');carrier=freq*1000-moff;carrier /=1000;dx.list[gid]={"gid":gid,"carrier":carrier,"lo":obj.lo,"hi":obj.hi,"freq":freq,"moff":moff,"flags":flags,"begin":obj.b,"end":obj.e,"ident":ident,"notes":notes,"params":params};console_log_lbl('dx_label_cb db='+dx.db+' dx_idx='+dx_idx+'/'+arr.length+' gid='+gid+' dx.list.len='+dx.list.length+' '+freq.toFixed(2)+' x='+x);dx.displayed[dx_idx]=dx.list[gid];console_log_lbl(dx.list[gid]);var has_ext=(params!='')?' id-has-ext':'';s_a[dx_idx]='<div id="'+dx_idx+'-id-dx-label" class="cl-dx-label '+(filtered?'cl-dx-label-filtered':'')+' '+gid+'-id-dx-gid'+has_ext+'" '+'style="left:'+(x-10)+'px; z-index:'+dx_z+'; '+'background-color:'+color+';" '+'onmouseenter="dx_enter(this,'+cmkr_x+', event)" onmouseleave="dx_leave(this,'+cmkr_x+')" '+'onmousedown="ignore(event)" onmousemove="ignore(event)" onmouseup="ignore(event)" ontouchmove="ignore(event)" ontouchend="ignore(event)" '+'onclick="dx_click(event,'+gid+')" ontouchstart="dx_click(event,'+gid+')" name="'+((params=='')?0:1)+'">'+'</div>'+'<div class="cl-dx-line" id="'+dx_idx+'-id-dx-line" style="left:'+x+'px; z-index:110"></div>';dx_z++;}
if(eibi&&dx.f_same.length>2)optimize_label_layout(same_x,lock_z,-1);console_log_lbl(dx.list);console_log_lbl(dx.displayed);var s='';for(i=0;i<arr.length-1;i++)
if(isDefined(s_a[i]))
s+=s_a[i];dx_div.innerHTML=s;dx.last_freq=0;dx.last_ident='';if(!dx.font){var el=w3_el('0-id-dx-label');if(el){dx.font=css_style(el,'font-size')+' '+css_style(el,'font-family');console_log_lbl('dx.font=<'+dx.font+'>');}}
...
return s;};for(i=1;i<arr.length;i++){dx_idx=i-1;obj=arr[i];var el;var ident=obj.i;var freq=obj.f;if(eibi){if(ident==dx.last_ident&&freq==dx.last_freq){console_log_lbl('2222 dx_idx='+dx_idx+' last_dx_idx='+dx.last_dx_idx+' '+obj.f+' SAME AS LAST '+ident);el=w3_el(dx.last_dx_idx+'-id-dx-label');el.title+='\n'+dx_title(obj);continue;}}
dx.last_dx_idx=dx_idx;dx.last_freq=freq;dx.last_ident=ident;if(eibi)console_log_lbl('2222 dx_idx='+dx_idx+' '+obj.f+' OK '+ident);var notes=(isDefined(obj.n))?obj.n:'';el=w3_el(dx_idx+'-id-dx-label');if(!el)continue;var _ident=kiwi_decodeURIComponent('dx_ident2',ident).trim();if(_ident=='')_ident='&nbsp;&nbsp;';el.innerHTML=_ident.replace(/\\n/g,'<br>');var idx=dx_type2idx(obj.fl&dx.DX_TYPE);var ex=(eibi&&dx.eibi_ext[idx]!='')?'\nshift-click to open extension':'';if(eibi)console_log_lbl(obj.i+' '+idx+' '+dx.eibi_ext[idx]+' '+ex);if(eibi){el.title=dx.eibi_svc_s[idx]+' // home country: '+obj.c+ex+'\n'+dx_title(obj);}else{var title=kiwi_decodeURIComponent('dx_notes',notes);title=title.replace(/<br>/g,'\n');title=title.replace(/\\n/g,'\n');var dow=obj.fl&dx.DX_DOW;if(dow!=dx.DX_DOW||(obj.b!=0||!(obj.b==0&&obj.e==2400)))
...
function dx_label_step(dir){var i,dl;var eibi=(dx.db==dx.DB_EiBi);if(dir==1){for(i=0;i<dx.displayed.length;i++){dl=dx.displayed[i];if(!dl)continue;var f=Math.round(dl.freq*1e3);if(f>freq_displayed_Hz)break;}
...
+' filter_tod='+(eibi?dx.eibi_filter_tod:dx.stored_filter_tod));return;}}else{for(i=dx.displayed.length-1;i>=0;i--){dl=dx.displayed[i];if(!dl)continue;var f=Math.round(dl.freq*1e3);if(f<freq_displayed_Hz)break;}
if(i<0){wf_send('SET MARKER db='+dx.db+' dir=-1 freq='+(freq_displayed_Hz/1e3).toFixed(3)
+' eibi_types_mask='+dx.eibi_types_mask.toHex()
+' filter_tod='+(eibi?dx.eibi_filter_tod:dx.stored_filter_tod));return;}}
...
if(dx.db==dx.DB_STORED&&gid!=-1&&dx_keys.shift&&dx_keys.alt){var type=dx.o.ft;type=(type==dx.DX_QUICK_0)?dx.DX_QUICK_1:dx.DX_QUICK_0;dx.o.ft=type;var flags=(dx.o.fd<<dx.DX_DOW_SFT)|(type<<dx.DX_TYPE_SFT)|dx.o.fm;dx_send_update('DX_QUICK',dx.o.gid,dx.o.fr,flags,dx.o);return;}
extint_panel_hide();dx.o.fr=(parseFloat(dx.o.fr)).toFixed(2);dx.o.pb='';if(dx.o.lo||dx.o.hi){if(dx.o.lo==-dx.o.hi){dx.o.pb=(Math.abs(dx.o.hi)*2).toFixed(0);}else{dx.o.pb=dx.o.lo.toFixed(0)+', '+dx.o.hi.toFixed(0);}}
...
function dx_send_update(from,gid,fr,flags,dx_o){dx.retry=isNull(dx_o)?{}:kiwi_deep_copy(dx_o);dx.retry.gid=gid;dx.retry.fr=fr;dx.retry.flags=flags;if(gid!=-1&&fr==-1){wf_send('SET DX_UPD g='+gid+' f=-1');}else{wf_send('SET DX_UPD g='+gid+' f='+fr+' lo='+dx_o.lo.toFixed(0)+' hi='+dx_o.hi.toFixed(0)+' o='+dx_o.o.toFixed(0)+' fl='+flags+' b='+dx_o.begin+' e='+dx_o.end+' i='+encodeURIComponent(dx_o.i+'x')+' n='+encodeURIComponent(dx_o.n+'x')+' p='+encodeURIComponent(dx_o.p+'x'));}}
function dx_send_update_retry(){dx_send_update('dx_send_update_retry',dx.retry.gid,dx.retry.fr,dx.retry.flags,dx.retry);}
function dx_modify_cb(id,val){console_log_dbgUs('DX COMMIT modify entry #'+dx.o.gid+' f='+dx.o.fr);console_log_dbgUs(dx.o);if(dx.o.gid==-1)return;var flags=(dx.o.fd<<dx.DX_DOW_SFT)|(dx.o.ft<<dx.DX_TYPE_SFT)|dx.o.fm;if(dx.o.fr<0)dx.o.fr=0;dx_send_update('DX_MODIFY',dx.o.gid,dx.o.fr,flags,dx.o);setTimeout(function(){dx_close_edit_panel(id);},250);}
function dx_add_cb(id,val){var flags=(dx.o.fd<<dx.DX_DOW_SFT)|(dx.o.ft<<dx.DX_TYPE_SFT)|dx.o.fm;if(dx.o.fr<0)dx.o.fr=0;dx_send_update('DX_ADD',-1,dx.o.fr,flags,dx.o);setTimeout(function(){dx_close_edit_panel(id);},250);owrx.dx_click_gid_last=undefined;}
function dx_delete_cb(id,val){if(dx.o.gid==-1)return;dx_send_update('DX_DELETE',dx.o.gid,-1,0,null);setTimeout(function(){dx_close_edit_panel(id);},250);owrx.dx_click_gid_last=undefined;}
function dx_click(ev,gid){if(ev.shiftKey&&dx.db==dx.DB_STORED){dx_show_edit_panel(ev,gid);}else{w3_menu_close('dx_click');owrx.dx_click_gid_last=(dx.db==dx.DB_STORED)?gid:undefined;var label=dx.list[gid];var freq=label.freq;var f_base=freq-kiwi.freq_offset_kHz;var mode=kiwi.modes_l[label.flags&dx.DX_MODE];var lo=label.lo;var hi=label.hi;var params=label.params;extint.extname=extint.param=null;if(isArg(params)){var ext=(params=='')?[]:decodeURIComponent(params).split(',');if(mode=='drm'){extint.extname='drm';ext.push('lo:'+lo);ext.push('hi:'+hi);extint.param=ext.join(',');}else{extint.extname=ext[0];extint.param=ext.slice(1).join(',');}}
var extname=extint.extname?extint.extname:'';console_log_dbgUs('dx_click f='+freq+'|'+f_base+' mode='+mode+' cur_mode='+cur_mode+' lo='+lo+' hi='+hi+' params=<'+params+'> extname=<'+extname+'> param=<'+extint.param+'>');if(dx.db==dx.DB_EiBi){}
freqmode_set_dsp_kHz(f_base,mode,{open_ext:true});if(lo||hi){ext_set_passband(lo,hi,false,f_base);}
