...
/* web/extensions/ext.min.js (web/extensions/ext.js = Wed Jul 24 07:11:11 2024) */
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Thu Jul 25 08:12:05 2024) */
...
break;default:if(param[0].startsWith('antsw_')){if(ant_switch_msg(param))
break;}
rtn=false;break;}
return rtn;}
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Tue Jul 23 08:01:39 2024) */
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Thu Jul 25 08:03:42 2024) */
...
var selected_antennas_list=ant_selected_antenna.split(',');var re=/^Antenna ([1]?[0-9]+)/i;w3_els('id-antsw-btn',function(el,i){if(!el.textContent.match(re))return;w3_unhighlight(el);var antN=el.textContent.parseIntEnd();ant_sw.buttons_selected[antN]=false;if(!isArray(selected_antennas_list))return;if(selected_antennas_list.indexOf(antN.toString())<0)return;ant_sw.buttons_selected[antN]=true;w3_highlight(el);if(ant_sw.denymixing&&selected_antennas_list.length==1){var s='ant_switch.ant'+antN+'offset';var offset=ext_get_cfg_param(s,'',EXT_NO_SAVE);offset=+offset;if(!isNumber(offset))offset=-1;if(offset!=-1){console.log('ant='+antN+' setting offset='+offset+' last='+ant_sw.last_offset);if(offset!=ant_sw.last_offset){ext_send('SET antsw_freq_offset='+offset);}
s='ant_switch.ant'+antN+'high_side';var high_side=ext_get_cfg_param(s,'',EXT_NO_SAVE);if(high_side!=ant_sw.last_high_side){ext_send('SET antsw_high_side='+(high_side?1:0));ant_sw.last_high_side=high_side;}}else{console.log('ant='+antN+' not setting offset');}
ant_sw.last_offset=offset;}});if(ant_sw.url_ant&&ant_sw.url_ant.length>0){if(ant_sw.url_idx==-1){ant_sw.url_idx=0;if(!ant_sw.denymixing){ant_switch_select_antenna('g');return;}}
