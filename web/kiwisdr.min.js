...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Mon Oct 10 08:44:45 2022) */
...
if(p.init!=true){removeAllLines(parent_el);p.el=appendEmptyLine(parent_el);p.NONE=0;p.ESC=1;p.CSI=2;p.NON_CSI=3;p.NOSHIFT_MODE=true;p.esc={s:'',state:p.NONE};p.sgr={span:0,bright:0,fg:null,bg:null};p.return_pending=false;p.must_scroll_down=false;p.traceEvery=false;p.ccol=1;p.line=[];p.line_sgr=[];p.line_html=[];p.inc=1;p.r=p.c=1;p.margin_set=false;p.margin_top=1;p.margin_bottom=p.rows;p.show_cursor=p.show_cursor||false;p.r_cursor=p.c_cursor=0;p.insertMode=true;p.eol_wrap=false;p.screen=[];p.color=[];p.dirty=[];p.els=[];for(var r=0;r<=p.rows;r++){p.screen[r]=[];p.color[r]=[];}
...
prt=true;}}
if(c=='\n'||(p.eol_wrap&&p.ccol==p.cols)){wasScrolledDown=w3_isScrolledDown(el_scroll);if(p.isAltBuf){p.c=1;dirty();p.r++;dirty();if(p.r>p.rows)p.r=1;}else{var stmp=line_join(1);p.el.innerHTML=(stmp=='')?'&nbsp;':stmp;p.line=[];p.line_sgr=[];p.line_html=[];p.el=appendEmptyLine(parent_el);p.ccol=1;}
