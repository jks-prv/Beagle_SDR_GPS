...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Tue Apr 23 11:20:40 2024) */
...
/* web/kiwi/waterfall.min.js (web/kiwi/waterfall.js = Tue Apr 23 11:15:20 2024) */
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Tue Apr 23 11:17:05 2024) */
var ant_sw={ver:'',n_ant:8,backend:-1,backend_s:null,backends_s:null,can_mix:true,ip_or_url:null,exantennas:0,focus:false,isConfigured:false,running:false,buttons_locked:false,buttons_selected:[],denymixing:0,thunderstorm:0,notify_timeout:null,url_ant:null,url_deselected:false,url_idx:0,desc_lc:[],last_offset:-1,last_high_side:-1,EVERYONE:0,LOCAL_CONN:1,LOCAL_OR_PWD:2,denyswitching:0,deny_s:['everyone','local connections only','local connections or user password only'],DENY_NONE:0,DENY_SWITCHING:1,DENY_MULTIUSER:2,denied_permission:false,denied_because_multiuser:false};function ant_switch_disabled(){return(!ant_sw.isConfigured||!cfg.ant_switch.enable);}
function ant_switch_view(){if(!ant_sw.focus)return;keyboard_shortcut_nav('rf');var Hrf;w3_do_when_cond(function(){Hrf=w3_el('id-optbar-rf').clientHeight;return(Hrf!=0);},function(){var Hopt=kiwi.OPTBAR_CONTENT_HEIGHT;var Hant=w3_el('id-optbar-rf-antsw').clientHeight+8;pct=w3_clamp(kiwi_round(1-(Hant-Hopt)/(Hrf-Hopt),2),0,1);w3_scrollTo('id-optbar-content',pct);},null,200);var p=ext_param();if(isNonEmptyString(p)){ant_sw.url_ant=p.split(',');ant_switch_prompt_query();}}
function ant_switch_msg(param){var rv=true;switch(param[0]){case"antsw_backend_ver":var ver=param[1].split('.');if(ver.length==2&&(ver[0]||ver[1])){ant_sw.ver=param[1];console.log('ant_switch: backend v'+ant_sw.ver);}
...
function ant_switch_select_antenna(ant){ext_send('SET antsw_SetAntenna='+ant);}
function ant_switch_select_antenna_cb(path,val){ant_switch_select_antenna(val);}
function ant_switch_prompt_query(){ext_send('SET antsw_GetAntenna');}
function ant_switch_process_reply(ant_selected_antenna){var need_to_inform=false;ant_switch_buttons_setup();ant_sw.denyswitching=ext_get_cfg_param('ant_switch.denyswitching',ant_sw.EVERYONE,EXT_NO_SAVE);if(ant_switch_disabled()){ant_switch_display_update('Antenna switch is disabled.');return;}
if(ant_sw.exantennas!=ant_selected_antenna){need_to_inform=true;ant_sw.exantennas=ant_selected_antenna;}
if(ant_selected_antenna=='g'){if(need_to_inform)
ant_switch_display_update('All antennas are grounded.');}else{if(need_to_inform)
...
s='ant_switch.ant'+antN+'high_side';var high_side=ext_get_cfg_param(s,'',EXT_NO_SAVE);if(high_side!=ant_sw.last_high_side){ext_send('SET antsw_high_side='+(high_side?1:0));ant_sw.last_high_side=high_side;}}});if(ant_sw.url_ant&&ant_sw.url_ant.length>0){if(ant_sw.url_deselected==false){ant_switch_select_antenna('g');ant_sw.url_deselected=true;}else{if(ant_sw.url_idx==0||!ant_sw.denymixing){var ant=decodeURIComponent(ant_sw.url_ant.shift());if(ant=='help'){ant_switch_help();}else{var n=parseInt(ant);if(!(!isNaN(n)&&n>=1&&n<=ant_sw.n_ant)){if(ant==''){n=0;}else{ant=ant.toLowerCase();for(n=1;n<=ant_sw.n_ant;n++){if(ant_sw.desc_lc[n].indexOf(ant)!=-1)
break;}}}
...
function ant_switch_admin_msg(param){switch(param[0]){case"antsw_backends":ant_sw.backends_s=decodeURIComponent(param[1]).split(' ');ant_sw.backends_s.unshift('disable');console.log('antsw_backends='+ant_sw.backends_s);w3_innerHTML('id-antsw-backends',w3_select('w3-width-auto w3-label-inline','Switch device:','','ant_sw.backend',ant_sw.backend,ant_sw.backends_s,'ant_switch_backend_cb'));w3_innerHTML('id-antsw-ip-url',w3_input('/w3-label-inline w3-label-not-bold w3-nowrap/w3-padding-small||size=30','IP address or URL:','ant_sw.ip_or_url',ant_sw.ip_or_url,'ant_switch_set_ip_or_url_cb'));return true;case"antsw_backend":console.log('antsw_backend='+param[1]+' enable='+cfg.ant_switch.enable);ant_sw.backend_s=param[1];if(isNonEmptyString(ant_sw.backend_s)&&cfg.ant_switch.enable){if(w3_select_set_if_includes('ant_sw.backend',ant_sw.backend_s)){ext_set_cfg_param('ant_switch.backend',ant_sw.backend_s,EXT_SAVE);console.log('cfg.ant_switch.backend='+ant_sw.backend_s+' ant_sw.backend='+ant_sw.backend);w3_hide2('id-antsw-backend-info',false);w3_hide2('id-antsw-container',false);}}else
if(!cfg.ant_switch.enable){w3_select_set_if_includes('ant_sw.backend','disable');}
ant_switch_users_notify_change();return true;case"antsw_ver":console.log('antsw_ver='+param[1]);ant_sw.ver=param[1];var s='Backend script v'+ant_sw.ver;if(ant_sw.ver=='0.0')s='';w3_innerHTML('id-antsw-ver',s);return true;case"antsw_nch":console.log('antsw_nch='+param[1]);ant_switch_config_html2(+param[1]);return true;case"antsw_mix":console.log('antsw_mix='+param[1]);ant_sw.can_mix=(param[1]=='mix')?true:false;w3_innerHTML('id-antsw-mix','Supports mixing? '+(ant_sw.can_mix?'yes':'no'));w3_switch_set_value('id-ant_switch.denymixing',w3_SWITCH_YES_IDX);w3_disable('id-antsw-mix-switch',!ant_sw.can_mix);return true;case"antsw_ip_or_url":console.log('antsw_ip_or_url='+param[1]);var ip_url=param[1];if(ip_url=='(null)')ip_url='';w3_set_value('id-ant_sw.ip_or_url',ip_url);ant_sw.ip_or_url=ip_url;return true;}
return false;}
function ant_switch_backend_cb(path,val,first){val=+val;if(first)return;ant_sw.backend_s=ant_sw.backends_s[+val];ext_set_cfg_param('ant_switch.backend',ant_sw.backend_s,EXT_NO_SAVE);var enable=(val!=0);ext_set_cfg_param('ant_switch.enable',enable,EXT_SAVE);if(enable){ext_send('ADM antsw_SetBackend='+ant_sw.backend_s);ext_send('ADM antsw_GetInfo');}else{ant_switch_users_notify_change();}
w3_hide2('id-antsw-backend-info',!enable);w3_hide2('id-antsw-container',!enable);}
function ant_switch_users_notify_change(){kiwi_clearTimeout(ant_sw.notify_timeout);ant_sw.notify_timeout=setTimeout(function(){ext_send('ADM antsw_notify_users');},1000);}
function ant_switch_desc_cb(path,val,first){w3_string_set_cfg_cb(path,val,first);ant_switch_users_notify_change();}
function ant_switch_set_ip_or_url_cb(path,val,first){if(first)return;ant_sw.ip_or_url=val;w3_set_value('id-ant_sw.ip_or_url',ant_sw.ip_or_url);ext_send('ADM antsw_SetIP_or_URL='+ant_sw.ip_or_url);}
function ant_denyswitch_cb(path,val,first){w3_int_set_cfg_cb(path,val,first);ant_switch_users_notify_change();}
function ant_switch_cb(path,val,first){admin_radio_YN_cb(path,val);ant_switch_users_notify_change();}
function ant_switch_config_html2(n_ch){if(n_ch)ant_sw.n_ant=n_ch;var s='';for(var i=1;i<=ant_sw.n_ant;i++){s+=w3_inline_percent('w3-margin-T-16 w3-valign-center/',w3_input_get('w3-defer','Antenna '+i+' description','ant_switch.ant'+i+'desc','ant_switch_desc_cb',''),50,'&nbsp;',3,w3_checkbox_get_param('w3-defer//w3-label-inline','Default<br>antenna','ant_switch.ant'+i+'default','admin_bool_cb',false),7,'&nbsp;',3,w3_checkbox_get_param('w3-defer w3-restart//w3-label-inline','High-side<br>injection','ant_switch.ant'+i+'high_side','admin_bool_cb',false),10,'&nbsp;',1,w3_input_get('w3-defer w3-restart//','Frequency scale offset (kHz)','ant_switch.ant'+i+'offset','w3_int_set_cfg_cb',0));}
