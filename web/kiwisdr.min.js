...
/* web/extensions/ext.min.js (web/extensions/ext.js = Sun Apr 14 10:46:35 2024) */
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Sun Apr 14 14:21:29 2024) */
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Sun Apr 14 11:20:20 2024) */
...
function kiwi_storeRead(k,def){var rv;if(isNoArg(k))return null;if(kiwi.noLocalStorage){var rv=readCookie(k,def);return rv;}
...
return rv;}
function kiwi_storeInit(k,init){var rv;if(isNoArg(k))return null;if(kiwi.noLocalStorage){var rv=initCookie(k,init);return rv;}
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Sun Apr 14 14:14:51 2024) */
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Sun Apr 14 15:57:40 2024) */
...
var zin=w3_contains(div,'id-zoom-in');var key_mod=shortcut_key_mod(evt);var title=owrx.zoom_titles[key_mod][zin];if(disabled&&key_mod==shortcut.NO_MODIFIER)title='';console.log('zoom_over key_mod='+key_mod+' zin='+zin+' title='+title);el.title=title;}
...
button_9_10(step_9_10);s='';mode_buttons.forEach(function(mba,i){var ms=mba.s[0];var mslc=ms.toLowerCase();var disabled=mba.dis||(mslc=='drm'&&!kiwi.DRM_enable);var attr=disabled?'':' onclick="mode_button(event, this)"';attr+=' onmouseover="mode_over(event, this)"';s+=w3_div('id-button-'+mslc+' id-mode-'+ext_mode(mslc).str+' class-button'+(disabled?' class-button-disabled':'')+' ||id="'+i+'-id-mode-col"'+attr+' onmousedown="cancelEvent(event)"',ms);});w3_el("id-control-mode").innerHTML=w3_inline('w3-halign-space-between/',s);var afd1=wf.audioFFT_active?' w3-disabled||onclick="freqset_select()" disabled=""':'';var afd2=wf.audioFFT_active?' w3-disabled||disabled="" ':'||';w3_el("id-control-zoom").innerHTML=w3_inline('w3-halign-space-between/',w3_div('id-zoom-in class-icon'+afd2+'onclick="zoom_click(event, 1)" onmouseover="zoom_over(event)"','<img src="icons/zoomin.png" width="32" height="32" />'),w3_div('id-zoom-out class-icon'+afd2+'onclick="zoom_click(event,-1)" onmouseover="zoom_over(event)"','<img src="icons/zoomout.png" width="32" height="32" />'),w3_div('id-maxin'+afd1,w3_div('class-icon||onclick="zoom_click(event,8)" title="max in"','<img src="icons/maxin.png" width="32" height="32" />')),w3_div('id-maxin-nom w3-hide'+afd1,w3_div('class-icon||onclick="zoom_click(event,8)" title="max in"','<img src="icons/maxin.nom.png" width="32" height="32" />')),w3_div('id-maxin-max w3-hide'+afd1,w3_div('class-icon||onclick="zoom_click(event,8)" title="max in"','<img src="icons/maxin.max.png" width="32" height="32" />')),w3_div('id-maxout'+afd1,w3_div('class-icon||onclick="zoom_click(event,-9)" title="max out"','<img src="icons/maxout.png" width="32" height="32" />')),w3_div('id-maxout-max w3-hide'+afd1,w3_div('class-icon||onclick="zoom_click(event,-9)" title="max out"','<img src="icons/maxout.max.png" width="32" height="32" />')),w3_div('class-icon'+afd1+'||onclick="zoom_click(event,0)" title="zoom to band"','<img src="icons/zoomband.png" width="32" height="16" style="padding-top:13px; padding-bottom:13px;"/>'),w3_div('class-icon'+afd2+'onclick="page_scroll_icon_click(event,-page_scroll_amount)" title="page down\nalt: label step"','<img src="icons/pageleft.png" width="32" height="32" />'),w3_div('class-icon'+afd2+'onclick="page_scroll_icon_click(event,+page_scroll_amount)" title="page up\nalt: label step"','<img src="icons/pageright.png" width="32" height="32" />'));var optbar_colors=['w3-green','w3-pink','w3-blue','w3-purple','w3-aqua','w3-yellow','w3-black','w3-red','w3-amber','w3-green','w3-orange','w3-lime','w3-indigo','w3-brown','w3-teal'];var ci=0;var psa1=' w3-center|width:12.8%';var psa2=psa1+';margin-right:6px';w3_el('id-optbar').innerHTML=w3_navbar('id-navbar-optbar cl-optbar',w3_nav(optbar_colors[ci++]+psa2,'RF','id-navbar-optbar','optbar-rf','optbar'),w3_nav(optbar_colors[ci++]+psa2,'WF','id-navbar-optbar','optbar-wf','optbar'),w3_nav(optbar_colors[ci++]+psa2,'Audio','id-navbar-optbar','optbar-audio','optbar'),w3_nav(optbar_colors[ci++]+psa2,'AGC','id-navbar-optbar','optbar-agc','optbar'),w3_nav(optbar_colors[ci++]+psa2,'User','id-navbar-optbar','optbar-users','optbar'),w3_nav(optbar_colors[ci++]+psa2,'Stat','id-navbar-optbar','optbar-status','optbar'),w3_navdef(optbar_colors[ci++]+psa1,'Off','id-navbar-optbar','optbar-off','optbar'));wf_filter=kiwi_storeRead('last_wf_filter',wf_sp_menu_e.OFF);spec_filter=kiwi_storeRead('last_spec_filter',wf_sp_menu_e.IIR);if(isString(spectrum_show)){var ss=spectrum_show.toLowerCase();wf_sp_menu_s.forEach(function(e,i){if(ss==e.toLowerCase())spec_filter=i;});}
