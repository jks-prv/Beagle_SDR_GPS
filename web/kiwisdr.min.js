...
/* web/extensions/ext.min.js (web/extensions/ext.js = Fri May 19 21:09:45 2023) */
...
extint.prev_mode=cur_mode;console.log('### ext_set_mode '+mode+' prev='+extint.prev_mode);demodulator_analog_replace(mode,freq);var open_ext=w3_opt(opt,'open_ext',false);var no_drm_proc=w3_opt(opt,'no_drm_proc',false);var drm_active=(typeof(drm)!='undefined'&&drm.active);if(!no_drm_proc){if(new_drm&&open_ext){if(drm_active){toggle_panel("ext-controls",1);}else{extint_open('drm');}}else
...
var cfg={};var dxcfg={};var dxcomm_cfg={};var adm={};function config_save(cfg_s,cfg){var kiwi_cfg=kiwi[cfg_s];{config_save2(kiwi_cfg,cfg);}}
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Mon May 22 09:54:04 2023) */
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Fri Jun  2 14:38:45 2023) */
...
/* web/kiwi/monitor.min.js (web/kiwi/monitor.js = Fri May 19 21:09:45 2023) */
...
set_gen(gen_freq,gen_attn);},null,500);var _lo=kiwi_passbands(init_mode).lo;if(!isNumber(_lo))_lo=-4000;var _hi=kiwi_passbands(init_mode).hi;if(!isNumber(_hi))_hi=4000;snd_send('SET mod='+init_mode+' low_cut='+_lo+' high_cut='+_hi+' freq='+init_frequency);set_agc();snd_send("SET browser="+navigator.userAgent);if(snd_winf!=0)snd_send('SET window_func='+snd_winf);wf_send("SERVER DE CLIENT openwebrx.js W/F");wf_send("SET send_dB=1");wf_send("SET zoom=0 start=0");wf_send("SET maxdb=0 mindb=-100");if(wf_compression==0)wf_send('SET wf_comp=0');if(wf_interp!=-1)wf_send('SET interp='+wf_interp);if(wf_winf!=0)wf_send('SET window_func='+wf_winf);wf_speed=wf_rates[wf_rate];if(wf_speed==undefined)wf_speed=WF_SPEED_FAST;wf_send('SET wf_speed='+wf_speed);}
...
function init_top_bar(){w3_innerHTML('id-rx-desc',cfg.index_html_params.RX_LOC+' | Grid '+
...
var rx_photo_spacer_height=owrx.height_top_bar_parts;function init_rx_photo(){var el=w3_el('id-top-photo-img');el.src=cfg.index_html_params.RX_PHOTO_FILE;el.alt=el.title=cfg.index_html_params.RX_PHOTO_DESC;el.style.paddingLeft=RX_PHOTO_LEFT_MARGIN?'50px':0;RX_PHOTO_HEIGHT+=rx_photo_spacer_height;w3_el("id-top-photo-clip").style.maxHeight=px(RX_PHOTO_HEIGHT);if(dbgUs||kiwi_isMobile()){close_rx_photo();}else{window.setTimeout(function(){close_rx_photo()},3000);}}
...
demodulator_response_time=100;var passbands_fallback={am:{lo:-4900,hi:4900},amn:{lo:-2500,hi:2500},sam:{lo:-4900,hi:4900},sal:{lo:-4900,hi:0},sau:{lo:0,hi:4900},sas:{lo:-4900,hi:4900},qam:{lo:-4900,hi:4900},drm:{lo:-5000,hi:5000},lsb:{lo:-2700,hi:-300},lsn:{lo:-2400,hi:-300},usb:{lo:300,hi:2700},usn:{lo:300,hi:2400},cw:{lo:300,hi:700,pbw:400},cwn:{lo:470,hi:530,pbw:60},nbfm:{lo:-6000,hi:6000},nnfm:{lo:-3000,hi:3000},iq:{lo:-5000,hi:5000},};function kiwi_passbands(mode){var pb=cfg.passbands[mode];if(isDefined(pb)){return pb;}else{console.log('kiwi_passbands('+mode+') fallback:');console.log(passbands_fallback[mode]);return passbands_fallback[mode];}}
...
console.log("ZTB-user f="+f+" cf="+cf+" b="+b1.name+" z="+b2.zoom_level);}else{var ITU_region=cfg.init.ITU_region+1;var _dxcfg=dx_cfg_db();var _kiwi_bands=kiwi_bands_db(dx.INIT_BANDS_NO);for(i=0;i<_dxcfg.bands.length;i++){b1=_dxcfg.bands[i];b2=_kiwi_bands[i];if(!(b1.itu==kiwi.BAND_SCALE_ONLY||b1.itu==kiwi.ITU_ANY||b1.itu==ITU_region))continue;if(f>=b2.minHz&&f<=b2.maxHz){break;}}
...
function freqmode_set_dsp_kHz(fdsp,mode,opt){var dont_clear_wf=w3_opt(opt,'dont_clear_wf',false);var open_ext=w3_opt(opt,'open_ext',false);var no_set_freq=w3_opt(opt,'no_set_freq',0);var no_clear_last_gid=w3_opt(opt,'no_clear_last_gid',0);fdsp*=1000;if(dont_clear_wf==false)wf.audioFFT_clear_wf=true;if(!no_clear_last_gid)owrx.dx_click_gid_last_until_tune=undefined;if(isArg(mode)&&(mode!=cur_mode||open_ext==true)){ext_set_mode(mode,fdsp,opt);}else{freq_car_Hz=freq_dsp_to_car(fdsp);if(!no_set_freq){demodulator_set_offset_frequency(owrx.FSET_SET_FREQ,freq_car_Hz-center_freq);}}}
...
function band_svc_lookup(svc_key){var idx=null;var _dxcfg=dx_cfg_db();_dxcfg.band_svc.forEach(function(a,i){if(a.key==svc_key)
idx=i;});if(idx==null){console.log('$$$ band_svc_lookup NO KEY MATCH for <'+svc_key+'>');return null;}
return{o:_dxcfg.band_svc[idx],i:idx};}
function bands_init(){var i,j,k,z,b,bnew;var _dxcfg=dx_cfg_db();if(_dxcfg.bands&&_dxcfg.band_svc&&_dxcfg.dx_type){console.log('BANDS: using stored _dxcfg.bands db='+dx.db+' ---------------------------------');bands_addl_info(0);dx_color_init();return;}
console.log('BANDS: using old config.js/bands db='+dx.db+'-------------------------------------');dxcfg.dx_type=[];var max_i=-1;dx.legacy_stored_types.forEach(function(a,i){dxcfg.dx_type[i]={key:i,name:dx.legacy_stored_types[i],color:dx.legacy_stored_colors[i]};max_i=i;});j=dx_type2idx(dx.DX_MASKED);for(i=max_i+1;i<j;i++){dxcfg.dx_type[i]={key:i,name:'',color:''};}
...
function dx_cfg_db(){return(dx.db==dx.DB_COMMUNITY)?dxcomm_cfg:dxcfg;}
...
var _dxcfg=dx_cfg_db();var _kiwi_bands=kiwi_bands_db(dx.INIT_BANDS_YES);for(i=0;i<_dxcfg.bands.length;i++){b1=_dxcfg.bands[i];b2=_kiwi_bands[i]={};_dxcfg.bands[i].chan=isUndefined(b1.chan)?0:b1.chan;var min=b1.min,max=b1.max;if(offset_kHz&&b1.min>=offset_kHz){min-=offset_kHz;max-=offset_kHz;b2.isOffset=true;}else
if(b1.min>32000)
...
var band_menu=[];function setup_band_menu(){var i,op=0,service=null;var s='<option value="0" selected disabled>select band</option>';band_menu[op++]=null;var ITU_region=cfg.init.ITU_region+1;var _dxcfg=dx_cfg_db();var _kiwi_bands=kiwi_bands_db(dx.INIT_BANDS_NO);for(i=0;i<_dxcfg.bands.length;i++){var b1=_dxcfg.bands[i];var b2=_kiwi_bands[i];if(kiwi.isOffset){if(!b2.isOffset)continue;}else{if(b2.isOffset)continue;}
...
function find_band(freq){var ITU_region=cfg.init.ITU_region+1;var _dxcfg=dx_cfg_db();var _kiwi_bands=kiwi_bands_db(dx.INIT_BANDS_NO);for(var i=0;i<_dxcfg.bands.length;i++){var b1=_dxcfg.bands[i];var b2=_kiwi_bands[i];if(!(b1.itu==kiwi.BAND_SCALE_ONLY||b1.itu==kiwi.ITU_ANY||b1.itu==ITU_region))continue;var max=(b1.name=='MW')?1710000:b2.maxHz;if(freq>=b2.minHz&&freq<=max){return b1;}}
return null;}
band_canvas_h=30;band_canvas_top=0;band_scale_h=20;band_scale_top=5;band_scale_text_top=5;dx_container_h=80;dx_container_top=band_canvas_h;dx_label_top=5;dx_line_h=dx_container_h-dx_label_top;scale_canvas_h=47;scale_canvas_top=band_canvas_h+dx_container_h;scale_container_h=band_canvas_h+dx_container_h+scale_canvas_h;function mk_bands_scale(){var r=g_range;var tw=band_ctx.canvas.width;var i,x,y=band_scale_top,w,h=band_scale_h,ty=y+band_scale_text_top;var start=r.start;var end=r.end;band_ctx.globalAlpha=1;band_ctx.fillStyle="White";band_ctx.fillRect(0,band_canvas_top,tw,band_canvas_h);var ITU_region=cfg.init.ITU_region+1;var _dxcfg=dx_cfg_db();var _kiwi_bands=kiwi_bands_db(dx.INIT_BANDS_NO);for(i=0;i<_dxcfg.bands.length;i++){var b1=_dxcfg.bands[i];var b2=_kiwi_bands[i];if(kiwi.isOffset){if(!b2.isOffset)continue;}else{if(b2.isOffset)continue;}
...
if((r=w3_ext_param('filter',a)).match){dx.filter_tod[dx.db]=r.has_value?r.num:1;console.log('SET filter_tod='+dx.filter_tod[dx.db]+' r.has_value='+r.has_value+' r.num='+r.num);}
...
console.log('FINAL dx.eibi_types_mask='+dx.eibi_types_mask.toHex(8)+' dx.help='+dx.help);dx_show_edit_panel(null,-1);dx_schedule_update();},null,500);}else{dx.db=kiwi_storeGet('last_db',cfg.dx_default_db);}
...
function dx_color_init(){if(dx.db!=dx.DB_EiBi){var _dxcfg=dx_cfg_db();for(var i=0;i<_dxcfg.dx_type.length;i++){var o=_dxcfg.dx_type[i];var color=(o.color=='')?'white':o.color;var colorObj=w3color(color);if(colorObj.valid){colorObj.lightness=dx.stored_lightness;dx.stored_colors_light[i]=colorObj.toHslString();}}}else{dx.eibi_hue.forEach(function(color_or_hue,i){if(isString(color_or_hue)){dx.eibi_colors[i]=color_or_hue;}else{dx.eibi_colors[i]='hsl('+color_or_hue+',100%,'+dx.eibi_light[i]+'%)';}
...
+' m='+kiwi.modes_uc[dx_decode_mode(flags)]+' <'+ident+'> <'+notes+'>');carrier=freq*1000-mkr_off;carrier /=1000;dx.list[gid]={"gid":gid,"carrier":carrier,"lo":obj.lo,"hi":obj.hi,"freq":freq,"mkr_off":mkr_off,"sig_bw":obj.s,"flags":flags,"begin":obj.b,"end":obj.e,"ident":ident,"notes":notes,"params":params};console_log_lbl('dx_label_cb db='+dx.db+' dx_idx='+dx_idx+'/'+arr.length+' gid='+gid+' dx.list.len='+dx.list.length+' '+freq.toFixed(2)+' x='+x);dx.displayed[dx_idx]=dx.list[gid];console_log_lbl(dx.list[gid]);var has_ext=(params!='');var _class=w3_sb('w3-custom-events w3-hold cl-dx-label',has_ext?'dx-has-ext':'',filtered?'cl-dx-label-filtered':'',(has_ext&&!filtered)?'cl-dx-label-ext':'');var _style_attr=sprintf('|left:%s; z-index:%d; background:%s|id="%s-id-dx-label"',px(x-10),dx_z,color,dx_idx);s_a[dx_idx]=w3_button_path(_class+_style_attr,gid+'-gid','','dx_evt',w3_sbc(',',gid,cmkr_x))+
w3_div(sprintf('cl-dx-line|left:%s; z-index:110|id="%s-id-dx-line"',px(x),dx_idx));dx_z++;}
if(eibi&&dx.f_same.length>2)optimize_label_layout(same_x,lock_z,-1);console_log_lbl(dx.list);console_log_lbl(dx.displayed);var s=w3_div('id-dx-sig cl-dx-sig w3-hide');for(i=0;i<arr.length-1;i++)
if(isDefined(s_a[i]))
...
return s;};for(i=1;i<arr.length;i++){dx_idx=i-1;obj=arr[i];var el;var ident=obj.i;var freq=obj.f;var flags=obj.fl;if(eibi){if(ident==dx.last_ident&&freq==dx.last_freq){console_log_lbl('2222 dx_idx='+dx_idx+' last_dx_idx='+dx.last_dx_idx+' '+obj.f+' SAME AS LAST '+ident);el=w3_el(dx.last_dx_idx+'-id-dx-label');el.title+='\n'+dx_title(obj);continue;}}
dx.last_dx_idx=dx_idx;dx.last_freq=freq;dx.last_ident=ident;if(eibi)console_log_lbl('2222 dx_idx='+dx_idx+' '+obj.f+' OK '+ident);var notes=(isDefined(obj.n))?obj.n:'';el=w3_el(dx_idx+'-id-dx-label');if(!el)continue;var _ident=kiwi_decodeURIComponent('dx_ident2',ident).trim();if(_ident=='')_ident='&nbsp;&nbsp;';el.innerHTML=_ident.replace(/\\n/g,'<br>');var idx=dx_type2idx(obj.fl&dx.DX_TYPE);if(eibi){var ex=(flags&dx.DX_HAS_EXT)?'\nshift or alt/ctrl-click to not open extension':'';console_log_lbl(obj.i+' '+idx+' '+dx.eibi_ext[idx]+' '+ex);el.title=dx.eibi_svc_s[idx]+' // home country: '+obj.c+ex+'\n'+dx_title(obj);}else{var title=kiwi_decodeURIComponent('dx_notes',notes);title=title.replace(/<br>/g,'\n');title=title.replace(/\\n/g,'\n');if(stored)title=w3_nl(title)+'shift-click to edit label\nshift-option-click to toggle active';if(w3_contains(el,'dx-has-ext'))
title=w3_nl(title)+(stored?'':'shift or ')+'alt/ctrl-click to not open extension';var dow=obj.fl&dx.DX_DOW;if(dow!=dx.DX_DOW||(obj.b!=0||!(obj.b==0&&obj.e==2400)))
...
+' filter_tod='+dx.filter_tod[dx.db]);return;}}else{for(i=dx.displayed.length-1;i>=0;i--){dl=dx.displayed[i];if(!dl)continue;var f=Math.round(dl.freq*1e3);if(f<freq_displayed_Hz)break;}
if(i<0){wf_send('SET MARKER db='+dx.db+' dir=-1 freq='+(freq_displayed_Hz/1e3).toFixed(3)
+' eibi_types_mask='+dx.eibi_types_mask.toHex()
+' filter_tod='+dx.filter_tod[dx.db]);return;}}
var mode_s=kiwi.modes_lc[dx_decode_mode(dl.flags)];freqmode_set_dsp_kHz(dl.freq,mode_s);if(dl.lo!=0&&dl.hi!=0){ext_set_passband(dl.lo,dl.hi);}else{var dpb=kiwi_passbands(mode_s);ext_set_passband(dpb.lo,dpb.hi);}}
var dx_keys;function dx_admin_pwd_cb(path,val){w3_string_cb(path,val);ext_valpwd('admin',val,ws_wf);}
function dx_show_edit_panel(ev,gid,from_shortcut){dx_keys=ev?{shift:ev.shiftKey,alt:ev.altKey,ctrl:ev.ctrlKey,meta:ev.metaKey}:{shift:0,alt:0,ctrl:0,meta:0};dx.o.gid=gid;if(dx.db==dx.DB_STORED){ext_hasCredential('admin',function(badp){if(badp!=kiwi.BADP_OK){extint_panel_hide();var s=w3_inline('',w3_div('w3-medium w3-text-aqua w3-bold','DX labels'),w3_select('w3-margin-L-32/w3-label-inline w3-text-white /w3-text-red','database:','','dx.db',dx.db,dx.db_s,'dx_database_cb'))+
w3_div('w3-margin-T-16',w3_checkbox('/w3-label-inline w3-label-not-bold w3-text-white/','filter by time/day-of-week',dx.DB_STORED.toString()+'-filter-tod',dx.filter_tod[dx.DB_STORED],'dx_time_dow_cb'))+
...
if(dx.db==dx.DB_EiBi){for(var i=0;i<dx.DX_N_EiBi;i++){w3_color('eibi-cbox'+i+'-label','black',dx.eibi_colors[i]);}}},function(){},true);ext_set_controls_width_height(550,290);if(dx.help){extint_help_click_now();dx.help=false;}}
function dx_database_cb(path,idx,first,key_mod,from_shortcut){if(first)return;dx.list=[];console_log_dbgUs('DX DB-SWITCHED db='+dx.db+' key_mod='+key_mod);if(key_mod==shortcut.CTL_ALT||key_mod==shortcut.SHIFT_PLUS_CTL_ALT){dx.filter_tod[dx.db]^=1;dx_time_dow_cb(dx.db.toString()+'-filter-tod',dx.filter_tod[dx.db]);}else{dx.db=+idx;if(key_mod==shortcut.SHIFT){dx_show_edit_panel(null,-1,from_shortcut);}else{if(ext_panel_displayed('dx'))
dx_close_edit_panel();}}
kiwi_storeSet('last_db',dx.db);bands_init();mk_bands_scale();mk_band_menu();dx_schedule_update();}
...
function dx_add_cb(id,val){var flags=(dx.o.fd<<dx.DX_DOW_SFT)|(dx.o.ft<<dx.DX_TYPE_SFT)|dx_encode_mode(dx.o.fm);if(dx.o.fr<0)dx.o.fr=0;dx_send_update('DX_ADD',-1,dx.o.fr,flags,dx.o);setTimeout(function(){dx_close_edit_panel(id);},250);owrx.dx_click_gid_last_until_tune=owrx.dx_click_gid_last_stored=undefined;}
function dx_delete_cb(id,val){if(dx.o.gid==-1)return;dx_send_update('DX_DELETE',dx.o.gid,-1,0,null);setTimeout(function(){dx_close_edit_panel(id);},250);owrx.dx_click_gid_last_until_tune=owrx.dx_click_gid_last_stored=undefined;}
...
return rv;}
function dx_sig(gid,show){var label=dx.list[gid];var el=w3_el('id-dx-sig');if(show&&isArg(label.sig_bw)&&label.sig_bw!=0){var hbw=label.sig_bw/2;var freq=label.freq;var f_base=freq-kiwi.freq_offset_kHz;var xl=scale_px_from_freq((f_base-hbw)*1000,g_range);var xr=scale_px_from_freq((f_base+hbw)*1000,g_range);console.log('$dx_sig f='+f_base+' hbw='+hbw+' xl='+xl+' xr='+xr);el.style.left=px(xl);el.style.width=px(xr-xl);w3_hide2(el,false);}else{w3_hide2(el,true);console.log('$dx_sig OFF');}}
function dx_click(ev,gid){var hold=(ev.type=='hold');if(!hold&&ev.shiftKey&&dx.db==dx.DB_STORED){dx_show_edit_panel(ev,gid);}else{w3_menu_close('dx_click');dx_sig(gid,true);owrx.dx_click_gid_last_stored=(dx.db==dx.DB_STORED)?gid:undefined;owrx.dx_click_gid_last_until_tune=gid;var label=dx.list[gid];var freq=label.freq;var f_base=freq-kiwi.freq_offset_kHz;var mode=kiwi.modes_lc[dx_decode_mode(label.flags)];var lo=label.lo;var hi=label.hi;var params=label.params;extint.extname=extint.param=null;if(isArg(params)){params=decodeURIComponent(params);var ext=(params=='')?[]:params.split(',');if(mode=='drm'){extint.extname='drm';ext.push('lo:'+lo);ext.push('hi:'+hi);}else{extint.extname=ext[0];ext=ext.slice(1);}
if(ext[0]=='*')ext[0]=freq.toFixedNZ(2);extint.param=ext.join(',');}
var extname=extint.extname?extint.extname:'';console_log_dbgUs('dx_click gid='+gid+' f='+freq+'|'+f_base+' mode='+mode+' cur_mode='+cur_mode+' lo='+lo+' hi='+hi+' params=<'+params+'> extname=<'+extname+'> param=<'+extint.param+'>');if(dx.db==dx.DB_EiBi){}
...
function toggle_or_set_spec_peak(set,val,trace,ev){var hold=(ev&&ev.type=='hold');var shift=(ev&&ev.type=='click'&&ev.shiftKey);if(hold){trace=+val;console.log('toggle_or_set_spec_peak HOLD trace='+trace);if(spec.peak_show[trace]==spec.PEAK_ON){spec.peak_clear_btn[trace]=true;}
return;}
var prev;if(isNumber(set)){trace=+trace;prev=spec.peak_show[trace];spec.peak_show[trace]=kiwi_toggle(set,val,spec.peak_show[trace],'last_spec_peak'+(trace?'1':''));}else{trace=+val;prev=spec.peak_show[trace];if(shift){spec.peak_show[trace]--;if(spec.peak_show[trace]<0)spec.peak_show[trace]=2;}else{spec.peak_show[trace]=(spec.peak_show[trace]+1)%3;}}
...
function toggle_or_set_spec(set,val,dir){var no_close_ext=false;if(isNumber(set)&&(set&toggle_e.SET)){spec.source=+val;no_close_ext=((set&toggle_e.NO_CLOSE_EXT)!=0);}else{if(isUndefined(dir)||dir==1){spec.source=(spec.source+1)%spec.CHOICES;}else{spec.source--;if(spec.source<0)spec.source=spec.CHOICES-1;}}
...
/* web/pkgs/xdLocalStorage/xdLocalStorage.min.js (web/pkgs/xdLocalStorage/xdLocalStorage.js = Sat May 13 08:24:35 2023) */
'use strict';window.xdLocalStorage=window.xdLocalStorage||(function(){var MESSAGE_NAMESPACE='cross-domain-local-message';var options={iframeId:'cross-domain-iframe',iframeUrl:undefined,initCallback:function(){}};var requestId=-1;var iframe;var requests={};var wasInit=false;var iframeReady=false;function applyCallback(data){if(requests[data.id]){requests[data.id](data);delete requests[data.id];}}
...
function buildMessage(action,key,value,callback){requestId++;requests[requestId]=callback;var data={namespace:MESSAGE_NAMESPACE,etag:0,server:0,id:requestId,action:action,key:key,value:value};iframe.contentWindow.postMessage(JSON.stringify(data),'*');}
...
buildMessage('clear',null,null,callback);},wasInit:function(){return wasInit;}};})();window.xdLocalStorageHA=window.xdLocalStorageHA||(function(){var MESSAGE_NAMESPACE='cross-domain-local-message';var options={iframeId:'cross-domain-iframe',iframeUrls:undefined,initCallback:function(){}};var requestId=-1;var iframes=[];var seen=[];var seen_ct=0;var requests={};var replies=[];var wasInit=false;var iframeReady=false;setTimeout(function(){buildMessage('ping',null,null,null);setTimeout(function(){console.log('XDLS seen servers='+seen_ct);iframeReady=true;options.initCallback();},1000);},5000);function applyCallback(data){if(requests[data.id]){requests[data.id](data);delete requests[data.id];}}
function receiveMessage(event){var data;try{data=JSON.parse(event.data);}catch(err){}
if(data&&data.namespace===MESSAGE_NAMESPACE){if(data.id==='iframe-ready'){}else
if(data.action==='ping'){seen[data.server]=true;seen_ct++;console.log('XDLS ping reply server='+data.server+' seen_ct='+seen_ct);}else{replies[data.server]=data;}}}
function buildMessage(action,key,value,callback){requestId++;requests[requestId]=callback;var data={namespace:MESSAGE_NAMESPACE,etag:Date.now(),id:requestId,action:action,key:key,value:value};for(var server=0;server<iframes.length;server++){if(action!='ping'&&!seen[server])continue;data.server=server;replies[server]=null;iframes[server].contentWindow.postMessage(JSON.stringify(data),'*');}
if(action=='ping')return;setTimeout(function(){var mostRecent_etag=-1,mostRecent_server=-1;for(var server=0;server<iframes.length;server++){var data=replies[server];if(data==null)continue;var etag=data.etag?data.etag:0;if(etag>mostRecent_etag){mostRecent_etag=etag;mostRecent_server=server;}}
if(mostRecent_server!=-1)
applyCallback(replies[mostRecent_server]);else
console.log('XDLS wait longer for response?');},250);}
function init(customOptions){options=XdUtils.extend(customOptions,options);if(window.addEventListener){window.addEventListener('message',receiveMessage,false);}else{window.attachEvent('onmessage',receiveMessage);}
var i=0;options.iframeUrls.forEach(function(url){var iframeId='id-'+options.iframeId+'-'+i;var el=document.createElement('div');w3_add(el,'id-'+options.iframeId);el.innerHTML='<iframe id="'+iframeId+'" src='+url+' style="display: none;"></iframe>';document.body.appendChild(el);iframes[i]=document.getElementById(iframeId);i++;});}
function isApiReady(){if(!wasInit){console.log('You must call xdLocalStorage.init() before using it.');return false;}
if(!iframeReady){console.log('You must wait for iframe ready message before using the api.');return false;}
return true;}
return{init:function(customOptions){if(!customOptions.iframeUrls){throw'You must specify iframeUrls';}
if(wasInit){console.log('xdLocalStorage was already initialized!');return;}
wasInit=true;if(document.readyState==='complete'){init(customOptions);}else{window.onload=function(){init(customOptions);};}},setItem:function(key,value,callback){if(!isApiReady()){return;}
buildMessage('set',key,value,callback);},getItem:function(key,callback){if(!isApiReady()){return;}
buildMessage('get',key,null,callback);},removeItem:function(key,callback){if(!isApiReady()){return;}
...
/* web/pkgs/xdLocalStorage/xd-utils.min.js (web/pkgs/xdLocalStorage/xd-utils.js = Tue Oct 22 10:32:15 2019) */
