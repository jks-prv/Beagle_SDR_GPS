...
/* web/extensions/ext.min.js (web/extensions/ext.js = Mon Jul 15 11:41:46 2024) */
...
if(isArg(freq_dial_Hz)){freq_dial_Hz*=1000;freq_car_Hz=freq_dsp_to_car(freq_dial_Hz);extint.ext_is_tuning=true;demodulator_set_offset_frequency(owrx.FSET_EXT_SET_PB,freq_car_Hz-center_freq);extint.ext_is_tuning=false;}else{demodulators[0].set();if(!wf.audioFFT_active)
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Sun Jul 14 12:21:59 2024) */
...
if(i==rx_chan&&isNumber(obj.fo)&&obj.fo!=kiwi.freq_offset_kHz&&!confirmation.displayed){var s=w3_div('','Frequency scale offset changed. Page will be reloaded.');confirmation_show_content(s,425,50,null,'red');kiwi_open_or_reload_page({url:'reload',delay:2000});}
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Mon Jul  8 10:53:13 2024) */
...
function canvas_log_int(s){var el;if(!kiwi.canvas_log_init){el=w3_el('id-news');el.style.top='';el.style.bottom='';el.style.left='';el.style.right='';if(kiwi_isMobile()){var w=screen.width;var h=screen.height;if(w>=600&&h>=950){el.style.top='200px';el.style.left='0';el.style.width='350px';el.style.height='400px';}else{el.style.top='36px';el.style.right='0';el.style.width='150px';el.style.height='150px';}}else{el.style.bottom='0';el.style.left='0';el.style.width='350px';el.style.height='300px';}
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Mon Jul 15 11:56:21 2024) */
...
/* web/kiwi/waterfall.min.js (web/kiwi/waterfall.js = Thu Jul 11 17:38:32 2024) */
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Mon Jul 15 11:36:22 2024) */
...
function update_web_grid(init){var grid=isNonEmptyString(kiwi.GPS_auto_grid)?kiwi.GPS_auto_grid:cfg.index_html_params.RX_QRA;if(init!=true&&!cfg.GPS_update_web_grid)return;w3_innerHTML('id-web-grid',w3_link('','https://www.levinecentral.com/ham/grid_square.php?Grid='+grid,grid,'','dont_toggle_rx_photo'));}
function update_web_map(init){var rx_gps=w3_json_to_string('rx_gps',cfg.rx_gps);rx_gps=isNonEmptyString(kiwi.GPS_auto_latlon)?kiwi.GPS_auto_latlon:rx_gps;if(init!=true&&!cfg.GPS_update_web_lores&&!cfg.GPS_update_web_hires)return;w3_link_set('id-rx-gps','https://www.google.com/maps/place/'+rx_gps);}
...
var scale_canvas_drag_params={mouse_down:false,drag:false,start_x:0,last_x:0,start_y:0,last_y:0,key_modifiers:{shiftKey:false,altKey:false,ctrlKey:false}};function scale_canvas_mousedown(evt){mouse_button_state(evt);scale_canvas_drag_params.drag=false;scale_canvas_drag_params.start_x=evt.pageX;scale_canvas_drag_params.start_y=evt.pageY;scale_canvas_drag_params.key_modifiers.shiftKey=evt.shiftKey;scale_canvas_drag_params.key_modifiers.altKey=evt.altKey;scale_canvas_drag_params.key_modifiers.ctrlKey=evt.ctrlKey;if(!w3_menu_active())
...
right_click_menu(scale_canvas_drag_params.start_x,scale_canvas_drag_params.start_y,1);scale_canvas_drag_params.mouse_down=owrx.scale_canvas.mouse_out=false;scale_canvas_ignore_mouse_event=true;return;}
...
function scale_container_mouseover(evt){if(owrx.debug_drag)canvas_log('SMO Cdrag='+TF(canvas_dragging)+' Sdrag='+TF(owrx.spec_pb_mkr_dragging));if(canvas_dragging||owrx.spec_pb_mkr_dragging){spectrum_tooltip_cancel();canvas_end_drag2();}}
...
owrx.drag_count=0;canvas_mouse_down_or_touch=true;canvas_dragging=false;owrx.canvas.drag_last_x=owrx.canvas.drag_start_x=x;owrx.canvas.drag_last_y=owrx.canvas.drag_start_y=y;owrx.canvas.target=evt.target;spectrum_tooltip_update(owrx.START,evt,x,y);return cancelEvent(evt);}
...
function canvas_touchStart(evt){var touches=evt.targetTouches.length;var x=Math.round(evt.targetTouches[0].pageX);var y=Math.round(evt.targetTouches[0].pageY);if(owrx.debug_drag)canvas_log("$C-TS"+touches+'-x'+x+'-y'+y);owrx.double_touch_start=false;if(touches==1){if(evt.target.id.startsWith('id-spectrum')){owrx.canvas.drag_last_x=owrx.canvas.drag_start_x=x;owrx.canvas.drag_last_y=owrx.canvas.drag_start_y=y;if(owrx.debug_drag)canvas_log('*SPEC* -x'+x+'-y'+y);owrx.canvas.target=evt.target;spectrum_tooltip_update(owrx.START,evt,x,y);}else{canvas_start_drag(evt,x,y);}}else
...
function canvas_drag(evt,idx,x,y,clientX,clientY){if(!waterfall_setup_done)return;if(spectrum_tooltip_update(owrx.DRAG,evt,clientX,clientY))return;owrx.drag_count=!isNumber(owrx.drag_count)?0:(owrx.drag_count+1);if(owrx.scale_canvas.mouse_out){scale_canvas_drag(evt,x,y);return;}
...
if(!waterfall_setup_done)return;if(owrx.debug_drag)canvas_log('C-ED IME'+(canvas_ignore_mouse_event?1:0)+' CD'+(canvas_dragging?1:0));if(!canvas_dragging){var tooltip_complete=spectrum_tooltip_update(owrx.END);if(owrx.debug_drag)canvas_log('MUP-ttip'+(tooltip_complete?1:0));if(owrx.debug_drag)canvas_log('CMDT'+(canvas_mouse_down_or_touch?1:0)+' TL'+owrx.tuning_locked);if(canvas_mouse_down_or_touch&&!canvas_ignore_mouse_event&&!tooltip_complete){if(owrx.tuning_locked)canvas_log('TLOCK='+owrx.tuning_locked);if(owrx.tuning_locked){var el=w3_el('id-tuning-lock-container');el.style.opacity=0.8;w3_show(el);var el2=w3_el('id-tuning-lock');el2.style.marginTop=px(w3_center_in_window(el2,'TL'));setTimeout(function(){el.style.opacity=0;setTimeout(function(){w3_hide(el);},500);},300);}else
...
canvas_log('C-TE-x'+x+'-DTS'+(owrx.double_touch_start?1:0)+'-DTSw'+(owrx.double_touch_swipe?1:0)+'-DRAG'+(canvas_dragging?1:0));try{canvas_end_drag(evt,x);}catch(ex){canvas_catch_log(ex);}
if(owrx.double_touch_start){if(owrx.debug_drag)canvas_log('DTS-D='+TF(canvas_dragging));demodulator_set_offset_frequency(owrx.FSET_SCALE_TOUCH_DRAG_END,freq_car_Hz-center_freq);if(!canvas_dragging){if(kiwi_isMobile()&&owrx.mobile&&owrx.mobile.small)
...
evt.preventDefault();}
function canvas_mousewheel_cb(evt){if(!waterfall_setup_done)return;var x=evt.deltaX;var y=evt.deltaY;var fwd_bak=((x<0&&y<=0)||(x>=0&&y<0));var key_mod=shortcut_key_mod(evt);if(evt.target&&evt.target.id&&(evt.target.id=='id-scale-canvas'||evt.target.id.startsWith('id-pb-'))){var edge=owrx.PB_LO_EDGE;if(key_mod==shortcut.NO_MODIFIER)edge=owrx.PB_NO_EDGE;else
...
if(arg2!=ext_zoom.TO_PASSBAND){var x_rel=arg2;x_norm=x_rel / waterfall_width;}else{var pb_bin=passband_visible();if(pb_bin>=0){x_norm=(pb_bin-x_bin)/ bins_at_cur_zoom();}else{x_norm=zoom_center;}}
...
function mobile_init(){var mobile=owrx.mobile=ext_mobile_info();if(mobile.small){toggle_or_set_hide_bars(owrx.HIDE_TOPBAR);keyboard_shortcut_nav('off');}
...
function spectrum_tooltip_cancel(){var demod=demodulators[0];var allow_pb_adj=(!kiwi_isMobile()&&owrx.allow_pb_adj);var is_active=(owrx.spec_pb_mkr_dragging!=owrx.PB_NO_EDGE);if(allow_pb_adj)demod.envelope.drag_end(0);owrx.spec_pb_mkr_dragging=owrx.PB_NO_EDGE;owrx.allow_pb_adj_at_start=false;return is_active;}
function spectrum_tooltip_update(type,evt,clientX,clientY){var mouse_L=(evt&&evt.buttons&mouse.BUTTONS_L);if(!mouse_L)owrx.allow_pb_adj_at_start=owrx.allow_pb_adj;var allow_pb_adj=(!kiwi_isMobile()&&owrx.allow_pb_adj_at_start);var is_active=(owrx.spec_pb_mkr_dragging!=owrx.PB_NO_EDGE);var demod=demodulators[0];if(type==owrx.END)
return spectrum_tooltip_cancel();var target=(evt&&(evt.target==spec.dB||evt.currentTarget==spec.dB||evt.target==spec.dB_ttip||evt.currentTarget==spec.dB_ttip));if(!target)
return spectrum_tooltip_cancel();if(type==owrx.DRAG&&is_active&&!mouse_L){return spectrum_tooltip_cancel();}
if(owrx.debug_drag)canvas_log('*STTU*');var h=spec.height_spectrum_canvas;if(clientY<0||clientY>=h)return false;var adj_pb=false,show_dB=true;var cx=clientX-(kiwi_isMobile()?spec.tooltip_offset:0);if(owrx.debug_drag)canvas_log('['+cx+','+clientY+']');var f_kHz,f_text;var isRF=(spec.source==spec.RF);if(isRF){var x1=owrx.pbx1,x2=owrx.pbx2;var o=5;var lo_edge=(cx>=(x1-o)&&cx<=(x1+o));var hi_edge=(cx>=(x2-o)&&cx<=(x2+o));if(allow_pb_adj&&(lo_edge||hi_edge||owrx.spec_pb_mkr_dragging!=owrx.PB_NO_EDGE)){if(canvas_mouse_down_or_touch){if(type==owrx.START){owrx.spec_pb_mkr_dragging=lo_edge?owrx.PB_LO_EDGE:owrx.PB_HI_EDGE;demod.envelope.env_drag_start(cx,0,{shiftKey:false,altKey:false,ctrlKey:false});}else
if(type==owrx.DRAG){demod.envelope.drag_move(cx);}
adj_pb=true;}
...
setTimeout(keyboard_shortcut_url_keys,5000);canvas_mouse_wheel_set(kiwi_storeInit('wheel_tunes'));wf_snap(kiwi_storeInit('wf_snap'));if(kiwi_isMobile())
mobile_init();dx_init();if(cfg.init.setup==owrx.SETUP_RF_SPEC_WF){toggle_or_set_hide_bars(owrx.HIDE_ALLBARS);toggle_or_set_spec(toggle_e.SET,spec.RF);}else
...
if(cfg.init.tab)keyboard_shortcut_nav(kiwi.tab_s[cfg.init.tab]);waterfall_setup_done=1;}
