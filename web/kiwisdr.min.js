...
/* web/extensions/ext.min.js (web/extensions/ext.js = Sun Feb 11 12:30:08 2024) */
...
var extint_first_ext_load=true;function extint_select(value){extint_blur_prev(0);w3_call(extint.hide_func);freqset_select();value=+value;var el=w3_el('id-select-ext');if(!el){console.log('$ extint_select NOT READY v='+value);setTimeout(extint_select,1000,value);return;}
...
return s;}
function extint_open(name,delay){console.log('extint_open rx_chan='+rx_chan+' ext_auth='+ext_auth());name=name.toLowerCase();if(name=='dsc')name='navtex';else
if(name=='ft4')name='ft8';else;var found=0;extint_names_enum(function(i,value,id,id_en){var enable=ext_get_cfg_param(id_en+'.enable');if(enable==null||ext_auth()==kiwi.AUTH_LOCAL)enable=true;if(!found&&enable&&id.toLowerCase().includes(name)){if(delay){console.log('extint_open '+name+' delay='+delay);setTimeout(function(){extint_select(value);},delay);}else{console.log('extint_open '+name+' NO DELAY');extint_select(value);}
found=1;}});}
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Sat Feb 10 18:20:13 2024) */
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Mon Feb 12 09:59:29 2024) */
...
var len=pwr_dBm.length;if(len){pwr_dBm.sort(function(a,b){return a-b});var noise=pwr_dBm[Math.floor(0.50*len)];var signal=pwr_dBm[Math.floor(0.95*len)];var _10=pwr_dBm[Math.floor(0.10*len)];var _20=pwr_dBm[Math.floor(0.20*len)];}else{signal=-110;noise=-120;}
...
function keyboard_shortcut_nav(nav){virt_optbar_click=true;w3_el('id-nav-optbar-'+nav).click();shortcut.nav_click=true;}
...
var prev_optbar=null;var virt_optbar_click=false;function optbar_focus(next_id,cb_arg){writeCookie('last_optbar',next_id);var h;if(next_id=='optbar-off'){if(cb_arg!='init'&&prev_optbar=='optbar-off'&&!virt_optbar_click)toggle_or_set_hide_bars();virt_optbar_click=false;w3_hide('id-optbar-content');w3_el('id-control-top').style.paddingBottom='8px';}else{w3_show_block('id-optbar-content');w3_el('id-control-top').style.paddingBottom='';}
...
var bin_server=0;var zoom_server=0;function owrx_msg_cb(param,ws){switch(param[0]){case"wf_setup":wf_init();break;case"extint_list_json":extint_list_json(param[1]);console.log('extint_list_json override_ext='+override_ext+' waterfall_setup_done='+waterfall_setup_done);if(override_ext){w3_do_when_cond(function(){console.log('### '+(waterfall_setup_done?'GO':'WAIT')+' extint_open('+override_ext+')');return waterfall_setup_done;},function(){extint_open(override_ext,3000);},null,200);}
