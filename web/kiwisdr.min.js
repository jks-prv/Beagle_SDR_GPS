...
w3_input('w3-RIF w3-margin-TB-8/w3-label-inline w3-label-not-bold/kiwi-pw|padding:1px|size=40','Password:','id-epwd','','kiwi_show_error_ask_exemption_cb');kiwi_show_msg(s);w3_field_select('id-epwd',{mobile:1});}
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Wed May 15 07:36:17 2024) */
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Wed May 15 15:12:22 2024) */
var w3={_last_:0};var w3int={btn_grp_uniq:0,menu_cur_id:null,menu_active:false,menu_debug:false,menu_close_func:null,prev_menu_hover_el:null,reload_timeout:null,reload_seq:0,rate_limit:{},rate_limit_need_shift:{},rate_limit_evt:{},_last_:0};function visible_block(){}
...
return null;}
function w3_event_log(el_id,id,event_name){var el=w3_el(el_id);if(el==null)return;el.addEventListener(event_name,function(){console.log('w3_event_log '+id+': '+event_name);});}
function w3_retain_input_focus(opt){var ae=document.activeElement;var id=w3_id(ae);if(ae&&id=='id-freq-input'){return false;}
var closed_ext_input_still_holding_focus=(ae&&w3_contains(ae,'w3-RIF-EXT')&&!opt.ext);if(closed_ext_input_still_holding_focus){console.log('#### closed_ext_input_still_holding_focus');}
var retain_input_focus=(ae&&(w3_match_wildcard(ae,'w3-RIF')||w3_contains(ae,'w3-ADJ')));if(retain_input_focus){}
if(opt.scan){return true;}
var no_retain=(!ae||!retain_input_focus||closed_ext_input_still_holding_focus);if(0){console.log('w3_retain_input_focus: RETAIN='+TF(!no_retain)+' id='+id+' RIF='+retain_input_focus+' closed_ext_input_still_holding_focus='+closed_ext_input_still_holding_focus);console.log(ae);}
return!no_retain;}
function w3_click_cancel_focus(){var ae=document.activeElement;if(ae&&w3_contains(ae,'w3-ADJ')){w3_field_select('id-freq-input',{mobile:1,log:1});}}
...
if(w3_match_wildcard(el,'w3-RIF'))
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Wed May 15 16:55:09 2024) */
...
canvas_log('C-TE-x'+x+'-DTS'+(owrx.double_touch_start?1:0)+'-DTSw'+(owrx.double_touch_swipe?1:0)+'-DRAG'+(canvas_dragging?1:0));canvas_end_drag(evt,x);spectrum_tooltip_update(evt,x,y);if(owrx.double_touch_start){if(owrx.debug_drag)canvas_log('DTS-D='+TF(canvas_dragging));demodulator_set_offset_frequency(owrx.FSET_SCALE_TOUCH_DRAG_END,freq_car_Hz-center_freq);if(!canvas_dragging){if(kiwi_isMobile()&&owrx.mobile&&owrx.mobile.small)
...
function freqset_select(){if(retain_input_focus()){}else{w3_field_select('id-freq-input',{mobile:1,log:1});}}
...
return s;}
function mk_band_menu(){band_menu=[];owrx.last_selected_band=0;var s='id-select-band';var el=w3_el(s);w3_innerHTML(el,setup_band_menu());}
...
w3_divs('w3-text-aqua',w3_col_percent('',w3_divs('/w3-margin-T-8',w3_input('w3-label-inline/id-dx-filter-ident w3-RIF w3-input-any-change w3-padding-small','Ident','dx.filter_ident',dx.filter_ident,'dx_filter_cb'),w3_input('w3-label-inline/id-dx-filter-notes w3-RIF w3-input-any-change w3-padding-small','Notes','dx.filter_notes',dx.filter_notes,'dx_filter_cb')),90),w3_inline('w3-halign-space-around w3-margin-T-8 w3-text-white/',w3_checkbox('w3-RIF w3-label-inline w3-label-not-bold','case sensitive','dx.filter_case',dx.filter_case,'dx_filter_opt_cb'),w3_inline('',w3_text('','pattern match:'),w3_checkbox('w3-margin-left w3-RIF w3-label-inline w3-label-not-bold','grep','dx.filter_grep',dx.filter_grep,'dx_filter_opt_cb'))));confirmation_show_content(s,450,140,dx_filter_panel_close);w3_field_select('id-dx-filter-ident',{mobile:1});}
...
var field_or_slider=(evt.target.nodeName=='INPUT');var slider_not_field=(evt.target.type=='range');if(!field_or_slider||slider_not_field||(id=='id-freq-input'&&!field_input_key)){if(kiwi_isMacOS()){if(meta){return;}}else{if(ctl){return;}}
...
function test_audio_suspended(){var s1,onclick=null,input=false,ident=kiwi_storeInit('ident','');if(cfg.require_id&&isEmptyString(ident)){snd_send('SET require_id=1');s1=w3_input('w3-dump w3-flex w3-flex-col w3-valign-center//w3-custom-events w3-margin-T-10 w3-font-18px w3-normal'+'|padding:1px; width:300px|size=20'+' onchange="ident_complete(\'el\', 3)" onkeyup="ident_keyup(this, event, 3)"','Enter your name or callsign <br> to start KiwiSDR','ident-input3');onclick='';input=true;}else
...
var WF_SPEED_OFF=0;var WF_SPEED_1HZ=1;var WF_SPEED_SLOW=2;var WF_SPEED_MED=3;var WF_SPEED_FAST=4;var wf_speed=WF_SPEED_FAST;var wf_speeds=['off','1 Hz','slow','med','fast'];var wf={no_wf:false,cal:0,url_tstamp:0,ts_tz:0,COMPRESSED:1,NO_SYNC:2,scroll_multiple:3,no_sync:false,cmap:0,cmap_override:-1,custom_colormaps:[new Uint8Array(3*256),new Uint8Array(3*256),new Uint8Array(3*256),new Uint8Array(3*256)],aper:0,aper_algo:3,aper_param:0,sqrt:0,contr:0,last_zoom:-1,need_autoscale:0,FNAS:0,auto_ceil:{min:0,val:5,max:30},auto_floor:{min:-30,val:0,max:0},auto_maxdb:0,auto_mindb:0,audioFFT_active:false,audioFFT_prev_mode:'',audioFFT_clear_wf:false,};var wf_contr_s={0:'normal',1:'scheme 1',2:'scheme 2',3:'scheme 3',4:'scheme 4'};function wf_contr_cb(path,idx,first){}
