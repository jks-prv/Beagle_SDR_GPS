...
var ext_zoom={TO_BAND:0,IN:1,OUT:-1,ABS:2,WHEEL:3,CUR:4,NOM_IN:8,MAX_IN:9,MAX_OUT:-9};function ext_tune(freq_dial_kHz,mode,zoom,zlevel,low_cut,high_cut,opt){var pb_specified=(low_cut!=undefined&&high_cut!=undefined);extint.ext_is_tuning=true;freq_dial_kHz=freq_dial_kHz||(freq_displayed_Hz / 1000);freqmode_set_dsp_kHz(freq_dial_kHz,mode,opt);if(pb_specified)ext_set_passband(low_cut,high_cut);if(isArg(zoom)){zoom_step(zoom,zlevel);}else{zoom_step(ext_zoom.TO_BAND);}
...
extint.ext_is_tuning=true;demodulator_set_offset_frequency(owrx.FSET_EXT_SET_PB,freq_car_Hz-center_freq);extint.ext_is_tuning=false;}
...
w3_select_value(el,value);var menu=el.childNodes;var name=menu[value+1].innerHTML.toLowerCase();var idx;extint_enum_names(function(i,value,id,id_en){if(name.startsWith(id.toLowerCase())){idx=i;}});if(name=='ant_switch'){ant_switch_focus();ant_switch_view();}else{ant_switch_blur();if(['waterfall','noise_blank','noise_filter'].includes(name)){w3_call(name+'_view');}else{extint_blur_prev(0);w3_call(extint.hide_func);extint.current_ext_name=extint.ext_names[idx];if(extint.first_ext_load){extint_connect_server();extint.first_ext_load=false;}else{ext_send('SET ext_is_locked_status');}}}}
function extint_list_json(param){extint.ext_names=kiwi_JSON_parse('extint_list_json',decodeURIComponent(param));extint.ext_names.push('ant_switch');extint.ext_names.push('waterfall');extint.ext_names.push('noise_blank');extint.ext_names.push('noise_filter');kiwi_dedup_array(extint.ext_names);extint.ext_names.sort(kiwi_sort_ignore_case);}
...
function kiwi_init_cfg(stream_name){kiwi_set_freq_offset(cfg.freq_offset);var page_title=ext_get_cfg_param_string('cfg.index_html_params.PAGE_TITLE');if(page_title=='')page_title='KiwiSDR';w3_innerHTML('id-page-title',((stream_name=='admin')?'Admin ':'')+page_title);w3_innerHTML('id-rx-photo-title',ext_get_cfg_param_string('cfg.index_html_params.RX_PHOTO_TITLE'));w3_innerHTML('id-rx-photo-desc',ext_get_cfg_param_string('cfg.index_html_params.RX_PHOTO_DESC'));w3_innerHTML('id-rx-title',ext_get_cfg_param_string('cfg.index_html_params.RX_TITLE'));w3_innerHTML('id-owner-info',ext_get_cfg_param_string('cfg.owner_info'));}
...
((param[1]!='')?(' Reason: '+w3_json_to_html('reason_disabled',param[1])):'');break;case"sample_rate":extint.srate=parseFloat(param[1]);break;case'pref_import_ch':kiwi.prefs_import_ch=+param[1];break;case'pref_import':prefs_import_cb(param[1],kiwi.prefs_import_ch);break;case'adc_clk_nom':extint.adc_clock_nom_Hz=+param[1];break;case'notify_msg':var s=w3_json_to_html('notify_msg',param[1]);console.log('notify_msg: '+s);if(confirmation.displayed)break;s=w3_div('',s);confirmation_show_content(s,425,50);setTimeout(confirmation_panel_close,3000);break;case'kiwi_kick':var s=ext_get_cfg_param_string('cfg.reason_kicked');var s=w3_json_to_html('kiwi_kick',param[1])+((s!='')?('<br>Reason: '+s):'');if(confirmation.displayed)break;s=w3_div('',s);confirmation_show_content(s,425,75,null,'red');break;case'snr_stats':var p=param[1].split(',');kiwi_snr_stats(p[0],p[1]);break;default:if(param[0].startsWith('antsw_')){if(ant_switch_msg(param))
break;}
rtn=false;break;}
return rtn;}
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Sun Mar 31 10:33:03 2024) */
...
if(!website)console.log('Safari='+kiwi_isSafari()+' Firefox='+kiwi_isFirefox()+' Chrome='+kiwi_isChrome()+' Opera='+kiwi_isOpera());if(!website){migrateCookies();kiwi_load_js_dir('pkgs/js/',['sprintf/sprintf.js','SHA256.js','coloris/coloris.js'],function(){kiwi_load2();});}else{kiwi_load2();}}}
...
var kiwi_version_fail=false;function kiwi_version_cb(response_obj){version_maj=response_obj.maj;version_min=response_obj.min;kiwi.admin_save_pwd=response_obj.sp
var s='';kiwi_check_js_version.forEach(function(el){if(el.VERSION_MAJ!=version_maj||el.VERSION_MIN!=version_min){if(kiwi_version_fail==false){s='Your browser is trying to use incorrect versions of the KiwiSDR Javascript files.<br>'+'Please clear your browser cache and try again.<br>'+'Or click the button below to continue anyway.<br><br>'+'v'+version_maj+'.'+version_min+': KiwiSDR server<br>';kiwi_version_fail=true;}
...
function kiwi_array_iter(s,func){var ra=[];s.split('').forEach(function(c,i){ra[i]=func(c,i);});return ra;}
function kiwi_string_to_hex(s,sep){var sa=kiwi_array_iter(s,function(c,i){return c.charCodeAt(0).toHex(-2);});return sa.join(sep||'');}
...
return str;}
function kiwi_inet4_d2h(inet4_str,opt){var s=inet4_str.split('.');if(s.length!=4)return null;var check=function(v){var n=parseInt(v);if(!isNumber(n)||n<0||n>255)return null;return n;}
...
function kiwi_open_or_reload_page(obj){var url=w3_opt(obj,'url',null);if(isNull(url)){var host_port=w3_opt(obj,'hp',kiwi_host_port());var pathname=w3_opt(obj,'path','','/');var query_string=w3_opt(obj,'qs','','/?');url=kiwi_SSL()+host_port+pathname+query_string;}
console.log('kiwi_open_or_reload_page: '+url);var rv;if(w3_opt(obj,'tab')){rv=window.open(url,'_blank');}else{window.location.href=url;rv=true;}
return rv;}
...
function kiwi_storeRead(k,def){var rv;if(isNoArg(k))return null;if(localStorage==null){return readCookie(v,def);}
...
return rv;}
function kiwi_storeInit(k,init){var rv;if(isNoArg(k))return null;if(localStorage==null){return initCookie(v,init);}
...
return rv;}
function kiwi_storeWrite(k,v){if(isNoArg(k))return null;if(localStorage==null){return writeCookie(k,v);}
...
return true;}
function kiwi_storeDelete(k){if(isNoArg(k))return null;if(localStorage==null){return deleteCookie(k);}
try{localStorage.removeItem(k);}catch(ex){return null;}
return true;}
function migrateCookies(){if(!isArg(document.cookie))return;var ca=document.cookie.split(';');var len=ca.length;if(len==0||(len==1&&ca[0]==''))return;for(var i=0;i<len;i++){var c=ca[i].trim();var a=c.split('=');var k=a[0];var v=a[1];console.log('$>migrateCookies k='+k+' v='+sq(v)+' '+typeof(v));var rv=kiwi_storeRead(k);if(isArg(rv)){console.log('$>migrateCookies WARNING key already exists in new storage? k='+k+' v='+sq(rv));}else{kiwi_storeWrite(k,v);}
deleteCookie(k);}
...
var sendmail=function(to,subject){var s="mailto:"+enc(decodeURIComponent(to))+(isDefined(subject)?('?subject='+subject):'');var o={url:s};if(!(kiwi_isSafari()||kiwi_isFirefox()))o.tab=1;kiwi_open_or_reload_page(o);}
...
var no_wf=kiwi_url_param('no_wf');var tstamp=(w3_opt(opt,'new_ts')==true)?((new Date()).getTime()):kiwi.conn_tstamp;ws_url=ws_protocol+ws_url+'/'+(no_wf?'no_wf/':'kiwi/')+tstamp+'/'+stream;var qs=w3_opt(opt,'qs');if(isString(qs))
ws_url+='?'+qs;else
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Wed Apr  3 14:34:26 2024) */
var w3={_last_:0};var w3int={menu_cur_id:null,menu_active:false,menu_debug:false,menu_close_func:null,prev_menu_hover_el:null,reload_timeout:null,reload_seq:0,rate_limit:{},rate_limit_need_shift:{},rate_limit_evt:{},_last_:0};function visible_block(){}
...
function w3_json_to_string(id,s){s=kiwi_decodeURIComponent(id,s);s=s.replace(/\\"/g,'"');return s;}
function w3_json_to_html(id,s){s=kiwi_decodeURIComponent(id,s);s=s.replace(/\n/g,'<br>').replace(/\\"/g,'"');return s;}
...
function w3_set_psa(el,psa){var el=w3_el(el);if(!el||isEmptyString(psa))return null;psa=psa.split('/')[0];psa=psa.split('|');var _class=psa[0];var _style=psa[1];var _attr=psa[2];if(isArg(_class))w3_add(el,_class);if(isArg(_style))el.style=_style;}
function w3int_init(){}
...
function w3_checkbox_get_param(psa,label,path,cb,init_val){var update_path_var=psa.includes('w3-update');var save=psa.includes('w3-defer')?EXT_SAVE_DEFER:undefined;var cur_val=ext_get_cfg_param(path,(init_val==undefined)?null:init_val,save,update_path_var);return w3_checkbox(psa,label,path,cur_val,cb);}
...
function w3_select_get_param(psa,label,title,path,opts,cb,init_val){var update_path_var=psa.includes('w3-update');var save=psa.includes('w3-defer')?EXT_SAVE_DEFER:undefined;var cur_val=ext_get_cfg_param(path,(init_val==undefined)?0:init_val,save,update_path_var);return w3_select(psa,label,title,path,cur_val,opts,cb);}
...
evt.preventDefault();}
function w3_slider_wheel(cb,el,cur,slow,fast){var el=w3_el(el);if(!el)return null;var evt=w3int.rate_limit_evt[cb];if(w3int.rate_limit_need_shift[cb]&&!evt.shiftKey)
return null;var x=evt.deltaX;var y=evt.deltaY;var ay=Math.abs(y);var up_down=(ay<50)?slow:fast;if(y>0)up_down=-up_down;var nval=cur+up_down;return w3_clamp(nval,+el.min,+el.max);}
...
function w3_string_set_cfg_cb(path,val,first){console.log('w3_string_set_cfg_cb: path='+path+' '+typeof(val)+' <'+val+'> first='+first);w3_json_set_cfg_cb(path,val,first);}
function w3_json_set_cfg_cb(path,val,first){console.log('w3_json_set_cfg_cb: path='+path+' '+typeof(val)+' <'+val+'> first='+first);var v=val;if(isString(v))v=JSON.stringify(v);try{JSON.parse(v);}catch(ex){console.log('w3_json_set_cfg_cb: path='+path+' '+typeof(val)+' <'+val+'> first='+first);console.log('w3_json_set_cfg_cb: DANGER value did not parse as JSON!');return;}
var save=isArg(first)?(first?false:true):true;ext_set_cfg_param(path,val,save);}
...
/* web/kiwi/monitor.min.js (web/kiwi/monitor.js = Tue Mar 12 09:30:37 2024) */
...
function kiwi_queue_reload_cb(){kiwi_open_or_reload_page({url:kiwi_remove_search_param(window.location.href,'camp')});}
...
/* web/kiwi/waterfall.min.js (web/kiwi/waterfall.js = Tue Mar 26 09:47:16 2024) */
...
function waterfall_controls_setup(){var controls_html=w3_div('id-waterfall-controls w3-text-white',w3_divs('w3-margin-T-8',w3_text('w3-margin-B-2 w3-text-css-orange','<b>Aperture auto mode</b>'),w3_col_percent('w3-valign w3-margin-LR-16/',w3_select('id-wfext-aper-algo w3-text-red','','avg','wfext.aper_algo',wfext.aper_algo,wfext.aper_algo_s,'waterfall_aper_algo_cb'),25,w3_div('id-wfext-aper-param',w3_slider('id-wfext-aper-param-slider','Parameter','wfext.aper_param',wfext.aper_param,0,10,1,'waterfall_aper_param_cb')),40,'&nbsp;',3,w3_div('id-wfext-aper-param-field'))),w3_div('w3-margin-LR-16 w3-margin-T-8',w3_text('','Min,max: total = computed + floor,ceil'),w3_inline('id-wfext-maxmin w3-background-fade w3-hide w3-text-white w3-small|background:#575757/',w3_div('id-wfext-content','&nbsp;')),w3_div('id-wfext-maxmin-spacer w3-small','(aperture manual mode selected)')),w3_div('w3-margin-T-8',w3_text('w3-margin-B-8 w3-text-css-orange','<b>Spec RF passband marker</b>'),w3_inline('w3-margin-LR-16/w3-hspace-16',w3_input('coloris-square//coloris coloris-instance1 coloris-input w3-input-evt||data-coloris','','wfext.spb_color',wfext.spb_color,'wfext_spb_color_cb'),w3_checkbox('w3-label-inline w3-label-not-bold','Enable','wfext.spb_on',wfext.spb_on,'wfext_spb_on_cb'))),w3_div('w3-margin-T-8',w3_text('w3-margin-B-8 w3-text-css-orange','<b>Timestamps</b>'),w3_inline('w3-margin-LR-16/w3-hspace-16',w3_select('id-wfext-tstamp '+wfext.sfmt,'','','wfext.tstamp_i',wfext.tstamp_i,wfext.tstamp_s,'wfext_tstamp_cb'),w3_input('id-wfext-tstamp-custom w3-ext-retain-input-focus|padding:0;width:auto|size=4','','wfext.tstamp_f',wfext.tstamp_f,'wfext_tstamp_custom_cb'),w3_select(wfext.sfmt,'','','wf.ts_tz',wf.ts_tz,wfext.tstamp_tz_s,'w3_num_cb'),w3_button('w3-padding-smaller w3-aqua','Save WF as JPG','export_waterfall'),)),w3_div('w3-margin-T-8 w3-margin-B-8',w3_text('w3-text-css-orange','<b>Waterfall FFT</b>'),w3_inline('w3-margin-LR-16/w3-hspace-16',w3_select(wfext.sfmt,'','window function','wfext.winf_i',wfext.winf_i,wfext.winf_s,'wfext_winf_cb'),w3_select(wfext.sfmt,'','interp','wfext.interp_i',wfext.interp_i,wfext.interp_s,'wfext_interp_cb'),w3_checkbox('w3-label-inline w3-label-not-bold','CIC<br>comp','wfext.cic_comp',wfext.cic_comp,'wfext_cic_comp_cb'))));w3_innerHTML('id-wf-more',controls_html);coloris_init();w3_show_hide('id-wfext-tstamp-custom',false,null,2);if(wf.aper==kiwi.APER_AUTO){w3_show_inline('id-wfext-maxmin');waterfall_maxmin_cb();w3_hide('id-wfext-maxmin-spacer');}}
function waterfall_init(){console.log('$$waterfall_init audioFFT_active='+wf.audioFFT_active);if(!wf.audioFFT_active){var init_aper=+ext_get_cfg_param('init.aperture',-1,EXT_NO_SAVE);var last_aper=kiwi_storeRead('last_aper',(init_aper==-1)?0:init_aper);if(isArg(wf_auto))last_aper=wf_auto?1:0;wf_aper_cb('wf.aper',last_aper,false);w3_show('id-aper-data');}else{wf_aper_cb('wf.aper',false,false);}
...
function waterfall_maxmin_cb(){w3_flash_fade('id-wfext-maxmin','cyan',50,300,'#575757');w3_innerHTML('id-wfext-content',mindb.toString().positiveWithSign()+','+maxdb.toString().positiveWithSign()+'&nbsp;=&nbsp;'+
...
/* web/kiwi/noise_blank.min.js (web/kiwi/noise_blank.js = Sun Mar 31 12:27:33 2024) */
...
var controls_html=w3_div('id-noise-blanker-controls w3-margin-right w3-text-white',w3_divs('/w3-tspace-8',w3_inline('w3-gap-16/',w3_div('w3-text-aqua','<b>Noise blanker</b>'),w3_select('w3-text-red||title="noise blanker selection"','','blanker','nb_algo',noise_blank.algo,noise_blank.menu_s,'nb_algo_cb','m'),w3_button('w3-padding-tiny w3-yellow','Defaults','noise_blank_load_defaults'),w3_button('id-noise-blanker-help-btn w3-right w3-green w3-small w3-padding-small','help','noise_blank_help')),w3_div('w3-margin-LR-16',s)));w3_innerHTML('id-nblank-more',controls_html);var test_html=w3_div('id-noise-blanker-controls w3-text-white',w3_divs('w3-margin-B-8/w3-tspace-8 w3-margin-right',w3_inline('',w3_div('w3-text-aqua','<b>Noise blanker test</b>')),w3_div('w3-margin-LR-16',w3_select('w3-margin-B-8 w3-text-red','','test mode','noise_blank.test',noise_blank.test,noise_blank.test_s,'noise_blank_test_cb'),w3_slider('','Test pulse gain','noise_blank.test_gain',noise_blank.test_gain,-90,0,1,'noise_blank_test_gain_cb'),w3_slider('','Test pulse width','noise_blank.test_width',noise_blank.test_width,1,30,1,'noise_blank_test_width_cb'))));w3_innerHTML('id-ntest-more',test_html);}
...
/* web/kiwi/noise_filter.min.js (web/kiwi/noise_filter.js = Sun Mar 31 12:28:08 2024) */
...
var controls_html=w3_div('id-noise-filter-controls w3-margin-right w3-text-white',w3_divs('/w3-tspace-8',w3_inline('w3-gap-16/',w3_div('w3-text-aqua','<b>Noise filter</b>'),w3_select('w3-text-red||title="noise filter selection"','','filter','nr_algo',noise_filter.algo,noise_filter.menu_s,'nr_algo_cb','m'),w3_button('w3-padding-tiny w3-yellow','Defaults','noise_filter_load_defaults'),w3_button('id-noise-filter-help-btn w3-right w3-green w3-small w3-padding-small','help','noise_filter_help')),w3_div('w3-margin-LR-16',s)));return controls_html;}
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Sun Mar 31 12:09:54 2024) */
var ant_sw={ver:'',n_ant:8,backend:-1,backend_s:null,backends_s:null,ip_or_url:null,exantennas:0,focus:false,isConfigured:false,running:false,buttons_locked:false,buttons_selected:[],denymixing:0,thunderstorm:0,notify_timeout:null,url_ant:null,url_deselected:false,url_idx:0,desc_lc:[],last_offset:-1,last_high_side:-1,EVERYONE:0,LOCAL_CONN:1,LOCAL_OR_PWD:2,denyswitching:0,deny_s:['everyone','local connections only','local connections or user password only'],DENY_NONE:0,DENY_SWITCHING:1,DENY_MULTIUSER:2,denied_permission:false,denied_because_multiuser:false};function ant_switch_view(){console.log('ant_switch_view ant_sw.focus='+ant_sw.focus);if(!ant_sw.focus)return;keyboard_shortcut_nav('rf');var Hrf;w3_do_when_cond(function(){Hrf=w3_el('id-optbar-rf').clientHeight;return(Hrf!=0);},function(){var Hopt=kiwi.OPTBAR_CONTENT_HEIGHT;var Hant=w3_el('id-optbar-rf-antsw').clientHeight+8;pct=w3_clamp(kiwi_round(1-(Hant-Hopt)/(Hrf-Hopt),2),0,1);w3_scrollTo('id-optbar-content',pct);},null,200);var p=ext_param();if(isNonEmptyString(p)){console.log('ant_switch_view: EXT param = <'+p+'>');ant_sw.url_ant=p.split(',');ant_switch_prompt_query();}}
function ant_switch_msg(param){var rv=true;console.log('ant_switch_msg '+param.join('='));switch(param[0]){case"antsw_backend_ver":var ver=param[1].split('.');if(ver.length==2&&(ver[0]||ver[1])){ant_sw.ver=param[1];console.log('ant_switch: backend v'+ant_sw.ver);}
...
console.log('ant_switch: Antenna configuration ant_sw.n_ant='+ant_sw.n_ant);var buttons_html='';var n_ant=0;var backend=(cfg.ant_switch&&isNonEmptyString(cfg.ant_switch.backend))?cfg.ant_switch.backend:'';if(backend!=''){for(i=1;i<=ant_sw.n_ant;i++){var s=antdesc[i]?antdesc[i]:'';if(isNonEmptyString(s)){var locked=ant_sw.buttons_locked?' w3-disabled':'';var highlight=ant_sw.buttons_selected[i]?' w3-selection-green':'';buttons_html+=w3_div('w3-valign w3-margin-T-8',w3_button('id-antsw-btn w3-padding-smaller w3-round-6px'+locked+highlight,'Antenna '+i,'ant_switch_select_antenna_cb',i),w3_div('w3-margin-L-8',s));n_ant++;}
ant_sw.desc_lc[i]=s.toLowerCase();if(!ant_sw.isConfigured){if(isNonEmptyString(s))
ant_sw.isConfigured=true;}}}
w3_innerHTML('id-antsw-user',buttons_html);ant_switch_view();console.log('ant_switch_buttons_setup DONE');}
function ant_switch_user_refresh(){if(!ant_sw.running)return;console.log('ant_switch_user_refresh');ant_switch_prompt_query();}
function ant_switch_user_init(){ant_sw.denyswitching=ext_get_cfg_param('ant_switch.denyswitching',ant_sw.EVERYONE,EXT_NO_SAVE);ant_sw.denymixing=ext_get_cfg_param('ant_switch.denymixing','',EXT_NO_SAVE)?1:0;ant_sw.thunderstorm=ext_get_cfg_param('ant_switch.thunderstorm','',EXT_NO_SAVE)?1:0;var controls_html=w3_div('id-antsw-controls w3-margin-right w3-text-white',w3_inline('w3-gap-12/',w3_div('w3-text-aqua','<b>Antenna switch</b>'),w3_div('w3-text-white','by Kari Karvonen'),w3_button('id-antsw-help-btn w3-right w3-green w3-small w3-padding-small||onclick="ant_switch_help()"','help')),w3_div('w3-margin-LR-16',w3_div('id-antsw-display-selected w3-margin-T-4'),w3_div('id-antsw-display-permissions'),w3_div('id-antsw-user w3-margin-B-8')));w3_innerHTML('id-optbar-rf-antsw',controls_html);ant_sw.running=true;console.log('SET antsw_init');ext_send('SET antsw_init');ant_switch_buttons_setup();ant_switch_prompt_query();var p=kiwi_url_param('ant');console.log('ant_switch_user_init p='+p);if(isArg(p)){if(isNonEmptyString(p)){console.log('ant_switch_user_init: URL param = <'+p+'>');ant_sw.url_ant=p.split(',');}
...
function ant_switch_select_antenna(ant){console.log('ant_switch: switching to antenna '+ant);ext_send('SET antsw_SetAntenna='+ant);setTimeout(function(){ant_switch_prompt_query();},1000);}
function ant_switch_select_antenna_cb(path,val){ant_switch_select_antenna(val);}
function ant_switch_prompt_query(){console.log('ant_switch_prompt_query');ext_send('SET antsw_GetAntenna');}
function ant_switch_process_reply(ant_selected_antenna){var need_to_inform=false;ant_switch_buttons_setup();ant_sw.denyswitching=ext_get_cfg_param('ant_switch.denyswitching',ant_sw.EVERYONE,EXT_NO_SAVE);if(!ant_sw.isConfigured){ant_switch_display_update('Antenna switch is not configured.');return;}
if(ant_sw.exantennas!=ant_selected_antenna){need_to_inform=true;ant_sw.exantennas=ant_selected_antenna;}
if(ant_selected_antenna=='g'){if(need_to_inform)console.log('ant_switch: all antennas grounded');ant_switch_display_update('All antennas are grounded.');}else{if(need_to_inform)console.log('ant_switch: antenna '+ant_selected_antenna+' in use');ant_switch_display_update('Selected antennas are now: '+ant_selected_antenna);}
var selected_antennas_list=ant_selected_antenna.split(',');var re=/^Antenna ([1]?[0-9]+)/i;w3_els('id-antsw-btn',function(el,i){if(!el.textContent.match(re))return;w3_unhighlight(el);var antN=el.textContent.parseIntEnd();ant_sw.buttons_selected[antN]=false;if(!isArray(selected_antennas_list))return;if(selected_antennas_list.indexOf(antN.toString())<0)return;ant_sw.buttons_selected[antN]=true;w3_highlight(el);if(ant_sw.denymixing&&selected_antennas_list.length==1){var s='ant_switch.ant'+antN+'offset';var offset=ext_get_cfg_param(s,'',EXT_NO_SAVE);offset=+offset;if(!isNumber(offset))offset=0;if(offset!=ant_sw.last_offset){ext_send('SET antsw_freq_offset='+offset);ant_sw.last_offset=offset;}
var s='ant_switch.ant'+antN+'high_side';var high_side=ext_get_cfg_param(s,'',EXT_NO_SAVE);if(high_side!=ant_sw.last_high_side){ext_send('SET antsw_high_side='+(high_side?1:0));ant_sw.last_high_side=high_side;}}});if(ant_sw.url_ant&&ant_sw.url_ant.length>0){console.log('ant_switch: url_deselected='+ant_sw.url_deselected);if(ant_sw.url_deselected==false){ant_switch_select_antenna('g');ant_sw.url_deselected=true;}else{console.log('ant_switch: URL url_idx='+ant_sw.url_idx+' denymixing='+ant_sw.denymixing);if(ant_sw.url_idx==0||!ant_sw.denymixing){var ant=decodeURIComponent(ant_sw.url_ant.shift());console.log('ant_switch: URL ant = <'+ant+'>');if(ant=='help'){ant_switch_help();}else{var n=parseInt(ant);if(!(!isNaN(n)&&n>=1&&n<=ant_sw.n_ant)){if(ant==''){n=0;}else{ant=ant.toLowerCase();for(n=1;n<=ant_sw.n_ant;n++){if(ant_sw.desc_lc[n].indexOf(ant)!=-1)
break;}}}
...
var re=/^Ground all$/i;if(el.textContent.match(re)){w3_disable(el,lock);}});}
function ant_switch_showpermissions(){var s,lock=true;if(!ant_sw.isConfigured){s='';}else
...
function ant_switch_admin_msg(param){console.log('ant_switch_admin_msg '+param.join('='));switch(param[0]){case"antsw_backends":ant_sw.backends_s=decodeURIComponent(param[1]).split(' ');console.log('antsw_backends='+ant_sw.backends_s);w3_innerHTML('id-antsw-backends',w3_select('w3-width-auto w3-label-inline','Switch device:','select','ant_sw.backend',ant_sw.backend,ant_sw.backends_s,'ant_switch_backend_cb'));return true;case"antsw_backend":console.log('antsw_backend='+param[1]);ant_sw.backend_s=param[1];if(isNonEmptyString(ant_sw.backend_s))
w3_select_set_if_includes('ant_sw.backend',ant_sw.backend_s);ant_switch_users_notify_change();return true;case"antsw_ver":console.log('antsw_ver='+param[1]);ant_sw.ver=param[1];var s='backend script v'+ant_sw.ver;if(ant_sw.ver=='0.0')s='';w3_innerHTML('id-antsw-ver',s);return true;case"antsw_nch":console.log('antsw_nch='+param[1]);ant_switch_config_html2(+param[1]);return true;case"antsw_ip_or_url":console.log('antsw_ip_or_url='+param[1]);var ip_url=param[1];if(ip_url=='(null)')ip_url='';w3_set_value('id-ant_sw.ip_or_url',ip_url);ant_sw.ip_or_url=ip_url;return true;}
return false;}
function ant_switch_backend_cb(path,val,first){console.log('ant_switch_backend_cb path='+path+' val='+val+' first='+first);ant_sw.backend_s=ant_sw.backends_s[+val];ext_set_cfg_param('ant_switch.backend',ant_sw.backend_s,EXT_SAVE);console.log('ADM antsw_SetBackend='+ant_sw.backend_s);ext_send('ADM antsw_SetBackend='+ant_sw.backend_s);ext_send('ADM antsw_GetInfo');}
function ant_switch_users_notify_change(){console.log('$ant_switch_users_notify_change');kiwi_clearTimeout(ant_sw.notify_timeout);ant_sw.notify_timeout=setTimeout(function(){console.log('$antsw_notify_users');ext_send('ADM antsw_notify_users');},1000);}
function ant_switch_desc_cb(path,val,first){w3_string_set_cfg_cb(path,val,first);ant_switch_users_notify_change();}
function ant_switch_set_ip_or_url_cb(path,val,first){console.log('ant_switch_set_ip_or_url_cb: path='+path+' val='+val+' first='+first);if(first)return;ant_sw.ip_or_url=val;w3_set_value('id-ant_sw.ip_or_url',ant_sw.ip_or_url);console.log('ADM antsw_SetIP_or_URL='+ant_sw.ip_or_url);ext_send('ADM antsw_SetIP_or_URL='+ant_sw.ip_or_url);}
function ant_denyswitch_cb(path,val,first){console.log('ant_denyswitch_cb path='+path+' val='+val+'('+w3_switch_s(val)+') first='+first);w3_int_set_cfg_cb(path,val,first);ant_switch_users_notify_change();}
function ant_switch_cb(path,val,first){console.log('ant_switch_cb path='+path+' val='+val+'('+w3_switch_s(val)+') first='+first);admin_radio_YN_cb(path,val);ant_switch_users_notify_change();}
function ant_switch_config_html2(n_ch){if(n_ch)ant_sw.n_ant=n_ch;console.log('ant_switch_config_html2 n_ch='+n_ch);var s='';for(var i=1;i<=ant_sw.n_ant;i++){s+=w3_inline_percent('w3-margin-T-16 w3-valign-center/',w3_input_get('w3-defer','Antenna '+i+' description','ant_switch.ant'+i+'desc','ant_switch_desc_cb',''),50,'&nbsp;',3,w3_checkbox_get_param('w3-defer//w3-label-inline','Default<br>antenna','ant_switch.ant'+i+'default','admin_bool_cb',false),7,'&nbsp;',3,w3_checkbox_get_param('w3-defer w3-restart//w3-label-inline','High-side<br>injection','ant_switch.ant'+i+'high_side','admin_bool_cb',false),10,'&nbsp;',1,w3_input_get('w3-defer w3-restart//','Frequency scale offset (kHz)','ant_switch.ant'+i+'offset','w3_int_set_cfg_cb',0));}
...
w3_div('',w3_inline('w3-gap-32/',w3_div('id-antsw-backends'),w3_div('id-antsw-ver'),w3_input('/w3-label-inline w3-label-not-bold w3-nowrap/w3-padding-small||size=40','IP address or URL:','ant_sw.ip_or_url',ant_sw.ip_or_url,'ant_switch_set_ip_or_url_cb')),w3_div('','<br>If antenna switching is denied then users cannot switch antennas. <br>'+'Admin can always switch antennas, either from a user connection on the local network, or from the <br>'+'admin console tab using the script: <x1>/root/Beagle_SDR_GPS/pkgs/ant_switch/ant-switch-frontend</x1> <br><br>'+'The last menu option allows anyone connecting using a password to switch antennas <br>'+'(i.e. time limit exemption password on the admin page control tab, not the user login password). <br>'+'Other connections made without passwords are denied.'),w3_select('w3-width-auto w3-label-inline w3-margin-T-8|color:red','Allow antenna switching by:','','ant_switch.denyswitching',ant_sw.denyswitching,ant_sw.deny_s,'ant_denyswitch_cb'),w3_div('w3-margin-T-16','If antenna mixing is denied then users can select only one antenna at time.'),w3_switch_label_get_param('w3-defer/w3-label-inline w3-label-left w3-margin-T-8','Deny antenna mixing?','Yes','No','ant_switch.denymixing',true,false,'ant_switch_cb'),w3_div('w3-margin-T-16','If multiuser is denied then antenna switching is disabled when more than one user is online.'),w3_switch_label_get_param('w3-defer/w3-label-inline w3-label-left w3-margin-T-8','Deny multiuser switching?','Yes','No','ant_switch.denymultiuser',true,false,'ant_switch_cb'),w3_div('w3-margin-T-16','If thunderstorm mode is activated, all antennas and forced to ground and switching is disabled.'),w3_switch_label_get_param('w3-defer/w3-label-inline w3-label-left w3-margin-T-8','Enable thunderstorm mode?','Yes','No','ant_switch.thunderstorm',true,false,'ant_switch_cb'),w3_div('w3-margin-T-16','The <x1>Default antenna</x1> checkboxes below define which antenna(s) are selected initially. <br>'+'And also when no users are connected if the switch below is set to <x1>Yes</x1>. <br>'+'"Users" includes all connections including FT8/WSPR autorun and kiwirecorder (e.g. wsprdaemon).'),w3_switch_label_get_param('w3-defer/w3-label-inline w3-label-left w3-margin-T-8','Switch to default antenna(s) when no users connected?','Yes','No','ant_switch.default_when_no_users',true,true,'admin_radio_YN_cb'),w3_div('','<hr><b>Antenna buttons configuration</b><br>'),w3_col_percent('w3-margin-T-16/','Leave antenna description field empty if you want to hide antenna button from users. <br>'+'For two-line descriptions use break sequence &lt;br&gt; between lines.',74,'Overrides frequency scale offset value on<br>'+'config tab when any antenna selected. <br>'+'No effect if antenna mixing enabled.'),w3_div('',w3_div('id-antsw-list'),w3_col_percent('w3-margin-T-16/',w3_input_get('w3-defer','Antenna switch failure or unknown status decription','ant_switch.ant0desc','w3_string_set_cfg_cb',''),70)))));ext_send('ADM antsw_GetBackends');ext_send('ADM antsw_GetInfo');}





/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Wed Apr  3 13:47:56 2024) */
var owrx={debug_drag:false,height_top_bar_parts:67,hide_bars:0,HIDE_TOPBAR:1,HIDE_BANDBAR:2,HIDE_ALLBARS:3,PB_NO_EDGE:0,PB_RIGHT_EDGE:1,PB_LEFT_EDGE:2,allow_pb_adj:false,cfg_loaded:false,mobile:null,wf_snap:0,wf_cursor:'crosshair',last_shiftKey:false,last_pageX:0,last_pageY:0,mouse_freq:{},override_fmem:null,vfo_ab:0,vfo_first:true,optbar_last_scrollpos:[],rec_init:false,last_freq:-1,last_mode:'',last_locut:-1,last_hicut:-1,last_lo:[],last_hi:[],last_selected_band:0,last_mode_el:null,dseq:0,sMeter_dBm_biased:0,sMeter_dBm:0,smeter_interval:null,force_no_agc_thresh_smeter:false,overload_mute:90,squelched_overload:false,prev_squelched_overload:false,touch_hold_pressed:false,show_cursor_freq:0,tuning_locked:0,capture_input_field:false,dx_click_gid_last_stored:undefined,dx_click_gid_last_until_tune:undefined,sam_pll:1,sam_pll_s:['DX','med','fast'],chan_null:0,chan_null_s:['normal','null LSB','null USB'],SAM_opts:3,SAM_opts_s:['none','fade leveler','DC block','fade+block'],SAM_opts_sft:2,ovld_mute:0,ovld_mute_s:['off','on'],FSET_NOP:0,FSET_ADD:1,FSET_SET_FREQ:2,FSET_EXT_SET_PB:3,FSET_PB_CHANGE:4,FSET_SCALE_DRAG:5,FSET_SCALE_DRAG_END:6,FSET_SCALE_MOUSE_CLICK:7,FSET_SCALE_TOUCH_DRAG:8,FSET_SCALE_TOUCH_DRAG_END:9,fset_s:['nop','add','fset','extSetPB','pbChg','drag','dragEnd','mouseClick','touchDrag','touchClick'],WF_POS_AT_FREQ:0,WF_POS_RECENTER_IF_OUTSIDE:1,waterfall_tuned:0,scale_canvas:{mouse_out:false,target:null,restore_visibility:[],restore_visibility_delay:500},canvas:{target:null,restore_visibility:[],restore_visibility_delay:500},zoom_titles:[['passband narrow','passband widen'],['passband right edge','passband right edge'],['passband left edge','passband left edge']],__last:0};var bandwidth;var center_freq;var wf_fft_size;var cur_mode;var wf_fps,wf_fps_max;var ws_snd,ws_wf;var spectrum_show=0,spectrum_param=-1;var gen_freq=0,gen_attn=0;var squelch_threshold=0;var wf_rate='';var wf_mm='';var wf_auto=null;var wf_compression=1;var wf_interp=13;var wf_winf=2;var snd_winf=0;var debug_v=0;var sb_trace=0;var kiwi_gc=1;var kiwi_gc_snd=0;var kiwi_gc_wf=-1;var kiwi_gc_recv=-1;var kiwi_gc_wspr=-1;var override_ext=null;var muted_initially=0;var peak_initially1=null,peak_initially2=null;var param_nocache=false;var nocache=false;var param_ctrace=false;var ctrace=false;var no_clk_corr=false;var override_pbw='';var override_pbc='';var nb_click=false;var no_geoloc=false;var force_mobile=false;var mobile_laptop_test=false;var user_url=null;var override_1Hz=null;var wf_rates={'0':0,'off':0,'1':1,'1hz':1,'s':2,'slow':2,'m':3,'med':3,'f':4,'fast':4};var okay_wf_init=false;function kiwi_main(){w3_do_when_cond(function(){return owrx.cfg_loaded;},function(){kiwi_main_ready();},null,200);}
...
var host=window.location.href.split('?')[0];g_qs_idx=2;for(w=2;w<=num_win;w++){if(qs[w]){setTimeout(function(){var url=host+'?'+qs[g_qs_idx];g_qs_idx++;var win=kiwi_open_or_reload_page({url:url,tab:1});if(win==null)
...
pb_adj_cf.style.left=px(from_px+env_adj);pb_adj_cf.style.width=px(Math.max(0,to_px-from_px-2*env_adj));pb_adj_lo.style.left=px(from_px-env_slope-env_adj);pb_adj_lo.style.width=px(env_slope+2*env_adj);pb_adj_hi.style.left=px(to_px-env_adj);pb_adj_hi.style.width=px(env_slope+2*env_adj);from_px-=(env_att_w+env_bounding_line_w);to_px+=(env_att_w+env_bounding_line_w);scale_ctx.lineWidth=3;scale_ctx.strokeStyle=color;scale_ctx.fillStyle=color;var drag_ranges={envelope_on_screen:false,line_on_screen:false,allow_pb_adj:false};owrx.allow_pb_adj=false;if(!(to_px<0||from_px>window.innerWidth)){drag_ranges.beginning={x1:from_px-env_adj,x2:from_px+env_slope+env_adj};drag_ranges.ending={x1:to_px-env_slope-env_adj,x2:to_px+env_adj};drag_ranges.whole_envelope={x1:from_px,x2:to_px};owrx.pbx1=from_px;owrx.pbx2=to_px;drag_ranges.envelope_on_screen=true;drag_ranges.allow_pb_adj=owrx.allow_pb_adj=((to_px-from_px)>=50);if(!drag_ranges.allow_pb_adj)
scale_ctx.strokeStyle=scale_ctx.fillStyle='yellow';scale_ctx.beginPath();scale_ctx.moveTo(from_px,env_h1);scale_ctx.lineTo(from_px+env_bounding_line_w,env_h1);scale_ctx.lineTo(from_px+env_slope,env_h2);scale_ctx.lineTo(to_px-env_bounding_line_w-env_att_w,env_h2);scale_ctx.lineTo(to_px-env_bounding_line_w,env_h1);scale_ctx.lineTo(to_px,env_h1);scale_ctx.globalAlpha=0.3;scale_ctx.fill();scale_ctx.globalAlpha=1;scale_ctx.stroke();}else{owrx.pbx1=owrx.pbx2=0;}
...
function owrx_init_cfg(){w3_do_when_rendered('id-rx-gmap',function(){var rx_gmap=w3_json_to_string('RX_GMAP',cfg.index_html_params.RX_GMAP);w3_link_set('id-rx-gmap','https://www.google.com/maps/place/'+rx_gmap);});owrx.cfg_loaded=true;}
...
event_handled|=demodulators[i].envelope.drag_move(x);if(!event_handled){demodulator_set_offset_frequency(owrx.FSET_SCALE_DRAG,scale_offset_carfreq_from_px(x));}
...
setTimeout(keyboard_shortcut_url_keys,5000);wf_snap(kiwi_storeInit('wf_snap'));if(kiwi_isMobile())
...
var len=pwr_dBm.length;if(len){pwr_dBm.sort(kiwi_sort_numeric);var noise=pwr_dBm[Math.floor(0.50*len)];var signal=pwr_dBm[Math.floor(0.95*len)];var _10=pwr_dBm[Math.floor(0.10*len)];var _20=pwr_dBm[Math.floor(0.20*len)];}else{signal=-110;noise=-120;}
...
var afft={init:false,comp_1x:false,prev_lsb:false,audioFFT_dynload:{},offt:0,i_re:0,i_im:0,o_re:0,o_im:0,window_512:[],window_1k:[],window_2k:[],pwr_dB:[],dBi:[],CUTESDR_MAX_VAL:32767,};function audioFFT_setup(){if(!wf.audioFFT_active)return;zoom_level=0;if(wf.aper!=kiwi.APER_AUTO){var last_AF_max_dB=kiwi_storeRead('last_AF_max_dB',maxdb);var last_AF_min_dB=kiwi_storeRead('last_AF_min_dB',mindb_un);setmaxdb(1,last_AF_max_dB);setmindb(1,last_AF_min_dB);update_maxmindb_sliders();}
...
if(!no_clear_last_gid)owrx.dx_click_gid_last_until_tune=undefined;dx.last_stepped_gid=-1;if(isArg(mode)&&(mode!=cur_mode||open_ext==true)){ext_set_mode(mode,fdsp_Hz,opt);}else{freq_car_Hz=freq_dsp_to_car(fdsp_Hz);if(!no_set_freq){demodulator_set_offset_frequency(owrx.FSET_SET_FREQ,freq_car_Hz-center_freq);}}}
...
function freqset_update_ui(from){freq_displayed_Hz=freq_car_to_dsp(freq_car_Hz);var f_kHz=freq_displayed_Hz/1000;freq_displayed_kHz_str=f_kHz.toFixed(freq_field_prec(f_kHz));if(!waterfall_setup_done)return;if(freqset_restore_ui()==null)return;if(from==owrx.FSET_SET_FREQ&&zoom_center!=0.5){zoom_step(ext_zoom.CUR);}else{waterfall_position(owrx.WF_POS_RECENTER_IF_OUTSIDE);}
kiwi_storeWrite('last_freq',freq_displayed_kHz_str_with_freq_offset);freq_dsp_set_last=freq_displayed_kHz_str_with_freq_offset;freq_step_update_ui();freq_link_update();vfo_update();if(kiwi.fmem_auto_save&&from!=owrx.FSET_NOP&&from!=owrx.FSET_SCALE_DRAG&&from!=owrx.FSET_SCALE_TOUCH_DRAG&&from!=owrx.FSET_PB_CHANGE){freq_memory_add(freq_displayed_kHz_str_with_freq_offset);}
...
function freqset_select(){if(owrx.capture_input_field)return;var ae=document.activeElement;var closed_ext_input_still_holding_focus=(ae&&w3_contains(ae,'w3-ext-retain-input-focus')&&!extint.displayed);if(closed_ext_input_still_holding_focus){}
...
audioFFT_update();wf.audioFFT_prev_mode=cur_mode;}else{mkenvelopes(get_visible_freq_range());}}else{setTimeout(try_freqset_update_ui,1000,from);}}
...
var ident=kiwi_storeInit('ident','');ident=kiwi_strip_tags(ident,'').substring(0,len);ident=kiwi_strip_tags(ident,'').substring(0,len);var el1=w3_el('id-ident-input1');var el2=w3_el('id-ident-input2');w3_attribute(el1,'maxlength',len);w3_attribute(el2,'maxlength',len);el1.value=ident;el2.value=ident;ident_user=ident;send_ident=true;}
function ident_complete(from,which){var i,el=[];for(i=1;i<=3;i++){el[i]=w3_el('id-ident-input'+i);}
var ident=el[which].value;var len=Math.max(cfg.ident_len,kiwi.ident_min);ident=kiwi_strip_tags(ident,'').substring(0,len);kiwi_clearTimeout(ident_tout);if(which!=3||(which==3&&ident!='')){for(i=1;i<=3;i++){if(el[i]){el[i].value=ident;w3_schedule_highlight(el[i]);}}}
freqset_select();kiwi_storeWrite('ident',ident);ident_user=ident;send_ident=true;if(which==3&&ident!='')
...
if(key_mod==shortcut.SHIFT)edge=owrx.PB_RIGHT_EDGE;passband_increment(key=='ArrowUp',edge);break;case'v':shortcut_setvolume(-5);break;case'V':shortcut_setvolume(+5);break;case' ':toggle_or_set_mute();break;case'g':case'=':freqset_select();break;case'm':freq_memory_menu_open(true);break;case'b':band_scroll(1);break;case'B':band_scroll(-1);break;case'n':freq_vfo_cb();break;case'N':vfo_update(true);break;case'<':page_scroll(-page_scroll_amount);break;case'>':page_scroll(+page_scroll_amount);break;case'z':zoom_step(ctlAlt?ext_zoom.NOM_IN:ext_zoom.IN);break;case'Z':zoom_step(ctlAlt?ext_zoom.MAX_OUT:ext_zoom.OUT);break;case'w':incr_mindb(1,ctlAlt?-10:-1);break;case'W':incr_mindb(1,ctlAlt?+10:+1);break;case's':toggle_or_set_spec(null,null,dir);keyboard_shortcut_nav('wf');break;case'S':wf_autoscale_cb();keyboard_shortcut_nav('wf');break;case'D':toggle_or_set_slow_dev();keyboard_shortcut_nav('wf');break;case'!':keyboard_shortcut_nav('wf');wf_aper_cb('wf.aper',wf.aper^1,false);break;case'o':var nav=['off','users','status'][shortcut.nav_off];keyboard_shortcut_nav(nav);shortcut.nav_off=(shortcut.nav_off+1)%3;break;case'r':toggle_or_set_rec();break;case'R':keyboard_shortcut_nav('rf');break;case'$':owrx.freq_dsp_1Hz^=1;kiwi_storeWrite('freq_dsp_1Hz',owrx.freq_dsp_1Hz);freqset_update_ui(owrx.FSET_NOP);break;case'%':owrx.tuning_locked^=1;freqset_update_ui(owrx.FSET_NOP);break;case'x':toggle_or_set_hide_panels();break;case'y':toggle_or_set_hide_bars();break;case'@':dx_filter(key_mod==shortcut.SHIFT_PLUS_CTL_ALT);break;case'e':extension_scroll(1);break;case'E':extension_scroll(-1);break;case'?':case'h':keyboard_shortcut_help();break;case'H':if(extint.current_ext_name)
...
function test_audio_suspended(){var s1,onclick=null,input=false,ident=kiwi_storeInit('ident','');if(cfg.require_id&&isEmptyString(ident)){s1=w3_input('w3-flex w3-flex-col w3-valign-center//w3-custom-events w3-margin-T-10 w3-font-18px w3-normal'+'|padding:1px; width:300px|size=20'+' onchange="ident_complete(\'el\', 3)" onkeyup="ident_keyup(this, event, 3)"','Enter your name or callsign <br> to start KiwiSDR','ident-input3');onclick='';input=true;}else
if(ac_play_button.state!="running"){s1=(kiwi_isMobile()?'Tap to':'Click to')+' start KiwiSDR';onclick='onclick="play_button_click_cb()"';}
if(onclick!=null){var s2=w3_div('id-play-button-container class-overlay-container w3-valign w3-halign-center||'+onclick,w3_inline('id-play-button w3-relative w3-flex-col/','<img src="gfx/openwebrx-play-button.png" width="150" height="150" /><br><br>',s1));w3_create_appendElement('id-kiwi-body','div',s2);if(input){var el=w3_el('id-ident-input3');w3_attribute(el,'maxlength',Math.max(cfg.ident_len,kiwi.ident_min));owrx.capture_input_field=true;w3_field_select(el,{mobile:1});}
...
w3_opacity('id-play-button-container',0);setTimeout(function(){w3_hide('id-play-button-container');},1100);audio_reset();owrx.capture_input_field=false;freqset_select();}
function panels_setup(){var el;var s;el=w3_el("id-ident");el.innerHTML=w3_input('w3-label-not-bold/w3-custom-events|padding:1px|size=20'+' onchange="ident_complete(\'el\', 1)" onkeyup="ident_keyup(this, event, 1)"','Your name or callsign:','ident-input1');el.addEventListener('click',function(evt){return cancelEvent(evt);},true);var mobile=kiwi_isMobile()?(' inputmode='+dq(kiwi_is_iOS()?'decimal':'tel')+' ontouchstart="popup_keyboard_touchstart(event)" onclick="this.select()"'):'';w3_el("id-control-freq1").innerHTML=w3_inline('w3-halign-space-between/',w3_div('id-freq-cell','<form id="id-freq-form" name="form_freq" action="#" onsubmit="freqset_complete(0, event); return false;">'+
...
button_9_10(step_9_10);s='';mode_buttons.forEach(function(mba,i){var ms=mba.s[0];var mslc=ms.toLowerCase();var disabled=mba.dis||(mslc=='drm'&&!kiwi.DRM_enable);var attr=disabled?'':' onclick="mode_button(event, this)"';attr+=' onmouseover="mode_over(event, this)"';s+=w3_div('id-button-'+mslc+' id-mode-'+ext_mode(mslc).str+' class-button'+(disabled?' class-button-disabled':'')+' ||id="'+i+'-id-mode-col"'+attr+' onmousedown="cancelEvent(event)"',ms);});w3_el("id-control-mode").innerHTML=w3_inline('w3-halign-space-between/',s);var afd=wf.audioFFT_active?' w3-disabled||onclick="freqset_select()" disabled=""':'';w3_el("id-control-zoom").innerHTML=w3_inline('w3-halign-space-between/',w3_div('id-zoom-in class-icon'+afd+'||onclick="zoom_click(event,1)" onmouseover="zoom_over(event)" title="zoom in"','<img src="icons/zoomin.png" width="32" height="32" />'),w3_div('id-zoom-out class-icon'+afd+'||onclick="zoom_click(event,-1)" onmouseover="zoom_over(event)" title="zoom out"','<img src="icons/zoomout.png" width="32" height="32" />'),w3_div('id-maxin'+afd,w3_div('class-icon||onclick="zoom_click(event,8)" title="max in"','<img src="icons/maxin.png" width="32" height="32" />')),w3_div('id-maxin-nom w3-hide'+afd,w3_div('class-icon||onclick="zoom_click(event,8)" title="max in"','<img src="icons/maxin.nom.png" width="32" height="32" />')),w3_div('id-maxin-max w3-hide'+afd,w3_div('class-icon||onclick="zoom_click(event,8)" title="max in"','<img src="icons/maxin.max.png" width="32" height="32" />')),w3_div('id-maxout'+afd,w3_div('class-icon||onclick="zoom_click(event,-9)" title="max out"','<img src="icons/maxout.png" width="32" height="32" />')),w3_div('id-maxout-max w3-hide'+afd,w3_div('class-icon||onclick="zoom_click(event,-9)" title="max out"','<img src="icons/maxout.max.png" width="32" height="32" />')),w3_div('class-icon'+afd+'||onclick="zoom_click(event,0)" title="zoom to band"','<img src="icons/zoomband.png" width="32" height="16" style="padding-top:13px; padding-bottom:13px;"/>'),w3_div('class-icon'+afd+'||onclick="page_scroll_icon_click(event,'+ -page_scroll_amount+')" title="page down\nalt: label step"','<img src="icons/pageleft.png" width="32" height="32" />'),w3_div('class-icon'+afd+'||onclick="page_scroll_icon_click(event,'+page_scroll_amount+')" title="page up\nalt: label step"','<img src="icons/pageright.png" width="32" height="32" />'));var optbar_colors=['w3-green','w3-pink','w3-blue','w3-purple','w3-aqua','w3-yellow','w3-black','w3-red','w3-amber','w3-green','w3-orange','w3-lime','w3-indigo','w3-brown','w3-teal'];var ci=0;var psa1=' w3-center|width:12.8%';var psa2=psa1+';margin-right:6px';w3_el('id-optbar').innerHTML=w3_navbar('id-navbar-optbar cl-optbar',w3_nav(optbar_colors[ci++]+psa2,'RF','id-navbar-optbar','optbar-rf','optbar'),w3_nav(optbar_colors[ci++]+psa2,'WF','id-navbar-optbar','optbar-wf','optbar'),w3_nav(optbar_colors[ci++]+psa2,'Audio','id-navbar-optbar','optbar-audio','optbar'),w3_nav(optbar_colors[ci++]+psa2,'AGC','id-navbar-optbar','optbar-agc','optbar'),w3_nav(optbar_colors[ci++]+psa2,'User','id-navbar-optbar','optbar-users','optbar'),w3_nav(optbar_colors[ci++]+psa2,'Stat','id-navbar-optbar','optbar-status','optbar'),w3_navdef(optbar_colors[ci++]+psa1,'Off','id-navbar-optbar','optbar-off','optbar'));wf_filter=kiwi_storeRead('last_wf_filter',wf_sp_menu_e.OFF);spec_filter=kiwi_storeRead('last_spec_filter',wf_sp_menu_e.IIR);if(isString(spectrum_show)){var ss=spectrum_show.toLowerCase();wf_sp_menu_s.forEach(function(e,i){if(ss==e.toLowerCase())spec_filter=i;});}
...
w3_col_percent('w3-valign/class-slider',w3_text('w3-text-css-orange','WF floor'),19,w3_slider('id-input-floordb w3-wheel-shift','','',wf.auto_floor.val,-30,30,1,'setfloordb_cb'),60,w3_div('id-field-floordb class-slider'),19);var fmt='id-rf-attn-disable w3-btn w3-padding-tiny w3-margin-R-8 ';kiwi.rf_attn=(kiwi.model==kiwi.KiwiSDR_1)?0:+kiwi_storeInit('last_rf_attn',0);w3_el("id-optbar-rf").innerHTML=w3_div('',w3_col_percent('w3-valign/class-slider',w3_text('w3-text-css-orange','RF attn'),19,w3_slider('id-rf-attn id-rf-attn-disable w3-wheel-shift','','',kiwi.rf_attn,0,31.5,0.5,'rf_attn_cb'),60,w3_div('id-field-rf-attn class-slider'),19)+
...
var param=wf_sp_which?wf_param:sp_param;var nval=w3_slider_wheel('wf_sp_slider_wheel_cb',el,param,+el.step,+el.step*5);if(isNumber(nval))wf_sp_slider_cb('',nval,1);}
...
freqset_select();if(write_cookies){kiwi_storeWrite(wf.audioFFT_active?'last_AF_max_dB':'last_max_dB',maxdb.toFixed(0));kiwi_storeWrite(wf.audioFFT_active?'last_AF_min_dB':'last_min_dB',mindb_un.toFixed(0));}}}
...
if(!isNumber(kiwi.volume))kiwi.volume=50;kiwi.volume=w3_clamp3(kiwi.volume,0,200,0,50,50);kiwi.volume_f=(kiwi.muted||kiwi.volume==0)?1e-6:(kiwi.volume/100);if(done){w3_set_value('id-input-volume',kiwi.volume);kiwi_storeWrite('last_volume',kiwi.volume);freqset_select();}}
...
kiwi_storeWrite('last_audio',btn_less_buffering.toString());if(!isNumber(set)){window.location.reload(true);}}
...
btn_compression^=1;w3_els('id-button-compression',function(el){el.style.color=btn_compression?'lime':'white';el.style.visibility='visible';freqset_select();});kiwi_storeWrite('last_compression',btn_compression.toString());snd_send('SET compression='+btn_compression.toFixed(0));}
...
function agc_load_defaults(evt){setup_agc(toggle_e.SET);return cancelEvent(evt);}
...
owrx.vfo_ab^=1;w3_flip_colors('id-freq-vfo','w3-aqua w3-orange',owrx.vfo_ab);w3_innerHTML('id-freq-vfo',"AB"[owrx.vfo_ab]);var vfo=parse_freq_pb_mode_zoom(kiwi_storeRead('last_vfo_'+"AB"[owrx.vfo_ab]));var mode=undefined,zoom=0;if(vfo[3])mode=vfo[3].toLowerCase();if(vfo[4])zoom=+vfo[4];ext_tune(vfo[1],mode,ext_zoom.ABS,zoom);}
...
var mode_buttons=[{s:['AM','AMN','AMW'],dis:0},{s:['SAM','SAL','SAU','SAS','QAM'],dis:0},{s:['DRM'],dis:0},{s:['LSB','LSN'],dis:0},{s:['USB','USN'],dis:0},{s:['CW','CWN'],dis:0},{s:['NBFM','NNFM'],dis:0},{s:['IQ'],dis:0},];function mode_button(evt,el,dir){var mode=el.innerHTML.toLowerCase();if(isUndefined(dir))dir=1;if(any_alternate_click_event(evt)){if(evt.shiftKey||evt.button==mouse.middle){restore_passband(mode);ext_set_mode(mode,null,{open_ext:true});return;}
dir=-1;}
...
var divControl;function panel_setup_control(el){divControl=el;el.style.marginBottom='0';var el2=w3_el('id-control-inner');el2.innerHTML=w3_div('id-control-top',w3_div('id-control-freq1'),w3_div('id-control-freq2'),w3_div('id-control-mode w3-margin-T-6'),w3_div('id-control-zoom w3-margin-T-6'),w3_div('id-optbar w3-margin-T-4'))+
w3_div('id-optbar-content w3-margin-T-6 w3-scroll-y|height:'+px(kiwi.OPTBAR_CONTENT_HEIGHT),w3_div('id-optbar-rf w3-hide w3-scroll-y'),w3_div('id-optbar-wf w3-hide w3-scroll-y'),w3_div('id-optbar-audio w3-hide w3-scroll-y'),w3_div('id-optbar-agc w3-hide'),w3_div('id-optbar-users w3-hide w3-scroll-x'),w3_div('id-optbar-status w3-hide'))+
...
/* web/openwebrx/audio.min.js (web/openwebrx/audio.js = Fri Mar 22 16:24:46 2024) */
