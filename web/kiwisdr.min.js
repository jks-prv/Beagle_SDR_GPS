...
/* web/extensions/ext.min.js (web/extensions/ext.js = Wed Jul  5 08:19:03 2023) */
...
function ext_set_cfg_param(path,val,save){path=w3_add_toplevel(path);setVarFromString(path,val);save=(isArg(save)&&save==EXT_SAVE)?true:false;if(save){cfg_save_json('ext_set_cfg_param',path,val);}}
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Wed Jul  5 12:49:28 2023) */
...
if(path.includes('kiwisdr_com_register'))kiwi_trace();var s;if(path.startsWith('adm.')){config_save('adm',adm);}else
...
if(p.init!=true){p.INIT_ONCE=0;p.INIT_RESIZE=1;p.INIT_ALTBUF=2;p.resized=false;removeAllLines(parent_el);p.el=appendEmptyLine(parent_el);p.cols=p.cols||80;p.NONE=0;p.ESC=1;p.CSI=2;p.NON_CSI=3;p.NOSHIFT_MODE=true;p.esc={s:'',state:p.NONE};p.sgr={span:0,bright:0,fg:null,bg:null};p.return_pending=false;p.must_scroll_down=false;p.traceEvery=false;p.ccol=1;p.line=[];p.line_sgr=[];p.line_html=[];p.inc=1;p.nrows=p.rows;p.r=p.c=1;p.margin_set=false;p.margin_top=1;p.margin_bottom=p.nrows;p.show_cursor=p.show_cursor||false;p.r_cursor=p.c_cursor=0;p.insertMode=true;p.eol_wrap=false;init_common(p.INIT_ONCE);p.isAltBuf=false;p.altbuf_via_cursor_visible=false;p.altbuf_via_cup=false;p.alt_save='';p.rend_seq=0;p.metronome_running=false;p.sched_time=0;p.init=true;}
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Wed Jul  5 08:20:07 2023) */
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Wed Jul  5 08:25:19 2023) */
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Mon Jun 26 10:20:42 2023) */
...
function freqset_update_ui(from){freq_displayed_Hz=freq_car_to_dsp(freq_car_Hz);var f_kHz=freq_displayed_Hz/1000;freq_displayed_kHz_str=f_kHz.toFixed(freq_field_prec(f_kHz));if(!waterfall_setup_done)return;if(freqset_restore_ui()==null)return;waterfall_position(owrx.WF_POS_RECENTER_IF_OUTSIDE);writeCookie('last_freq',freq_displayed_kHz_str_with_freq_offset);freq_dsp_set_last=freq_displayed_kHz_str_with_freq_offset;freq_step_update_ui();freq_link_update();vfo_update();if(from!=owrx.FSET_NOP&&from!=owrx.FSET_SCALE_DRAG&&from!=owrx.FSET_SCALE_TOUCH_DRAG){freq_memory_update(freq_displayed_kHz_str_with_freq_offset);}
...
function dx_cfg_db(){var db=(dx.db==dx.DB_COMMUNITY)?dxcomm_cfg:dxcfg;return db;}
