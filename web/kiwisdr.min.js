...
/* web/extensions/ext.min.js (web/extensions/ext.js = Sun Sep 11 09:44:30 2022) */
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Sun Sep 11 10:15:15 2022) */
...
this.doset=function(){var freq=(freq_car_Hz/1000).toFixed(3);var mode=this.server_mode;var locut=this.low_cut.toString();var hicut=this.high_cut.toString();var m=mode.substr(0,2);var mparam=(m=='sa'||m=='qa')?(' param='+(owrx.chan_null|(owrx.SAM_opts<<owrx.SAM_opts_sft))):'';var s='SET mod='+mode+' low_cut='+locut+' high_cut='+hicut+' freq='+freq+mparam;snd_send(s);var changed=null;if(freq!=owrx.last_freq){changed=changed||{};changed.freq=1;owrx.last_freq=freq;}
...
w3_inline_percent('id-vol w3-valign w3-margin-T-2 w3-hide/class-slider w3-last-halign-end',w3_text(optbar_prefix_color,'Volume'),17,w3_slider('id-input-volume w3-wheel','','',kiwi.volume,0,200,1,'setvolume_cb'),50,'&nbsp;',8,w3_select('w3-text-red||title="de-emphasis"','','de-emp','de_emphasis',de_emphasis,de_emphasis_s,'de_emp_cb'))+
...
var squelch_save=0;function squelch_zero_cb(path,idx,first){idx=+idx;w3_color(path,idx?'white':'lime');var squelch_new;if(idx==0){squelch_save=squelch;squelch_new=0;}else{squelch_new=squelch_save;freqset_select();}
set_squelch_cb('',squelch_new,true,false,true);}
function pre_record_cb(path,val,first,no_write_cookie){if(first)return;pre_record=+val;if(no_write_cookie!=true&&cur_mode!='nbfm')
...
audio_data=new Int16Array(audio_buffer_size);audio_last_output_buffer=new Float32Array(audio_buffer_size)
...
decode_ima_adpcm_e8_i16(data_view,audio_data,bytes,audio_adpcm);samps=bytes*2;}else{if(audio.d){var now=Date.now();var msec=now-audio.last_msec;audio.last_msec=now;if(isIQ)
...
samps=bytes/2;for(i=0;i<samps;i++){audio_data[i]=data_view.getInt16(i*2,(flags&audio.SND_FLAG_LITTLE_ENDIAN)?true:false);}}
...
var samples=audio_mode_iq?1024:(compressed?2048:512);var pre_test=false;var pre_record_buffer=function(){if(!kiwi.pre_data)return;for(var i=0;i<samples;i++){var samp=pre_test?Math.floor(10000*(kiwi.pre_ping_pong?(kiwi.pre_size-kiwi.pre_off):kiwi.pre_off)/ kiwi.pre_size):audio_data[i];kiwi.pre_data.setInt16(kiwi.pre_off,samp,true);kiwi.pre_off+=2;if(kiwi.pre_off>=kiwi.pre_size){kiwi.pre_off=0;kiwi.pre_wrapped=true;kiwi.pre_ping_pong^=1;}}
kiwi.pre_captured=true;}
if(window.recording){var check_grow=function(){if(window.recording_meta.offset==65536){window.recording_meta.buffers.push(new ArrayBuffer(65536));window.recording_meta.data=new DataView(window.recording_meta.buffers[window.recording_meta.buffers.length-1]);window.recording_meta.offset=0;}};if(audio_last_sq==true){pre_record_buffer();}else{if(kiwi.pre_captured){if(kiwi.pre_wrapped){if(pre_test){for(var i=0;i<kiwi.pre_size;i+=2){var samp=kiwi.pre_data.getInt16(i,true);window.recording_meta.data.setInt16(window.recording_meta.offset,samp,true);window.recording_meta.offset+=2;check_grow();}
window.recording_meta.total_size+=kiwi.pre_size;}
console.log('AUDIO pre_record WRAPPED insert pre_off='+kiwi.pre_off+'/'+kiwi.pre_size+' ping_pong='+kiwi.pre_ping_pong);for(var i=kiwi.pre_off;i<kiwi.pre_size;i+=2){var samp=kiwi.pre_data.getInt16(i,true);window.recording_meta.data.setInt16(window.recording_meta.offset,samp,true);window.recording_meta.offset+=2;check_grow();}
