...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Sat Sep  3 08:12:05 2022) */
...
if(!p.init){p.el=appendEmptyLine(parent_el);p.esc={s:'',state:0};p.sgr={span:0,bright:0,fg:null,bg:null};p.return_pending=false;p.inc=1;p.r=p.c=1;p.show_cursor=false;p.insertMode=true;p.screen=[];p.color=[];p.dirty=[];p.els=[];for(var r=0;r<=p.rows;r++){p.screen[r]=[];p.color[r]=[];}
...
if(dbg)console.log(JSON.stringify(s));for(var si=0;si<s.length;si++){var c=s.charAt(si);if(p.inline_returns&&p.return_pending&&c!='\r'){if(p.isAltBuf){}else{p.tstr=snew='';p.col=0;p.return_pending=false;}}
if(c=='\f'){if(p.isAltBuf){}else{removeAllLines(parent_el);p.el=appendEmptyLine(parent_el);p.tstr=snew='';p.col=0;}}else
if(c=='\r'){if(dbg)console.log('\\r inline, isAltBuf='+p.isAltBuf);if(p.isAltBuf){p.c=1;}
if(p.inline_returns){p.return_pending=true;}}else
if(c=='\b'){if(p.tstr.length>1)
...
if(p.esc.state==1){if((c<'@'&&c!='>')||c=='['){p.esc.s+=c;}else{p.esc.s+=c;var first=p.esc.s.charAt(0);var second=p.esc.s.charAt(1);var last_hl=(c=='h'||c=='l');var enable=(c=='h');var result=0;var n1=1,n2=1;var t=p.esc.s.slice(1);if(t.length>1&&isNumber(+t[0])){var a=t.slice(0,t.length-1);a=a.split(';');n1=+a[0];n1=n1||1;if(a.length>1)n2=+a[1];n2=n2||1;}
...
p.dirty[p.r]=true;sched();}}else
if(c=='H'){result='move '+n1+','+n2;p.r=n1;p.c=n2;}else
if(c=='d'){result='move row '+n1+' (col = 1)';p.r=n1;p.c=1;}else
if(c=='G'){result='move col'+n1;p.c=n1;}else
if(second=='?'&&last_hl){n1=parseInt(p.esc.s.substr(2));result=(enable?'SET':'RESET')+' ';switch(n1){case 1:result+='automatic margins';break;case 7:result+='vertical autowrap';break;case 12:result+='(part of RESET cursor visible)';break;case 25:result+='cursor visible';p.show_cursor=true;break;case 1049:result+='alt screen buf';if(dbg&&enable&&!p.isAltBuf){p.alt_save=parent_el.innerHTML;removeAllLines(parent_el);for(var r=1;r<=p.rows;r++){p.dirty[r]=false;for(var c=1;c<=p.cols;c++){p.screen[r][c]=' ';p.color[r][c]={fg:null,bg:null};}
...
p.dirty[p.r]=true;sched();result='eraseX #'+n1+' chars';}else
...
p.dirty[p.r]=true;sched();result='del #'+n1+' chars';}else
if(c=='A'||c=='B'||c=='C'||c=='D'){result='arrow key '+c;}else{result=2;}}else
...
if(c=='\n'||p.col==p.cols){wasScrolledDown=w3_isScrolledDown(el_scroll);if(p.isAltBuf){p.c=1;p.r++;if(p.r>p.rows)p.r=1;}else{p.tstr+=snew;if(p.tstr=='')p.tstr='&nbsp;';p.el.innerHTML=p.tstr;if(dbg)console.log('TEXT2 '+JSON.stringify(p.tstr));p.tstr=snew='';p.el=appendEmptyLine(parent_el);p.col=0;}
...
var shortcut={nav_off:0,keys:null,NO_MODIFIER:0,SHIFT:1,CTL_OR_ALT:2,SHIFT_PLUS_CTL_OR_ALT:3,KEYCODE_ALT:18};function shortcut_key_mod(evt){var sft=evt.shiftKey;var ctl=evt.ctrlKey;var alt=evt.altKey;var meta=evt.metaKey;var ctlAlt=(ctl||alt);return(sft&&!ctlAlt)?shortcut.SHIFT:((!sft&ctlAlt)?shortcut.CTL_OR_ALT:((sft&&ctlAlt)?shortcut.SHIFT_PLUS_CTL_OR_ALT:shortcut.NO_MODIFIER));}
...
function keyboard_shortcut_help(){confirmation_show_content(shortcut.help,550,585);}
...
var sft=evt.shiftKey;var ctl=evt.ctrlKey;var alt=evt.altKey;var meta=evt.metaKey;var ctlAlt=(ctl||alt);var key_mod=shortcut_key_mod(evt);var field_input_key=((k>='0'&&k<='9'&&!ctl)||k=='.'||k==','||k=='/'||k==':'||k=='-'||k=='k'||k=='M'||k=='Enter'||k=='Backspace'||k=='Delete');if(evt.target.nodeName!='INPUT'||(id=='id-freq-input'&&!field_input_key)){if(kiwi_isMacOS()){if(meta){return;}}else{if(ctl){return;}}
if(alt&&!k.startsWith('Arrow')){k=String.fromCharCode(evt.keyCode).toLowerCase();if(sft)k=k.toUpperCase();}
keyboard_shortcut(k,key_mod,ctlAlt,evt.keyCode);return cancelEvent(evt);}else{}}
