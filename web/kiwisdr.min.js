...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Wed Aug 24 14:52:01 2022) */
...
s='ncc';if(q[s])no_clk_corr=parseInt(q[s]);s='wfdly';if(q[s])waterfall_delay=parseFloat(q[s]);s='wf_comp';if(q[s])wf_compression=parseInt(q[s]);s='wfi';if(q[s])wf_interp=parseInt(q[s]);s='wfw';if(q[s])wf_winf=parseInt(q[s]);s='gen';if(q[s])gen_freq=q[s].parseFloatWithUnits('kM',1e-3);s='attn';if(q[s])gen_attn=Math.abs(parseInt(q[s]));s='blen';if(q[s])audio_buffer_min_length_sec=parseFloat(q[s])/1000;s='audio';if(q[s])audio_meas_dly_ena=parseFloat(q[s]);s='gc';if(q[s])kiwi_gc=parseInt(q[s]);s='gc_snd';if(q[s])kiwi_gc_snd=parseInt(q[s]);s='gc_wf';if(q[s])kiwi_gc_wf=parseInt(q[s]);s='gc_recv';if(q[s])kiwi_gc_recv=parseInt(q[s]);s='gc_wspr';if(q[s])kiwi_gc_wspr=parseInt(q[s]);s='ctrace';if(q[s]){param_ctrace=true;ctrace=parseInt(q[s]);}
...
set_gen(gen_freq,gen_attn);},0,500);var _lo=cfg.passbands[init_mode].lo;if(!isNumber(_lo))_lo=-4000;var _hi=cfg.passbands[init_mode].hi;if(!isNumber(_hi))_hi=4000;snd_send('SET mod='+init_mode+' low_cut='+_lo+' high_cut='+_hi+' freq='+init_frequency);set_agc();snd_send("SET browser="+navigator.userAgent);wf_send("SERVER DE CLIENT openwebrx.js W/F");wf_send("SET send_dB=1");wf_send("SET zoom=0 start=0");wf_send("SET maxdb=0 mindb=-100");if(wf_compression==0)wf_send('SET wf_comp=0');if(wf_interp!=-1)wf_send('SET interp='+wf_interp);if(wf_winf!=0)wf_send('SET window_func='+wf_winf);wf_speed=wf_rates[wf_rate];if(wf_speed==undefined)wf_speed=WF_SPEED_FAST;wf_send('SET wf_speed='+wf_speed);}
