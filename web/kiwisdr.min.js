...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Thu Oct  6 11:59:09 2022) */
...
snew+=c;};var snew_dump=function(where){if(snew!=''){if(dbg)console.log('> '+(p.isAltBuf?'ALT':'REG')+' '+where+' ('+p.snew_r+','+p.snew_c+') '+' CHARS #'+snew.length+' '+kiwi_JSON(snew));snew='';}};var line_s=function(){return kiwi_JSON(p.line.join(''));};var line_join=function(col_start,col_stop){col_stop=col_stop||p.cols;var s='';for(var c=col_start;c<col_stop;c++){if(isString(p.line_sgr[c]))s+=p.line_sgr[c];if(isString(p.line[c]))s+=p.line[c];}
...
if(p.init!=true){removeAllLines(parent_el);p.el=appendEmptyLine(parent_el);p.NONE=0;p.ESC=1;p.CSI=2;p.NON_CSI=3;p.NOSHIFT_MODE=true;p.esc={s:'',state:p.NONE};p.sgr={span:0,bright:0,fg:null,bg:null};p.return_pending=false;p.must_scroll_down=false;p.traceEvery=false;p.ccol=1;p.line=[];p.line_sgr=[];p.inc=1;p.r=p.c=1;p.margin_set=false;p.margin_top=1;p.margin_bottom=p.rows;p.show_cursor=p.show_cursor||false;p.r_cursor=p.c_cursor=0;p.insertMode=true;p.eol_wrap=false;p.screen=[];p.color=[];p.dirty=[];p.els=[];for(var r=0;r<=p.rows;r++){p.screen[r]=[];p.color[r]=[];}
p.isAltBuf=false;p.altbuf_via_cursor_visible=false;p.altbuf_via_cup=false;p.alt_save='';p.rend_timeout=null;p.init=true;}
var snew='',stmp;var el_scroll=w3_el(id_scroll);var wasScrolledDown=null;if(p.process_return_alone&&s.charAt(0)=='\r'&&(s.length==1||s.charAt(1)!='\n')){if(p.isAltBuf){}else{s=s.substring(1);p.line=[];p.line_sgr=[];p.ccol=1;}}
...
if(c=='\f'){if(p.isAltBuf){}else{if(dbg)console.log('console \f');removeAllLines(parent_el);p.el=appendEmptyLine(parent_el);p.line=[];p.line_sgr=[];p.ccol=1;}}else
...
erase_in_display(r_start,r_end,c_start,c_end);}else{if(second=='2'||second=='3'){if(dbg)console.log('console ERASE FULL SCREEN');removeAllLines(parent_el);p.el=appendEmptyLine(parent_el);p.line=[];p.line_sgr=[];p.ccol=1;result='erase full screen';}}}else
...
if(c>=' '||c=='\n'||c=='\t'){if(!p.isAltBuf&&c=='<'){if(p.inc){p.line[p.ccol]='&lt;';p.ccol++;}}else
if(!p.isAltBuf&&c=='>'){if(p.inc){p.line[p.ccol]='&gt;';p.ccol++;}}else{if(c!='\n'){snew_add(c);if(p.isAltBuf){screen_char(c);}else{p.line[p.ccol]=c;p.ccol++;}}
if(c=='\n'||(p.eol_wrap&&p.ccol==p.cols)){wasScrolledDown=w3_isScrolledDown(el_scroll);if(p.isAltBuf){p.c=1;dirty();p.r++;dirty();if(p.r>p.rows)p.r=1;}else{var stmp=line_join(1);p.el.innerHTML=(stmp=='')?'&nbsp;':stmp;p.line=[];p.line_sgr=[];p.el=appendEmptyLine(parent_el);p.ccol=1;}
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Thu Sep 29 08:18:18 2022) */
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Thu Oct  6 10:36:25 2022) */
...
function pre_record_cb(path,val,first,no_write_cookie){if(first)return;pre_record=+val;audio_pre_record_buf_init();if(no_write_cookie!=true&&cur_mode!='nbfm')
