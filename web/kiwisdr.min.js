...
/* web/extensions/ext.min.js (web/extensions/ext.js = Sun Nov 19 06:37:03 2023) */
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Sat Nov 18 09:31:35 2023) */
...
/* web/kiwi/kiwi_ui.min.js (web/kiwi/kiwi_ui.js = Wed Nov 22 13:33:46 2023) */
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Sun Nov 19 08:24:39 2023) */
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Wed Nov 22 15:40:06 2023) */
...
function freqset_update_ui(from){freq_displayed_Hz=freq_car_to_dsp(freq_car_Hz);var f_kHz=freq_displayed_Hz/1000;freq_displayed_kHz_str=f_kHz.toFixed(freq_field_prec(f_kHz));if(!waterfall_setup_done)return;if(freqset_restore_ui()==null)return;waterfall_position(owrx.WF_POS_RECENTER_IF_OUTSIDE);writeCookie('last_freq',freq_displayed_kHz_str_with_freq_offset);freq_dsp_set_last=freq_displayed_kHz_str_with_freq_offset;freq_step_update_ui();freq_link_update();vfo_update();if(from!=owrx.FSET_NOP&&from!=owrx.FSET_SCALE_DRAG&&from!=owrx.FSET_SCALE_TOUCH_DRAG&&from!=owrx.FSET_PB_CHANGE){}
...
function freq_memory_init(){var fmem;if(isNonEmptyString(owrx.override_fmem)){fmem=owrx.override_fmem.split(',');}else{fmem=kiwi_JSON_parse('freq_memory_init',readCookie('freq_memory'));}
if(isNull(fmem))fmem=[10000];var prec=(cfg.show_1Hz||url_1Hz)?3:2;fmem.forEach(function(s,i){if(isNumber(s))s=s.toString();var f=s.parseFloatWithUnits('kM',1e-3);if(isNumber(f))
fmem[i]=w3_clamp(f,1,cfg.max_freq?32000:30000).toFixed(prec);});if(fmem){owrx.freq_memory=kiwi_dedup_array(fmem,function(v){var rv={err:false};v=+v;if(!isNumber(v)||v<=0)
rv.err=true;else
rv.v=v.toString();return rv;});owrx.freq_memory.forEach(function(s,i){if(isNumber(s))s=s.toString();var f=s.parseFloatWithUnits('kM',1e-3);if(isNumber(f))
...
x+=((x-128)>=0)?-128:16;var fmem_copy=kiwi_dup_array(owrx.freq_memory);fmem_copy.unshift('store');fmem_copy.push('clear all');fmem_copy.push('VFO A=B');w3_menu_items('id-freq-memory-menu',fmem_copy,Math.min(fmem_copy.length,10));w3_menu_popup('id-freq-memory-menu',function(evt,first){var close_keyup=(evt.type=='keyup'&&(evt.key!='m'||!owrx.freq_memory_menu_shown));var tgt_isMenu=w3_contains(evt.target,'w3-menu');var tgt_isMenuButton=w3_contains(evt.target,'w3-menu-button');var menu_shown=owrx.freq_memory_menu_shown;var click=((evt.type=='click'||((evt.type=='mousedown'||evt.type=='touchstart')&&!tgt_isMenuButton)||evt.type=='touchend'));var close_click=(click&&!first&&(!tgt_isMenu||!menu_shown));var close=(close_keyup||close_click);return close;},x,y);}
function freq_memory_menu_show_cb(set,val,trace,ev){var hold=(ev&&ev.type=='hold');w3_colors('id-freq-menu','w3-text-white','w3-text-css-lime',hold);if(hold){freq_memory_update(freq_displayed_kHz_str_with_freq_offset);}else
if(ev&&ev.type=='hold-done'){}else{freq_memory_menu_show(false);}}
...
function freq_memory_menu_cb(idx,x){idx=+idx;if(idx==-1)return;var f,set_f=false;if(idx==0){freq_memory_update(freq_displayed_kHz_str_with_freq_offset);}else
if(idx==owrx.freq_memory.length+1){f=freq_displayed_kHz_str_with_freq_offset;owrx.freq_memory=[f];writeCookie('freq_memory',JSON.stringify(owrx.freq_memory));}else
if(idx==owrx.freq_memory.length+2){vfo_update(true);}else
if(idx<=owrx.freq_memory.length){f=owrx.freq_memory[idx-1];set_f=true;owrx.freq_memory.unshift(f);owrx.freq_memory=kiwi_dedup_array(owrx.freq_memory);if(owrx.freq_memory.length>25)owrx.freq_memory.pop();}
if(set_f){freqmode_set_dsp_kHz(f-kiwi.freq_offset_kHz);}else{freqset_select();}}
