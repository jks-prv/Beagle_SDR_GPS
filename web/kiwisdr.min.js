...
/* web/extensions/ext.min.js (web/extensions/ext.js = Wed Apr 24 10:32:35 2024) */
...
function extint_list_json(param){extint.ext_names=kiwi_JSON_parse('extint_list_json',decodeURIComponent(param));extint.former_exts.forEach(function(e,i){extint.ext_names.push(e);});kiwi_dedup_array(extint.ext_names);extint.ext_names.sort(kiwi_sort_ignore_case);}
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Sat May  4 17:06:41 2024) */
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Sat May  4 17:14:31 2024) */
...
Number.prototype.withSign=function(){var n=Number(this);var s=n.toString();return(n<0)?s:('+'+s);};var kHz=function(f){return(f / 1e3).toFixed(3)+'k';};function kiwi_setTimeout(func,msec,param){func(param);return setTimeout(func,msec,param);}
...
function event_dump(evt,id,oneline){var k=evt.key||'(no key)';var ct_id;if(oneline){var trel=(isDefined(evt.relatedTarget)&&evt.relatedTarget)?(' Trel='+evt.relatedTarget.id):'';var key=key_stringify(evt);ct_id=evt.currentTarget?evt.currentTarget.id:'(null)';console.log('event_dump '+id+' |'+evt.type+'| k='+key+' T='+evt.target.id+' Tcur='+ct_id+trel);}else{console.log('================================');if(isNoArg(evt)){console.log('EVENT_DUMP: '+id+' bad evt');kiwi_trace();return;}
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Sat May  4 17:02:53 2024) */
...
/* web/kiwi/waterfall.min.js (web/kiwi/waterfall.js = Sat May  4 10:38:09 2024) */
...
function waterfall_maxmin_cb(){w3_flash_fade('id-wfext-maxmin','cyan',50,300,'#575757');var dyn_range=maxdb-mindb;var total_s=mindb.toString().positiveWithSign()+','+maxdb.toString().positiveWithSign();var computed_s=wf.auto_mindb.toString().positiveWithSign()+','+wf.auto_maxdb.toString().positiveWithSign();var floor_ceil_s=wf.auto_floor.val.toString().positiveWithSign()+','+wf.auto_ceil.val.toString().positiveWithSign();w3_innerHTML('id-wfext-content',paren(total_s)+'&nbsp;=&nbsp;'+paren(computed_s)+'&nbsp;+&nbsp;'+paren(floor_ceil_s)+'&nbsp;&nbsp;[&Delta; '+dyn_range+' dB]');}
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Wed Apr 24 13:09:59 2024) */
var ant_sw={ver:'',n_ant:8,backend:-1,backend_s:null,backends_s:null,can_mix:true,ip_or_url:null,exantennas:0,focus:false,isConfigured:false,running:false,buttons_locked:false,buttons_selected:[],denymixing:0,thunderstorm:0,notify_timeout:null,url_ant:null,url_idx:-1,desc_lc:[],last_offset:-1,last_high_side:-1,EVERYONE:0,LOCAL_CONN:1,LOCAL_OR_PWD:2,denyswitching:0,deny_s:['everyone','local connections only','local connections or user password only'],DENY_NONE:0,DENY_SWITCHING:1,DENY_MULTIUSER:2,denied_permission:false,denied_because_multiuser:false};function ant_switch_log(s){}
...
freqset_select();}
function openwebrx_resize(a){a=(isString(a)&&a.startsWith('orient'))?a:'event';resize_wf_canvases();resize_waterfall_container(true);extint_environment_changed({resize:1,passband_screen_location:1});resize_scale(a);check_top_bar_congestion();owrx.resize_seq++;}
...
demodulators=[];demodulator_color_index=0;demodulator_colors=["#ffff00","#00ff00","#00ffff","#058cff","#ff9600","#a1ff39","#ff4e39","#ff5dbd"];function demodulators_get_next_color(){if(demodulator_color_index>=demodulator_colors.length)demodulator_color_index=0;return(demodulator_colors[demodulator_color_index++]);}
...
return drag_ranges;}
function demod_envelope_where_clicked(x,y,drag_ranges,key_modifiers){in_range=function(x,g_range){return g_range.x1<=x&&g_range.x2>=x&&y<scale_canvas_h/2;};dr=demodulator.draggable_ranges;if(key_modifiers.shiftKey){if(drag_ranges.line_on_screen&&in_range(x,drag_ranges.line))return dr.bfo;if(drag_ranges.envelope_on_screen&&in_range(x,drag_ranges.whole_envelope))return dr.pbs;}
...
if(audio_meas_dly_ena){audio_meas_dly_start=(new Date()).getTime();}};this.envelope.parent=this;this.envelope.draw=function(visible_range){this.visible_range=visible_range;this.drag_ranges=demod_envelope_draw(g_range,center_freq+this.parent.offset_frequency+this.parent.low_cut,center_freq+this.parent.offset_frequency+this.parent.high_cut,this.color,center_freq+this.parent.offset_frequency);var isAdjPBT=(owrx.last_shiftKey||this.dragged_range==demodulator.draggable_ranges.pbs);var bw=Math.abs(this.parent.high_cut-this.parent.low_cut);var bw_pbt;if(isAdjPBT){bw_pbt=', PBT';}else{bw_pbt=', bw '+bw.toString();}
...
function scale_canvas_drag(evt,x,y){var i;if(scale_canvas_ignore_mouse_event)return;var event_handled=0;var relX=Math.abs(x-scale_canvas_drag_params.start_x);if(scale_canvas_drag_params.mouse_down&&!scale_canvas_drag_params.drag){scale_canvas_drag_params.drag=true;for(i=0;i<demodulators.length;i++)
...
if(scale_canvas_drag_params.drag){for(i=0;i<demodulators.length;i++)
...
for(j=0;j<1024;j++){var re=afft.o_re[j],im=afft.o_im[j];var pwr=re*re+im*im;afft.dBi[j&1]=Math.round(255+(10.0*Math.log10(pwr*afft.scale+1e-30)));if(j&1){afft.pwr_dB[k]=Math.max(afft.dBi[0],afft.dBi[1]);k+=ki;}}}else{for(i=0;i<1024;i++){afft.i_re[i]=audio_data[i]*afft.window_1k[i];}
afft.offt.fft(afft.offt,afft.i_re.buffer,afft.o_re.buffer,afft.o_im.buffer);if(lsb){k=512;ki=-1;}else{k=256;ki=+1;}
for(j=0;j<512;j++,k+=ki){var re=afft.o_re[j],im=afft.o_im[j];var pwr=re*re+im*im;afft.pwr_dB[k]=Math.round(255+(10.0*Math.log10(pwr*afft.scale+1e-30)));}
...
for(j=0;j<256;j++,k+=ki){var re=afft.o_re[j],im=afft.o_im[j];var pwr=re*re+im*im;afft.pwr_dB[k]=afft.pwr_dB[k+1]=Math.round(255+(10.0*Math.log10(pwr*afft.scale+1e-30)));}
...
function freqset_update_ui(from){freq_displayed_Hz=freq_car_to_dsp(freq_car_Hz);var f_kHz=freq_displayed_Hz/1000;freq_displayed_kHz_str=freq_field_prec_s(f_kHz);if(!waterfall_setup_done)return;if(freqset_restore_ui()==null)return;if(from==owrx.FSET_SET_FREQ&&zoom_center!=0.5){zoom_step(ext_zoom.CUR);}else{waterfall_position(owrx.WF_POS_RECENTER_IF_OUTSIDE);}
...
function freqstep(path,sel,first,ev){var shiftKey=ev?ev.shiftKey:false;var hold=(ev&&ev.type=='hold');console.log('freqstep: path='+path+' sel='+sel+' first='+first+' ev='+ev+' shiftKey='+shiftKey+' hold='+hold);if(hold){console.log('freqstep HOLD');return;}
var step_Hz=up_down[cur_mode][sel]*1000;if(step_Hz==0){var b=find_band(freq_displayed_Hz);step_Hz=special_step(b,sel,'freqstep');}
...
function keyboard_shortcut_help(){confirmation_show_content(shortcut.help,550,705);}
...
freqstep(null,owrx.wf_snap?key_mod:(2-key_mod));else
dx_label_step(-1);break;case'i':case'I':case'ArrowRight':if(key_mod!=shortcut.SHIFT_PLUS_CTL_ALT)
freqstep(null,owrx.wf_snap?(5-key_mod):(3+key_mod));else
...
function panels_setup(){var el,s,fmt;el=w3_el("id-ident");el.innerHTML=w3_input('w3-label-not-bold/w3-custom-events|padding:1px|size=20'+' onchange="ident_complete(\'el\', 1)" onkeyup="ident_keyup(this, event, 1)"','Your name or callsign:','ident-input1');el.addEventListener('click',function(evt){return cancelEvent(evt);},true);var mobile=kiwi_isMobile()?(' inputmode='+dq(kiwi_is_iOS()?'decimal':'tel')+' ontouchstart="popup_keyboard_touchstart(event)" onclick="this.select()"'):'';w3_el("id-control-freq1").innerHTML=w3_inline('w3-halign-space-between/',w3_div('id-freq-cell','<form id="id-freq-form" name="form_freq" action="#" onsubmit="freqset_complete(0, event); return false;">'+
...
w3_el('id-optbar-status').innerHTML=w3_div('id-status-msg')+
...
function rf_attn_cb(path,val,done,first,ui_only){if(first){val=kiwi.rf_attn;}
if(kiwi.rf_attn_disabled&&!ui_only)return;var attn=parseFloat(val);var input_attn=w3_el('id-rf-attn');var field_attn=w3_el('id-field-rf-attn');if(!input_attn||!field_attn)return;kiwi.rf_attn=attn;input_attn.value=attn;field_attn.innerHTML=attn.toFixed(1)+' dB';field_attn.style.color="white";if(ui_only==true)return;snd_send('SET rf_attn='+attn.toFixed(1));if(done){kiwi_storeWrite('last_rf_attn',val);freqset_select();}}
...
function set_ceilfloordb(done){maxdb=wf.auto_maxdb+wf.auto_ceil.val;mindb=wf.auto_mindb+wf.auto_floor.val;var range=maxdb-mindb;if(cfg.spec_min_range){if(range<cfg.spec_min_range){var boost=cfg.spec_min_range-range;maxdb+=boost;range=maxdb-mindb;}}
mindb_un=mindb+zoomCorrection();setmaxmindb(done,false);w3_call('waterfall_maxmin_cb');if(done){freqset_select();kiwi_storeWrite('last_ceil_dB',wf.auto_ceil.val.toFixed(0));kiwi_storeWrite('last_floor_dB',wf.auto_floor.val.toFixed(0));}}
...
var mobile1=kiwi_isMobile()?' w3-font-16px':'';var mobile2=' ontouchstart="popup_keyboard_touchstart(event)"';s+=w3_inline('w3-valign/1:w3-ialign-right 1:w3-ialign-bottom 1:w3-margin-B-4',w3_input('w3-margin-TB-4/w3-label-not-bold/w3-custom-events'+mobile1+'|padding:1px|size=100 onchange="ident_complete(\'el\', 2)" onkeyup="ident_keyup(this, event, 2)"'+mobile2,'Your name or callsign:','ident-input2'),w3_button('id-admin-page-btn w3-aqua w3-small w3-padding-small w3-margin-L-32 w3-margin-R-4','admin page','admin_page_cb'));w3_innerHTML('id-optbar-users',w3_div('w3-nowrap',s));}
function admin_page_cb(){setTimeout(function(){kiwi_open_or_reload_page({path:'admin',tab:1});},1);}
