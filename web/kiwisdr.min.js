...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Wed Aug 31 09:28:47 2022) */
...
function kiwi_output_msg(id,id_scroll,p){var i,j;var dbg=(1&&dbgUs);var parent_el=w3_el(id);if(!parent_el){console.log('kiwi_output_msg NOT_FOUND id='+id);return;}
var appendEmptyLine=function(parent_el){return w3_create_appendElement(parent_el,'pre','');};var removeAllLines=function(parent_el){while(parent_el.firstChild){parent_el.removeChild(parent_el.firstChild);}};var scr_char=function(c){p.scr[p.r][p.c]=c;p.els[p.r-1].innerHTML=p.scr[p.r].join('');if(p.els[p.r-1].innerHTML=='')p.els[p.r-1].innerHTML='&nbsp;';p.c++;if(p.c>p.cols){p.c=1;p.r++;if(p.r>p.rows)p.r=1;}
p.tstr+=c;kiwi_clearTimeout(p.rend_timeout);p.rend_timeout=setTimeout(function(){if(p.tstr!=''){if(dbg)console.log('REND '+JSON.stringify(p.tstr));p.tstr='';}},500);};var s;try{s=kiwi_decodeURIComponent('kiwi_output_msg',p.s);}catch(ex){console.log('decodeURIComponent FAIL:');console.log(p.s);s=p.s;}
if(!p.init){p.el=appendEmptyLine(parent_el);p.esc={s:'',state:0};p.sgr={span:0,bright:0,fg:null,bg:null};p.return_pending=false;p.inc=1;p.r=p.c=1;p.rows=10;p.cols=100;p.scr=[];p.color=[];p.els=[];for(i=0;i<=p.rows;i++){p.scr[i]=[];p.color[i]=[];for(j=0;j<=p.cols;j++){p.scr[i][j]='';p.color[i][j]=null;}}
p.isAltBuf=false;p.alt_save='';p.test_altbuf=true;p.traceEvery=false;p.rend_timeout=null;p.init=true;}
...
if(p.remove_returns)s=s.replace(/\r/g,'');if(p.inline_returns){s=s.replace(/\r\n/g,'\n');s=s.replace(/\r\n/g,'\n');}
if(dbg)console.log(JSON.stringify(s));if(p.test_altbuf&&p.isAltBuf&&s.includes('foo')){console.log('$EXIT alt buf');removeAllLines(parent_el);parent_el.innerHTML=p.alt_save;p.isAltBuf=false;}
for(i=0;i<s.length;i++){var c=s.charAt(i);if(p.inline_returns&&p.return_pending&&c!='\r'){if(p.isAltBuf){}else{p.tstr=snew='';p.col=0;p.return_pending=false;}}
if(c=='\f'){if(p.isAltBuf){}else{removeAllLines(parent_el);p.el=appendEmptyLine(parent_el);p.tstr=snew='';p.col=0;}}else
if(!p.isAltBuf&&p.inline_returns&&c=='\r'){if(p.isAltBuf){}else{p.return_pending=true;}}else
if(c=='\t'){if(p.isAltBuf){}else{snew+='&nbsp;';p.col++;while((p.col&7)!=0){snew+='&nbsp;';p.col++;}}}else
if(c=='\x1b'){p.esc.s='';p.esc.state=1;p.traceEvery=false;if(p.isAltBuf&&p.tstr!=''){kiwi_clearTimeout(p.rend_timeout);if(dbg)console.log('REND '+JSON.stringify(p.tstr));p.tstr='';}}else
if(p.esc.state==1){if(c<'@'||c=='['){p.esc.s+=c;}else{p.esc.s+=c;var first=p.esc.s.charAt(0);var second=p.esc.s.charAt(1);var last_hl=(c=='h'||c=='l');var enable=(c=='h');var n=0,result=0;var n1=1,n2=1;var t=p.esc.s.slice(1);if(t.length>1&&isNumber(+t[0])){var a=t.slice(0,t.length-1);a=a.split(';');n1=+a[0];n1=n1||1;if(a.length>1)n2=+a[1];n2=n2||1;}
if(first=='['){if(c=='m'){var rv=kiwi_output_sgr(p);result=rv.result;if(p.isAltBuf){if(rv.color_change)
p.color[p.r][p.c]={fg:p.sgr.fg,bg:p.sgr.bg};}else{snew+=rv.snew;}
...
if(c=='H'){result='move '+n1+','+n2;p.r=n1;p.c=n2;}else
if(c=='d'){result='move row '+n1;p.r=n1;p.c=1;}else
if(c=='G'){result='move col'+n1;p.c=n1;}else
if(second=='?'&&last_hl){n=parseInt(p.esc.s.substr(2));result=(enable?'SET':'RESET')+' ';switch(n){case 7:result+='vertical autowrap';break;case 25:result+='cursor visible';break;case 1049:result+='alt screen buf';if(dbg&&enable&&!p.isAltBuf){p.alt_save=parent_el.innerHTML;removeAllLines(parent_el);for(j=0;j<p.rows;j++){p.els[j]=appendEmptyLine(parent_el);p.els[j].innerHTML='&nbsp;';}
p.tstr='';p.isAltBuf=true;}else
if(!p.test_altbuf){if(!enable&&p.isAltBuf){console.log('$EXIT alt buf');parent_el.innerHTML=p.alt_save;p.isAltBuf=false;}}
break;default:result+='$UNKNOWN ='+n;break;}}else
if(c=='r'){result='set margins, top='+n1+', bottom='+n2;}else
if(second=='4'&&last_hl){result=(c=='h')?'INSERT mode':'REPLACE mode';}else
if(c=='P'){n=second-'0';result='del #'+n+' chars';}else{result=2;}}else
if(first=='('){result='define char set';p.traceEvery=true;}else{result=2;}
...
if(c=='>'){snew+='&gt;';p.col+=p.inc;}else{if(c!='\n'){if(p.isAltBuf){scr_char(c);}else{snew+=c;p.col+=p.inc;}}
if(c=='\n'||p.col==p.ncol){wasScrolledDown=w3_isScrolledDown(el_scroll);if(p.isAltBuf){p.c=1;p.r++;if(p.r>p.rows)p.r=1;}else{p.tstr+=snew;if(p.tstr=='')p.tstr='&nbsp;';p.el.innerHTML=p.tstr;if(dbg)console.log('TEXT2 '+JSON.stringify(p.tstr));p.tstr=snew='';p.el=appendEmptyLine(parent_el);p.col=0;}
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Tue Aug 30 12:58:17 2022) */
var owrx={debug_drag:false,height_top_bar_parts:67,hide_bars:0,HIDE_TOPBAR:1,HIDE_BANDBAR:2,HIDE_ALLBARS:3,cfg_loaded:false,mobile:null,wf_snap:0,wf_cursor:'crosshair',last_pageX:0,last_pageY:0,mouse_freq:{},freq_memory:[],freq_memory_menu_shown:0,override_fmem:null,vfo_ab:0,vfo_first:true,last_freq:-1,last_mode:'',last_locut:-1,last_hicut:-1,last_lo:[],last_hi:[],last_selected_band:0,last_mode_el:null,dseq:0,sMeter_dBm_biased:0,sMeter_dBm:0,smeter_interval:null,force_no_agc_thresh_smeter:false,overload_mute:90,squelched_overload:false,prev_squelched_overload:false,touch_hold_pressed:false,tuning_locked:0,dx_click_gid_last:undefined,sam_pll:1,sam_pll_s:['DX','med','fast'],chan_null:0,chan_null_s:['normal','null LSB','null USB'],ovld_mute:0,ovld_mute_s:['off','on'],FSET_NOP:0,FSET_ADD:1,FSET_SET_FREQ:2,FSET_EXT_SET_PB:3,FSET_PB_CHANGE:4,FSET_SCALE_DRAG:5,FSET_SCALE_DRAG_END:6,FSET_SCALE_MOUSE_CLICK:7,FSET_SCALE_TOUCH_DRAG:8,FSET_SCALE_TOUCH_DRAG_END:9,fset_s:['nop','add','fset','extSetPB','pbChg','drag','dragEnd','mouseClick','touchDrag','touchClick'],scale_canvas:{mouse_out:false,target:null,restore_visibility:[],restore_visibility_delay:500},canvas:{target:null,restore_visibility:[],restore_visibility_delay:500},__last:0};var bandwidth;var center_freq;var wf_fft_size;var cur_mode;var wf_fps,wf_fps_max;var ws_snd,ws_wf;var spectrum_show=0,spectrum_param=-1;var gen_freq=0,gen_attn=0;var squelch_threshold=0;var wf_rate='';var wf_mm='';var wf_compression=1;var wf_interp=13;var wf_winf=2;var snd_winf=0;var debug_v=0;var sb_trace=0;var kiwi_gc=1;var kiwi_gc_snd=0;var kiwi_gc_wf=-1;var kiwi_gc_recv=-1;var kiwi_gc_wspr=-1;var override_ext=null;var muted_initially=0;var peak_initially=null;var param_nocache=false;var nocache=false;var param_ctrace=false;var ctrace=false;var no_clk_corr=false;var override_pbw='';var override_pbc='';var nb_click=false;var no_geoloc=false;var force_mobile=false;var mobile_laptop_test=false;var user_url=null;var wf_rates={'0':0,'off':0,'1':1,'1hz':1,'s':2,'slow':2,'m':3,'med':3,'f':4,'fast':4};var okay_wf_init=false;function kiwi_main(){w3_do_when_cond(function(){return owrx.cfg_loaded;},function(){kiwi_main_ready();},0,200);}
...
s='mute';if(q[s])muted_initially=parseInt(q[s]);s='wf';if(q[s])wf_rate=q[s];s='wfm';if(q[s])wf_mm=q[s];s='cmap';if(q[s])wf.cmap_override=w3_clamp(parseInt(q[s]),0,kiwi.cmap_s.length-1,0);s='sqrt';if(q[s])wf.sqrt=w3_clamp(parseInt(q[s]),0,4,0);s='wfts';if(q[s])wf.url_tstamp=w3_clamp(parseInt(q[s]),2,60*60,2);s='peak';if(q[s])peak_initially=parseInt(q[s]);s='no_geo';if(q[s])no_geoloc=true;s='keys';if(q[s])shortcut.keys=isNonEmptyString(q[s])?q[s].split(''):null;s='user';if(q[s])user_url=q[s];s='u';if(q[s])user_url=q[s];s='m';if(q[s])force_mobile=true;s='mobile';if(q[s])force_mobile=true;s='mem';if(q[s])owrx.override_fmem=q[s];s='sqth';if(q[s])squelch_threshold=parseFloat(q[s]);s='click';if(q[s])nb_click=true;s='nocache';if(q[s]){param_nocache=true;nocache=parseInt(q[s]);}
...
snd_send("SERVER DE CLIENT openwebrx.js SND");snd_send("SET dbug_v="+debug_v);w3_do_when_cond(function(){return(isArg(rx_chan)&&owrx.cfg_loaded);},function(){if(rx_chan!=0)return;if(gen_attn!=0){var dB=gen_attn+cfg.waterfall_cal;var ampl_gain=Math.pow(10,-dB/20);gen_attn=0x01ffff*ampl_gain;}
set_gen(gen_freq,gen_attn);},0,500);var _lo=cfg.passbands[init_mode].lo;if(!isNumber(_lo))_lo=-4000;var _hi=cfg.passbands[init_mode].hi;if(!isNumber(_hi))_hi=4000;snd_send('SET mod='+init_mode+' low_cut='+_lo+' high_cut='+_hi+' freq='+init_frequency);set_agc();snd_send("SET browser="+navigator.userAgent);if(snd_winf!=0)snd_send('SET window_func='+snd_winf);wf_send("SERVER DE CLIENT openwebrx.js W/F");wf_send("SET send_dB=1");wf_send("SET zoom=0 start=0");wf_send("SET maxdb=0 mindb=-100");if(wf_compression==0)wf_send('SET wf_comp=0');if(wf_interp!=-1)wf_send('SET interp='+wf_interp);if(wf_winf!=0)wf_send('SET window_func='+wf_winf);wf_speed=wf_rates[wf_rate];if(wf_speed==undefined)wf_speed=WF_SPEED_FAST;wf_send('SET wf_speed='+wf_speed);}
...
function init_top_bar(){w3_append_innerHTML('id-rx-desc',w3_link('id-rx-gmap','','[map]','','dont_toggle_rx_photo')+
w3_div('id-rx-snr w3-show-inline'));}
var rx_photo_spacer_height=owrx.height_top_bar_parts;function init_rx_photo(){w3_el('id-top-photo-img').style.paddingLeft=RX_PHOTO_LEFT_MARGIN?'50px':0;RX_PHOTO_HEIGHT+=rx_photo_spacer_height;w3_el("id-top-photo-clip").style.maxHeight=px(RX_PHOTO_HEIGHT);if(dbgUs||kiwi_isMobile()){close_rx_photo();}else{window.setTimeout(function(){close_rx_photo()},3000);}}
var rx_photo_state=1;function open_rx_photo(){rx_photo_state=1;html("id-rx-photo-desc").style.opacity=1;html("id-rx-photo-title").style.opacity=1;animate_to(html("id-top-photo-clip"),"maxHeight","px",RX_PHOTO_HEIGHT,0.93,kiwi_isMobile()?1:1000,60,function(){resize_waterfall_container(true);});w3_hide('id-rx-details-arrow-down');w3_show_block('id-rx-details-arrow-up');}
function close_rx_photo(){rx_photo_state=0;animate_to(html("id-top-photo-clip"),"maxHeight","px",rx_photo_spacer_height,0.93,kiwi_isMobile()?1:1000,60,function(){resize_waterfall_container(true);});w3_show_block('id-rx-details-arrow-down');w3_hide('id-rx-details-arrow-up');}
...
function animate_to(object,style_name,unit,to,accel,time_ms,fps,to_exec){from=parseFloat(style_value(object,style_name));animate(object,style_name,unit,from,to,accel,time_ms,fps,to_exec);}
...
demodulator_response_time=100;function demodulator_default_analog(offset_frequency,subtype,locut,hicut){if(cfg.passbands[subtype]==null)subtype='am';demodulator.call(this,offset_frequency);this.subtype=subtype;this.envelope.dragged_range=demodulator.draggable_ranges.none;var sampleRateDiv2=audio_input_rate?audio_input_rate/2:5000;this.filter={min_passband:4,high_cut_limit:sampleRateDiv2,low_cut_limit:-sampleRateDiv2};this.server_mode=subtype;var lo=locut;if(isNaN(lo)){lo=owrx.last_lo[subtype];if(isNaN(lo)){lo=cfg.passbands[subtype].lo;}}
owrx.last_lo[subtype]=lo;var hi=hicut;if(isNaN(hi)){hi=owrx.last_hi[subtype];if(isNaN(hi)){hi=cfg.passbands[subtype].hi;}}
...
this.doset=function(){var freq=(freq_car_Hz/1000).toFixed(3);var mode=this.server_mode;var locut=this.low_cut.toString();var hicut=this.high_cut.toString();var mparam=(mode=='sam')?(' param='+owrx.chan_null):'';var s='SET mod='+mode+' low_cut='+locut+' high_cut='+hicut+' freq='+freq+mparam;snd_send(s);var changed=null;if(freq!=owrx.last_freq){changed=changed||{};changed.freq=1;owrx.last_freq=freq;}
...
this.envelope.parent=this;this.envelope.draw=function(visible_range){this.visible_range=visible_range;this.drag_ranges=demod_envelope_draw(g_range,center_freq+this.parent.offset_frequency+this.parent.low_cut,center_freq+this.parent.offset_frequency+this.parent.high_cut,this.color,center_freq+this.parent.offset_frequency);var bw=Math.abs(this.parent.high_cut-this.parent.low_cut);pb_adj_lo_ttip.innerHTML='lo '+this.parent.low_cut.toString()+', bw '+bw.toString();pb_adj_hi_ttip.innerHTML='hi '+this.parent.high_cut.toString()+', bw '+bw.toString();pb_adj_cf_ttip.innerHTML='cf '+(this.parent.low_cut+Math.abs(this.parent.high_cut-this.parent.low_cut)/2).toString()+', bw '+bw.toString();var f_kHz=(center_freq+this.parent.offset_frequency)/1000+kiwi.freq_offset_kHz;pb_adj_car_ttip.innerHTML=f_kHz.toFixed(freq_field_prec(f_kHz))+' kHz';};this.envelope.env_drag_start=function(x,y,key_modifiers){this.key_modifiers=key_modifiers;this.dragged_range=demod_envelope_where_clicked(x,y,this.drag_ranges,key_modifiers);this.drag_origin={x:x,low_cut:this.parent.low_cut,high_cut:this.parent.high_cut,offset_frequency:this.parent.offset_frequency};return this.dragged_range!=demodulator.draggable_ranges.none;};this.envelope.drag_move=function(x){var dr=demodulator.draggable_ranges;if(this.dragged_range==dr.none){return false;}
...
mkenvelopes(this.visible_range);freqset_car_Hz(this.parent.offset_frequency+center_freq);this.parent.set();freqset_update_ui(owrx.FSET_PB_CHANGE);return true;};this.envelope.drag_end=function(x){to_return=this.dragged_range!=demodulator.draggable_ranges.none;this.dragged_range=demodulator.draggable_ranges.none;return to_return;};}
demodulator_default_analog.prototype=new demodulator();function mkenvelopes(visible_range){scale_ctx.clearRect(0,0,scale_ctx.canvas.width,22);for(var i=0;i<demodulators.length;i++){demodulators[i].envelope.draw(visible_range);}}
...
function demodulator_analog_replace(subtype,freq){if(cfg.passbands[subtype]==null)subtype='am';var offset=0,prev_pbo=0;var wasCW=false,toCW=false,fromCW=false;if(demodulators.length){wasCW=demodulators[0].isCW;offset=demodulators[0].offset_frequency;prev_pbo=passband_offset();demodulator_remove(0);}else{var i_freqHz=Math.round((init_frequency-kiwi.freq_offset_kHz)*1000);offset=(i_freqHz<=0||i_freqHz>bandwidth)?0:(i_freqHz-center_freq);subtype=init_mode;}
...
evt.preventDefault();}
function scale_canvas_end_drag(evt,x,canvas_mouse_up){if(scale_canvas_ignore_mouse_event){scale_canvas_ignore_mouse_event=false;return;}
...
function get_visible_freq_range(){out={};if(wf.audioFFT_active&&cur_mode){var off,span;var srate=Math.round(audio_input_rate||12000);if(ext_is_IQ_or_stereo_curmode()){off=0;span=srate;}else
if(cur_mode&&(cur_mode.substr(0,2)=='ls'||cur_mode=='sal')){off=0;span=srate/2;}else{off=srate/4;span=srate/2;}
...
function mouse_freq_add(evt){if(isNumber(evt.pageX))owrx.last_pageX=evt.pageX;if(isNumber(evt.pageY))owrx.last_pageY=evt.pageY;if(kiwi_isMobile()||wf.audioFFT_active)return;var ctx=canvas_annotation.ctx;if(evt.target==canvas_annotation&&any_modifier_key(evt)){var cx=evt.offsetX;var cy=evt.offsetY+16;var tw,th=12;ctx.font=px(th)+' monospace';if(owrx.mouse_freq.vis)
ctx.clearRect(owrx.mouse_freq.tx,owrx.mouse_freq.cy,owrx.mouse_freq.tw,th);var f_kHz=(canvas_get_dspfreq(cx)+kiwi.freq_offset_Hz)/1000;owrx.mouse_freq.text=format_frequency("{x}",f_kHz,1,freq_field_prec(f_kHz));tw=Math.ceil(ctx.measureText(owrx.mouse_freq.text).width);if(tw&1)tw++;owrx.mouse_freq.tw=tw;owrx.mouse_freq.th=th;owrx.mouse_freq.tx=cx-tw/2;ctx.fillStyle='black';ctx.fillRect(cx-tw/2,cy,tw,th);ctx.fillStyle='white';ctx.fillText(owrx.mouse_freq.text,owrx.mouse_freq.tx,cy+th-2);owrx.mouse_freq.cx=cx;owrx.mouse_freq.cy=cy;owrx.mouse_freq.vis=true;}else{mouse_freq_remove();}}
function mouse_freq_remove(){if(owrx.mouse_freq.vis){canvas_annotation.ctx.clearRect(owrx.mouse_freq.tx,owrx.mouse_freq.cy,owrx.mouse_freq.tw,owrx.mouse_freq.th);owrx.mouse_freq.vis=false;}}
...
function export_waterfall(){var fscale=w3_el('id-scale-canvas');var wfW=waterfall_width;var fscaleW=wfW;var fscaleH=Math.round(fscale.height);var legendH=50;var h=legendH;var specC=w3_el('id-spectrum-canvas');var specH=(specC&&spec.source_wf)?specC.height:0;var canvas=document.createElement("canvas");var ctx=canvas.getContext("2d");var canW=canvas.width=wf_fft_size;var canH=canvas.height=legendH+fscaleH+specH+(old_canvases.length+wf_canvases.length)*wf_canvas_default_height;ctx.fillStyle="black";ctx.fillRect(0,0,canW,canH);if(specH){ctx.drawImage(specC,0,h);h+=specH;}
ctx.fillStyle="gray";ctx.fillRect(0,h,canW,fscaleH);ctx.drawImage(fscale,0,0,fscaleW,fscaleH,0,h,canW,fscaleH);h+=fscaleH;wf_canvases.forEach(function(wf_c,i){var wfH=wf_c.height;var wfY=0;if(i==0){var top=unpx(wf_c.style.top);wfH=wfH+top;wfY=-top;}
ctx.drawImage(wf_c,0,wfY,canW,wfH,0,h,canW,wfH);h+=wfH;});if(old_canvases.length>1){for(i=1;i<old_canvases.length;i++){ctx.drawImage(old_canvases[i],0,h);h+=old_canvases[i].height;}}
ctx.font="18px Arial";ctx.fillStyle="lime";var flabel=kiwi_host()+'     '+(new Date()).toUTCString()+'     '+freq_displayed_kHz_str+' kHz  '+cur_mode;var x=canW/2-ctx.measureText(flabel).width/2;ctx.fillText(flabel,x,35);var fileName=kiwi_host()+'_'+new Date().toISOString().replace(/:/g,'_').replace(/\.[0-9]+Z$/,'Z')+'_'+w3_el('id-freq-input').value+'_'+cur_mode+'.jpg';var imgURL=canvas.toDataURL("image/jpeg",0.85);var dlLink=document.createElement('a');dlLink.download=fileName;dlLink.href=imgURL;dlLink.target='_blank';dlLink.dataset.downloadurl=["image/jpeg",dlLink.download,dlLink.href].join(':');document.body.appendChild(dlLink);dlLink.click();document.body.removeChild(dlLink);}
...
console.log("ZTB-user f="+f+" cf="+cf+" b="+b1.name+" z="+b2.zoom_level);}else{var ITU_region=cfg.init.ITU_region+1;for(i=0;i<cfg.bands.length;i++){b1=cfg.bands[i];b2=kiwi.bands[i];if(!(b1.itu==kiwi.BAND_SCALE_ONLY||b1.itu==kiwi.ITU_ANY||b1.itu==ITU_region))continue;if(f>=b2.minHz&&f<=b2.maxHz){break;}}
if(i!=cfg.bands.length){zoom_level=b2.zoom_level;cf=b2.cfHz;}else{zoom_level=ZOOM_BAND;cf=f;}}
...
x_bin=clamp_xbin(x_bin);var dbins=out?(x_obin-x_bin):(x_bin-x_obin);var pixel_dx=bins_to_pixels(1,dbins,out?zoom_level:ozoom);if(sb_trace)console.log("Zs z"+ozoom+'>'+zoom_level+' b='+x_bin+'/'+x_obin+'/'+dbins+' bz='+bins_at_zoom(ozoom)+' r='+(dbins / bins_at_zoom(ozoom))+' px='+pixel_dx);var dz=zoom_level-ozoom;if(sb_trace)console.log('zoom_step oz='+ozoom+' zl='+zoom_level+' dz='+dz+' pdx='+pixel_dx);waterfall_zoom_canvases(dz,pixel_dx);mkscale();dx_schedule_update();if(sb_trace)console.log("SET Z"+zoom_level+" xb="+x_bin);wf_send("SET zoom="+zoom_level+" start="+x_bin);need_maxmindb_update=true;writeCookie('last_zoom',zoom_level);freq_link_update();vfo_update();zoom_finally();if(dbgUs){var f=get_visible_freq_range();console.log('^ZOOM z'+zoom_level+' '+(f.start/1e3).toFixed(0)+'|'+
(f.center/1e3).toFixed(0)+'|'+(f.end/1e3).toFixed(0)+' ('+(f.bw/1e3).toFixed(0)+' kHz)');}}
function passband_increment(wider){var pb=ext_get_passband();var pb_width=Math.abs(pb.high-pb.low);var pb_inc;if(wider)
pb_inc=((pb_width*(1/0.80))-pb_width)/ 2;else
pb_inc=(pb_width-(pb_width*0.80))/ 2;var rnd=(pb_inc>10)?10:1;pb_inc=Math.round(pb_inc/rnd)*rnd;pb.low+=wider?-pb_inc:pb_inc;pb.low=Math.round(pb.low/rnd)*rnd;pb.high+=wider?pb_inc:-pb_inc;pb.high=Math.round(pb.high/rnd)*rnd;ext_set_passband(pb.low,pb.high,true);}
function zoom_click(evt,dir,arg2){if(any_alternate_click_event(evt)){var div=evt.currentTarget;var zin=w3_contains(div,'id-zoom-in');var zout=w3_contains(div,'id-zoom-out');if(!zin&&!zout)return;passband_increment(zin);return;}
if(dir==ext_zoom.NOM_IN)
zoom_step(dir,1);else
zoom_step(dir);}
function zoom_over(evt){if(evt.target.nodeName!='IMG')return;var img=evt.target;var div=evt.currentTarget;var zin=w3_contains(div,'id-zoom-in');if(any_alternate_click_event(evt)){img.title=zin?'passband widen':'passband narrow';}else{img.removeAttribute('title');}}
...
function spectrum_tooltip_update(evt,clientX,clientY){var target=(evt.target==spec.dB||evt.currentTarget==spec.dB||evt.target==spec.dB_ttip||evt.currentTarget==spec.dB_ttip);if(target){if(owrx.debug_drag)canvas_log('*STTU*');var h=spec.height_spectrum_canvas;if(clientY>=0&&clientY<h){var cx=clientX-(kiwi_isMobile()?spec.tooltip_offset:0);spec.dB_ttip.style.left=px(cx);spec.dB_ttip.style.bottom=px(h+10-clientY);if(owrx.debug_drag)canvas_log('['+cx+','+clientY+']');var f_kHz=(canvas_get_dspfreq(cx)+kiwi.freq_offset_Hz)/1000;var f_text=format_frequency("{x}",f_kHz,1,freq_field_prec(f_kHz));var dB=(((h-clientY)/ h)*full_scale)+mindb;spec.dB_ttip.innerHTML=f_text+' kHz<br>'+dB.toFixed(0)+' dBm';}}else{if(owrx.debug_drag)canvas_log('STTU-ck');}}
function spectrum_update(data){var i,x,y,z,sw,sh,tw=25;spec.last_update=spec.update;var ctx=spec.ctx;sw=spec.canvas.width-tw;sh=spec.canvas.height;ctx.fillStyle="black";ctx.fillRect(0,0,sw,sh);ctx.fillStyle="lightGray";for(i=0;i<spec.dB_bands.length;i++){var band=spec.dB_bands[i];y=Math.round(band.norm*sh);ctx.fillRect(0,y,sw,1);}
if(spec.clear_avg){for(x=0;x<sw;x++){spec.avg[x]=color_index(data[x]);}
spec.clear_avg=false;spec.peak_clear=true;}
if(spec.peak_clear){for(x=0;x<sw;x++){spec.peak[x]=0;}
spec.peak_clear=false;}
if(spec.redraw_dB_scale){for(x=sw;x<spec.canvas.width;x++){ctx.putImageData(spec.colormap_transparent,x,0,0,0,1,sh);}
ctx.fillStyle="white";for(i=0;i<spec.dB_bands.length;i++){var band=spec.dB_bands[i];y=Math.round(band.norm*sh);ctx.fillText(band.dB,sw+3,y-4);}
spec.redraw_dB_scale=false;}
ctx.fillStyle='hsl(180, 100%, 70%)';for(x=0;x<sw;x++){z=color_index(wf_gnd?wf_gnd_value:data[x]);switch(spec_filter){case wf_sp_menu_e.IIR:var iir_gain=1-Math.exp(-sp_param*z/255);if(iir_gain<=0.01)iir_gain=0.01;var z1=spec.avg[x];z1=z1+iir_gain*(z-z1);if(z1>255)z1=255;if(z1<0)z1=0;z=spec.avg[x]=z1;break;case wf_sp_menu_e.MMA:if(z>255)z=255;if(z<0)z=0;spec.avg[x]=((spec.avg[x]*(sp_param-1))+z)/ sp_param;z=spec.avg[x];break;case wf_sp_menu_e.EMA:if(z>255)z=255;if(z<0)z=0;spec.avg[x]+=(z-spec.avg[x])/ sp_param;z=spec.avg[x];break;case wf_sp_menu_e.OFF:default:if(z>255)z=255;if(z<0)z=0;break;}
if(z>spec.peak[x])spec.peak[x]=z;y=Math.round((1-z/255)*sh);if(spectrum_slow_dev){ctx.fillRect(x,y,1,sh-y);}else{var first=true;for(i=0;i<spec.dB_bands.length;i++){var b=spec.dB_bands[i];if(first&&y>b.y2)continue;first=false;ctx.fillStyle=b.color;var h=b.y2-y;ctx.fillRect(x,y,1,h);y=b.y2;}}}
if(spec.peak_show){ctx.lineWidth=1;ctx.strokeStyle='yellow';ctx.beginPath();y=Math.round((1-spec.peak[0]/255)*sh)-1;ctx.moveTo(0,y);for(x=1;x<sw;x++){y=Math.round((1-spec.peak[x]/255)*sh)-1;ctx.lineTo(x,y);}
ctx.globalAlpha=0.55;ctx.stroke();ctx.globalAlpha=1;}}
var waterfall_setup_done=0;var waterfall_timer;var waterfall_ms;var waterfall_scrollable_height;var wf_canvases=[];var old_canvases=[];var wf_cur_canvas=null;var wf_canvas_default_height=200;var wf_canvas_actual_line;var wf_canvas_id_seq=1;var wf_canvas_maxshift=0;function wf_init(){w3_do_when_cond(function(){return okay_wf_init;},function(){wf_init_ready();},0,200);}
...
var pp=wf_canvases[wf_canvases.length-2];if(pp)old_canvases.unshift(pp);if(old_canvases.length>15)old_canvases.pop();if(kiwi_gc_wf)remove_canvas_listner(p);wf_container.removeChild(p);if(kiwi_gc_wf)p=wf_canvases[wf_canvases.length-1]=null;wf_canvases.pop();}
...
var waterfall_dont_scale=0;var need_maxmindb_update=false;var need_clear_wf_sp_avg=false;var need_clear_wfavg=false,clear_wfavg=true;var wfavg=[];var wf_swallow_samples=[2,4,8,18];var x_bin_server_last,wf_swallow=0;function waterfall_add(data_raw,audioFFT){var x,y,z;if(data_raw==null)return;var canvas=wf_cur_canvas;if(canvas==null)return;var data_arr_u8,data,decomp_data;var w=wf_fft_size;if(audioFFT==0){var u32View=new Uint32Array(data_raw,4,3);var x_bin_server=u32View[0];var u32=u32View[1];if(kiwi_gc_wf)u32View=null;var x_zoom_server=u32&0xffff;var flags=(u32>>16)&0xffff;var wf_flags={COMPRESSED:1,NO_SYNC:2};data_arr_u8=new Uint8Array(data_raw,16);var bytes=data_arr_u8.length;if(need_maxmindb_update&&zoom_level==x_zoom_server){update_maxmindb_sliders();need_maxmindb_update=false;}
...
if(x_bin_server!=x_bin_server_last){x_bin_server_last=x_bin_server;if(zoom_level>=11){wf_swallow=wf_swallow_samples[zoom_level-11];if(wf_fps!=wf_fps_max){wf_swallow=Math.round(wf_swallow*wf_fps / wf_fps_max);}}}}else{data=data_raw;if(spec.source_wf&&spec.need_clear_avg){spec.clear_avg=true;spec.need_clear_avg=false;}}
if(spec.source_wf&&spec.update!=spec.last_update){spectrum_update(data);}
...
signal+=30;if(signal<-80)signal=-80;noise-=10;if(wf.audioFFT_active){noise=(dbgUs&&devl.p4)?Math.round(devl.p4):-110;console_log_dbgUs('audioFFT_active: force noise = '+noise.toFixed(0)+' dBm');}
...
function dB_wire_to_dBm(db_value){if(db_value<0)db_value=0;if(db_value>255)db_value=255;var dBm=-(255-db_value);return(dBm+cfg.waterfall_cal);}
...
return freq;}
function passband_offset_dxlabel(mode){var pb=cfg.passbands[mode];var m=mode.substr(0,2);var usePBCenter=(m=='us'||m=='ls');var offset=usePBCenter?pb.lo+(pb.hi-pb.lo)/2:0;return offset;}
function freqmode_set_dsp_kHz(fdsp,mode,opt){var dont_clear_wf=w3_opt(opt,'dont_clear_wf',false);var open_ext=w3_opt(opt,'open_ext',false);var no_set_freq=w3_opt(opt,'no_set_freq',0);fdsp*=1000;if(dont_clear_wf==false)wf.audioFFT_clear_wf=true;if(isArg(mode)&&(mode!=cur_mode||open_ext==true)){ext_set_mode(mode,fdsp,opt);}else{freq_car_Hz=freq_dsp_to_car(fdsp);if(!no_set_freq){demodulator_set_offset_frequency(owrx.FSET_SET_FREQ,freq_car_Hz-center_freq);}}}
function freqset_car_Hz(fcar){freq_car_Hz=fcar;}
var freq_dsp_set_last;function freq_field_prec(f_kHz){var limit=100e3-(cfg.max_freq?32e3:30e3);var prec=(kiwi.freq_offset_kHz>=limit)?2:(cfg.show_1Hz?3:2);return prec;}
function freq_field_width(){var limit=100e3-(cfg.max_freq?32e3:30e3);var width=(kiwi.freq_offset_kHz>=limit)?6.25:(cfg.show_1Hz?6.25:5.75);return(width+'em');}
function freqset_update_ui(from){freq_displayed_Hz=freq_car_to_dsp(freq_car_Hz);var f_kHz=freq_displayed_Hz/1000;freq_displayed_kHz_str=f_kHz.toFixed(freq_field_prec(f_kHz));if(!waterfall_setup_done)return;var obj=w3_el('id-freq-input');if(isUndefined(obj)||obj==null)return;var f_kHz_with_freq_offset=(freq_displayed_Hz+kiwi.freq_offset_Hz)/1000;freq_displayed_kHz_str_with_freq_offset=f_kHz_with_freq_offset.toFixed(freq_field_prec(f_kHz_with_freq_offset));obj.value=freq_displayed_kHz_str_with_freq_offset;freqset_select();var pb_bin=-passband_visible()-1;if(pb_bin>=0){var wf_middle_bin=x_bin+bins_at_cur_zoom()/2;waterfall_pan_canvases(pb_bin-wf_middle_bin);}
check_band();writeCookie('last_freq',freq_displayed_kHz_str_with_freq_offset);freq_dsp_set_last=freq_displayed_kHz_str_with_freq_offset;freq_step_update_ui();freq_link_update();vfo_update();if(from!=owrx.FSET_NOP&&from!=owrx.FSET_SCALE_DRAG&&from!=owrx.FSET_SCALE_TOUCH_DRAG){freq_memory_update(freq_displayed_kHz_str_with_freq_offset);}
...
function modeset_update_ui(mode){if(owrx.last_mode_el!=null)owrx.last_mode_el.style.color="white";var m=mode.substr(0,2);if(m=='qa')m='sa';var el=w3_el('id-mode-'+m);el.innerHTML=mode.toUpperCase();if(el&&el.style)el.style.color="lime";owrx.last_mode_el=el;squelch_setup(toggle_e.FROM_COOKIE);writeCookie('last_mode',mode);freq_link_update();vfo_update();var disabled=ext_is_IQ_or_stereo_mode(mode);w3_els('id-button-compression',function(el){w3_set_props(el,'w3-disabled',disabled);});w3_hide2('id-sam-carrier-container',m!='sa');w3_hide2('id-chan-null',mode!='sam');}
...
function freqset_complete(from){var obj=w3_el('id-freq-input');kiwi_clearTimeout(freqset_tout);if(isUndefined(obj)||obj==null)return;var iPhone=kiwi_is_iPhone();var a,f,pb;var adj_pbw=obj.value.includes('/');var adj_pbc=obj.value.includes(':');if(!iPhone||adj_pbw||adj_pbc){a=obj.value.split(/[\/\:]/);f=a[0];pb=(a.length>=2)?a[1].split(','):null;}else{a=obj.value.split('.');f=(a.length==1)?a[0]:(a[0]+'.'+a[1]);pb=a;pb.shift();pb.shift();if(pb.length==0)pb=null;adj_pbw=true;}
...
var restore=true;if(f!=''){f=f.replace(',','.').parseFloatWithUnits('M',1e-3);if(f>0&&!isNaN(f)){f-=kiwi.freq_offset_kHz;if(f>0&&!isNaN(f)){freqmode_set_dsp_kHz(f,null);w3_field_select(obj,{mobile:1,log:2});restore=false;}}}
...
var num_step_buttons=6;var up_down={am:[0,-1,-0.1,0.1,1,0],amn:[0,-1,-0.1,0.1,1,0],sam:[0,-1,-0.1,0.1,1,0],sal:[0,-1,-0.1,0.1,1,0],sau:[0,-1,-0.1,0.1,1,0],sas:[0,-1,-0.1,0.1,1,0],qam:[0,-1,-0.1,0.1,1,0],drm:[0,-1,-0.1,0.1,1,0],usb:[0,-0.1,-0.01,0.01,0.1,0],usn:[0,-0.1,-0.01,0.01,0.1,0],lsb:[0,-0.1,-0.01,0.01,0.1,0],lsn:[0,-0.1,-0.01,0.01,0.1,0],cw:[0,-0.1,-0.01,0.01,0.1,0],cwn:[0,-0.1,-0.01,0.01,0.1,0],nbfm:[-5,-1,-0.1,0.1,1,5],iq:[0,-1,-0.1,0.1,1,0],};var NDB_400_1000_mode=1;function freq_step_amount(b){var cm=cur_mode.substr(0,2);var ssb_cw=(cm=='ls'||cm=='us'||cm=='cw');var step_Hz=ssb_cw?1000:5000;var s=ssb_cw?' 1k default':' 5k default';var am_sax_iq_drm=(cm=='am'||cm=='sa'||cm=='qa'||cm=='ls'||cm=='us'||cm=='iq'||cm=='dr');var ITU_region=cfg.init.ITU_region+1;var ham_80m_swbc_75m_overlap=(ITU_region==2&&b&&b.name=='75m');if(b&&b.name=='NDB'){if(cm=='cw'){step_Hz=NDB_400_1000_mode;}
s=' NDB';}else
if(b&&(b.name=='LW'||b.name=='MW')){if(am_sax_iq_drm){step_Hz=step_9_10?9000:10000;}
s=' LW/MW';}else{var svc=b?band_svc_lookup(b.svc):null;if(svc&&svc.o.name.includes('Broadcast')&&!ham_80m_swbc_75m_overlap){if(am_sax_iq_drm){step_Hz=5000;s=' SWBC 5k';}}else
...
var freq_step_last_mode,freq_step_last_band;function freq_step_update_ui(force){if(isUndefined(cur_mode)||cfg.passbands[cur_mode]==undefined)return;var b=find_band(freq_displayed_Hz);if(!force&&freq_step_last_mode==cur_mode&&freq_step_last_band==b){return;}
var cm=cur_mode.substr(0,2);var am_sax_iq_drm=(cm=='am'||cm=='sa'||cm=='qa'||cm=='iq'||cm=='dr');var show_9_10=(b&&(b.name=='LW'||b.name=='MW')&&am_sax_iq_drm)?true:false;if(kiwi_isMobile()){w3_hide2('id-9-10-cell',!show_9_10);}else{w3_hide2('id-9-10-cell',false);w3_disable('id-9-10-cell',!show_9_10);}
...
function freq_memory_init(){var fmem;if(isNonEmptyString(owrx.override_fmem)){fmem=owrx.override_fmem.split(',');var prec=cfg.show_1Hz?3:2;fmem.forEach(function(s,i){if(isNumber(s))s=s.toString();var f=s.parseFloatWithUnits('kM',1e-3);if(isNumber(f))
fmem[i]=w3_clamp(f,1,cfg.max_freq?32000:30000).toFixed(prec);});}else{fmem=kiwi_JSON_parse('freq_memory_init',readCookie('freq_memory'));}
if(fmem){owrx.freq_memory=kiwi_dedup_array(fmem,function(v){var rv={err:false};v=+v;if(!isNumber(v)||v<=0)
rv.err=true;else
rv.v=v.toString();return rv;});}}
function freq_memory_menu_init(){w3_menu('id-freq-memory-menu','freq_memory_menu_cb');}
function freq_memory_menu_show(shortcut_key){if(w3_isVisible('id-freq-memory-menu')){owrx.freq_memory_menu_shown=0;return;}
owrx.freq_memory_menu_shown=1;var x=owrx.last_pageX,y=owrx.last_pageY;if(shortcut_key!=true)
x+=((x-128)>=0)?-128:16;var fmem_copy=kiwi_dup_array(owrx.freq_memory);fmem_copy.push('clear all');fmem_copy.push('VFO A=B');w3_menu_items('id-freq-memory-menu',fmem_copy,Math.min(fmem_copy.length,10));w3_menu_popup('id-freq-memory-menu',function(evt,first){var close_keyup=(evt.type=='keyup'&&(evt.key!='m'||!owrx.freq_memory_menu_shown));var tgt_isMenu=w3_contains(evt.target,'w3-menu');var tgt_isMenuButton=w3_contains(evt.target,'w3-menu-button');var menu_shown=owrx.freq_memory_menu_shown;var click=((evt.type=='click'||((evt.type=='mousedown'||evt.type=='touchstart')&&!tgt_isMenuButton)||evt.type=='touchend'));var close_click=(click&&!first&&(!tgt_isMenu||!menu_shown));var close=(close_keyup||close_click);return close;},x,y);}
function freq_memory_update(f){if(!isNumber(+f))return;if(f<1)f='1';if(f!=owrx.freq_memory[0]){owrx.freq_memory.unshift(f);owrx.freq_memory=kiwi_dedup_array(owrx.freq_memory);}
if(owrx.freq_memory.length>25)owrx.freq_memory.pop();writeCookie('freq_memory',JSON.stringify(owrx.freq_memory));}
function freq_memory_menu_cb(idx,x){idx=+idx;if(idx==-1)return;var f,set_f=false;if(idx==owrx.freq_memory.length){f=owrx.freq_memory[0];set_f=true;owrx.freq_memory=[f];}else
if(idx==owrx.freq_memory.length+1){vfo_update(true);}else
if(idx<=owrx.freq_memory.length-1){f=owrx.freq_memory[idx];set_f=true;owrx.freq_memory.unshift(f);owrx.freq_memory=kiwi_dedup_array(owrx.freq_memory);if(owrx.freq_memory.length>25)owrx.freq_memory.pop();}
if(set_f){freqmode_set_dsp_kHz(f-kiwi.freq_offset_kHz);}}
...
function band_svc_lookup(svc_key){var idx=null;cfg.band_svc.forEach(function(a,i){if(a.key==svc_key)
idx=i;});if(idx==null){console.log('$$$ band_svc_lookup NO KEY MATCH for <'+svc_key+'>');return null;}
return{o:cfg.band_svc[idx],i:idx};}
function bands_init(){var i,j,k,z,b,bnew;if(cfg.bands&&cfg.band_svc&&cfg.dx_type){console.log('BANDS: using stored cfg.bands');bands_addl_info();dx_color_init();return;}
console.log('BANDS: using old config.js::bands');cfg.dx_type=[];var max_i=-1;dx.legacy_stored_types.forEach(function(a,i){cfg.dx_type[i]={key:i,name:dx.legacy_stored_types[i],color:dx.legacy_stored_colors[i]};max_i=i;});j=dx_type2idx(dx.DX_MASKED);for(i=max_i+1;i<j;i++){cfg.dx_type[i]={key:i,name:'',color:''};}
cfg.dx_type[j]={key:i,name:'masked',color:'lightGrey'};dx_color_init();cfg.band_svc=[];w3_obj_enum(svc,function(key,i,o){if(key=='X'&&o.name=='Beacons'){svc.L=svc.X;key='L';delete svc.X;}
console.log('key='+key);svc[key].key=key;cfg.band_svc[i]={key:key,name:o.name,color:o.color};if(o.name=='Industrial/Scientific'){cfg.band_svc[i].name='ISM';cfg.band_svc[i].longName='Industrial/Scientific';}});kiwi.cfg_bands_new=[];for(i=j=0;i<bands.length;i++){b=bands[i];if(b.name=='LW'&&b.region=='E')b.region='1';if(b.name=='LW'&&b.region=='>')b.region='23';if(b.name=='MW'&&(b.region=='3'||b.region=='>3'))b.region='13';if(b.name=='NDB'&&b.region=='E')b.region='1';if(b.name=='NDB'&&b.region=='U')b.region='2';if(b.name=='NDB'&&b.region=='>')b.region='3';if(b.name=='MF'&&b.region=='>23')b.region='*';if(b.name.startsWith('Time ')&&b.region=='*')b.region='m';if(b.region){var len=b.region.length;if(len==1){kiwi.cfg_bands_new[j]=kiwi_shallow_copy(b);bnew=kiwi.cfg_bands_new[j];j++;var c=bnew.region.charAt(0);if(c>='1'&&c<='3')
...
if(c=='-')
bnew.itu=kiwi.BAND_SCALE_ONLY;else{bnew.itu=kiwi.BAND_SCALE_ONLY;}
console.log('R'+c+'='+bnew.itu+' '+dq(b.name));console.log(bnew);}else{for(k=0;k<len;k++){var c=b.region.charAt(k);if(c=='>')continue;kiwi.cfg_bands_new[j]=kiwi_shallow_copy(b);bnew=kiwi.cfg_bands_new[j];j++;if(c>='1'&&c<='3')
bnew.itu=ord(c,'1')+1;else{bnew.itu=kiwi.BAND_SCALE_ONLY;}
console.log('R'+c+'='+bnew.itu+' '+dq(b.name));console.log(bnew);}}}else{kiwi.cfg_bands_new[j]=kiwi_shallow_copy(b);bnew=kiwi.cfg_bands_new[j];bnew.itu=kiwi.ITU_ANY;j++;console.log('DEFAULT ITU_ANY='+bnew.itu+' '+dq(b.name));console.log(bnew);}
...
bnew.s='L';}
for(i=0;i<kiwi.cfg_bands_new.length;i++){bnew=kiwi.cfg_bands_new[i];if(isString(bnew.s))bnew.svc=bnew.s;else
if(isObject(bnew.s))
bnew.svc=bnew.s.key;else{console.log('$$$ t/o(bnew.svc)='+typeof(bnew.s));}
delete bnew.s;delete bnew.region;}
cfg.bands=[];for(i=0;i<kiwi.cfg_bands_new.length;i++){b=kiwi.cfg_bands_new[i];var sel=b.sel||'';cfg.bands.push({min:b.min,max:b.max,name:b.name,svc:b.svc,itu:b.itu,sel:sel,chan:b.chan});}
console.log('kiwi.cfg_bands_new');console.log(kiwi.cfg_bands_new);bands_addl_info();}
function bands_addl_info(){var i,z,b1,b2;var offset_kHz=kiwi.freq_offset_kHz;console.log('BANDS: creating additional info in kiwi.bands');kiwi.bands=[];for(i=0;i<cfg.bands.length;i++){b1=cfg.bands[i];b2=kiwi.bands[i]={};cfg.bands[i].chan=isUndefined(b1.chan)?0:b1.chan;var min=b1.min,max=b1.max;if(offset_kHz&&b1.min>=offset_kHz){min-=offset_kHz;max-=offset_kHz;b2.isOffset=true;}else
if(b1.min>32000)
...
var band_menu=[];function setup_band_menu(){var i,op=0,service=null;var s='<option value="0" selected disabled>select band</option>';band_menu[op++]=null;var ITU_region=cfg.init.ITU_region+1;for(i=0;i<cfg.bands.length;i++){var b1=cfg.bands[i];var b2=kiwi.bands[i];if(kiwi.isOffset){if(!b2.isOffset)continue;}else{if(b2.isOffset)continue;}
...
function find_band(freq){var ITU_region=cfg.init.ITU_region+1;for(var i=0;i<cfg.bands.length;i++){var b1=cfg.bands[i];var b2=kiwi.bands[i];if(!(b1.itu==kiwi.BAND_SCALE_ONLY||b1.itu==kiwi.ITU_ANY||b1.itu==ITU_region))continue;var max=(b1.name=='MW')?1710000:b2.maxHz;if(freq>=b2.minHz&&freq<=max){return b1;}}
return null;}
band_canvas_h=30;band_canvas_top=0;band_scale_h=20;band_scale_top=5;band_scale_text_top=5;dx_container_h=80;dx_container_top=band_canvas_h;dx_label_top=5;dx_line_h=dx_container_h-dx_label_top;scale_canvas_h=47;scale_canvas_top=band_canvas_h+dx_container_h;scale_container_h=band_canvas_h+dx_container_h+scale_canvas_h;function mk_bands_scale(){var r=g_range;var tw=band_ctx.canvas.width;var i,x,y=band_scale_top,w,h=band_scale_h,ty=y+band_scale_text_top;var start=r.start;var end=r.end;band_ctx.globalAlpha=1;band_ctx.fillStyle="White";band_ctx.fillRect(0,band_canvas_top,tw,band_canvas_h);var ITU_region=cfg.init.ITU_region+1;for(i=0;i<cfg.bands.length;i++){var b1=cfg.bands[i];var b2=kiwi.bands[i];if(kiwi.isOffset){if(!b2.isOffset)continue;}else{if(b2.isOffset)continue;}
...
function band_scroll(dir){var i=owrx.last_selected_band;var b;do{i+=dir;if(i<0)i=band_menu.length-1;if(i>=band_menu.length)i=0;b=band_menu[i];}while(b==null);select_band(i);w3_set_value('id-select-band',i);}
function select_band(v,mode){var b,v_num=+v;if(isNumber(v_num)){b=band_menu[v_num];}else{var i;for(i=0;i<band_menu.length-1;i++){if(band_menu[i]&&band_menu[i].b1.name==v)
break;}
if(i<band_menu.length-1)
select_band(i,mode);return;}
if(b==null){check_band(true);return;}
var b1=b.b1;var b2=b.b2;if(dbgUs){console.log(b);console.log(b1);console.log(b2);}
var freq;if(b1.sel!=''){freq=parseFloat(b1.sel);if(isUndefined(mode)){mode=b1.sel.search(/[a-z]/i);mode=(mode==-1)?null:b1.sel.slice(mode);}}else{freq=b2.cfHz/1000;}
owrx.last_selected_band=v_num;if(dbgUs){sb_trace=0;}
freqmode_set_dsp_kHz(freq,mode);zoom_step(ext_zoom.TO_BAND,b);if(sb_trace){}}
...
function confirmation_panel_show(w,h){el=w3_el('id-confirmation');el.style.zIndex=1020;if(w==undefined)w=525;if(h==undefined)h=80;confirmation_panel_resize(w,h);confirmation.displayed=!confirmation.displayed;toggle_panel('id-confirmation');}
...
function set_freq_offset_dialog(){var s=w3_col_percent('/w3-text-aqua',w3_input_get('','Frequency scale offset (kHz, 1 Hz resolution)','cfg.freq_offset','w3_float_set_cfg_cb|3'),80);confirmation_show_content(s,525,80);w3_field_select('id-cfg.freq_offset',{mobile:1});}
...
function cal_adc_confirm(){ext_send('SET clk_adj='+cal_adc_new_adj);ext_set_cfg_param('cfg.clk_adj',cal_adc_new_adj,true);confirmation_panel_close();}
function admin_pwd_query(isAdmin_true_cb){ext_hasCredential('admin',admin_pwd_cb,isAdmin_true_cb,ws_wf);}
function admin_pwd_cb(badp,isAdmin_true_cb){console.log('admin_pwd_cb badp='+badp);if(!badp){isAdmin_true_cb();return;}
...
var dx={DB_STORED:0,DB_EiBi:1,db:0,db_s:['stored (writeable)','EiBi-B21 (read-only)'],db_flip_s:['EiBi','stored'],url_p:null,help:false,ignore_dx_update:false,DX_STEP:2,DX_JSON:0,DX_CSV:1,IDX_USER:null,UPD_MOD:0,UPD_ADD:1,o:{},legacy_stored_types:['active','watch-list','sub-band','DGPS','special event','interference'],legacy_stored_colors:['cyan','lightPink','aquamarine','lavender','violet','violet','lightGrey'],stored_colors_light:[],stored_lightness:0.95,stored_filter_tod:1,dow_colors:['lightCoral','lightSalmon','gold','yellowGreen','springGreen','deepSkyBlue','violet'],eibi_colors:[],eibi_hue:['cyan',300,'deepSkyBlue','silver',50,'peachPuff','mediumAquaMarine',270,180,70,'springGreen',0],eibi_light:[0,80,0,0,70,0,0,80,80,45,0,75],eibi_colors_light:[],eibi_lightness:0.90,eibi_svc_s:['Bcast','Utility','Time','ALE','HFDL','Milcom','CW','FSK','Fax','Aero','Maritime','Spy'],eibi_ext:['','','time','ale','hfdl','','cw','fsk','fax','','',''],eibi_filter_tod:1,DX_MODE:0x0000000f,DX_TYPE:0x000001f0,DX_TYPE_SFT:4,DX_N_STORED:16,DX_STORED:0x00000000,DX_QUICK_0:0x00000000,DX_QUICK_1:0x00000010,DX_MASKED:0x000000f0,DX_N_EiBi:12,DX_EiBi:0x00000100,DX_BCAST:0x00000100,DX_UTIL:0x00000110,DX_TIME:0x00000120,DX_ALE:0x00000130,DX_HFDL:0x00000140,DX_MILCOM:0x00000150,DX_CW:0x00000160,DX_FSK:0x00000170,DX_FAX:0x00000180,DX_AERO:0x00000190,DX_MARINE:0x000001a0,DX_SPY:0x000001b0,DX_DOW_SFT:9,DX_DOW:0x0000fe00,DX_MON:0x00008000,DX_FLAGS:0xffff0000,DX_FILTERED:0x00010000,DX_HAS_EXT:0x00020000,eibi_types_mask:0xffffffff,list:[],displayed:[],filter_ident:'',filter_notes:'',filter_case:0,filter_wild:0,filter_grep:0,ctrl_click:false,};function dx_init(){dx.DX_DOW_BASE=dx.DX_DOW>>dx.DX_DOW_SFT;if((dx.url_p=kiwi_url_param('dx','',null))!=null){dx.db=dx.DB_EiBi;w3_do_when_cond(function(){var el=w3_el('id-ext-controls');return el?el.init:false;},function(){console.log('DX url_p="'+dx.url_p+'"');if(dx.url_p!=''){var r,p=dx.url_p.split(',');p.forEach(function(a,i){console.log('DX url_p=<'+a+'>');if(w3_ext_param('help',a).match){dx.help=true;}else
if(i==0&&a==0){dx.db=dx.DB_STORED;dx.stored_filter_tod=0;freq_displayed_kHz_str='0';cur_mode='am';}else
if(w3_ext_param('none',a).match){dx.eibi_types_mask=0;console.log('SET none dx.eibi_types_mask='+dx.eibi_types_mask);}else
if((r=w3_ext_param('filter',a)).match){if(dx.db==dx.DB_STORED){dx.stored_filter_tod=r.has_value?r.num:1;console.log('SET stored_filter_tod='+dx.eibi_filter_tod+' r.has_value='+r.has_value+' r.num='+r.num);}else{dx.eibi_filter_tod=r.has_value?r.num:1;console.log('SET eibi_filter_tod='+dx.eibi_filter_tod+' r.has_value='+r.has_value+' r.num='+r.num);}}else
w3_ext_param_array_match_str(dx.eibi_svc_s,a,function(i,a){var v=dx.eibi_types_mask&(1<<i);if(v)
dx.eibi_types_mask&=~(1<<i);else
dx.eibi_types_mask|=1<<i;console.log('MATCH '+a+' v='+v.toHex(8)+'('+i+') dx.eibi_types_mask='+dx.eibi_types_mask.toHex(8));});});}
console.log('FINAL dx.eibi_types_mask='+dx.eibi_types_mask.toHex(8)+' dx.help='+dx.help);dx_show_edit_panel(null,-1);dx_schedule_update();},0,500);}}
...
function dx_color_init(){for(var i=0;i<cfg.dx_type.length;i++){var o=cfg.dx_type[i];var color=(o.color=='')?'white':o.color;var colorObj=w3color(color);if(colorObj.valid){colorObj.lightness=dx.stored_lightness;dx.stored_colors_light[i]=colorObj.toHslString();}}
dx.eibi_hue.forEach(function(color_or_hue,i){if(isString(color_or_hue)){dx.eibi_colors[i]=color_or_hue;}else{dx.eibi_colors[i]='hsl('+color_or_hue+',100%,'+dx.eibi_light[i]+'%)';}
...
function dx_update_request(){w3_do_when_cond(function(){return waterfall_setup_done;},function(){dx_update();bands_init();mk_bands_scale();mk_band_menu();},0,200);}
var dx_update_delay=350;var dx_update_timeout,dx_seq=0;function dx_schedule_update(){kiwi_clearTimeout(dx_update_timeout);dx_div.innerHTML="";dx_update_timeout=setTimeout(dx_update,dx_update_delay);}
function dx_update(){var eibi=(dx.db==dx.DB_EiBi);dx_seq++;var anti_clutter=(ext_panel_displayed('HFDL')&&dx_is_single_type(dx.DX_HFDL)&&zoom_level>=4)?0:1;wf_send('SET MARKER db='+dx.db+' min='+(g_range.start/1000).toFixed(3)+' max='+(g_range.end/1000).toFixed(3)+' zoom='+zoom_level+' width='+waterfall_width+' eibi_types_mask='+dx.eibi_types_mask.toHex()+' filter_tod='+(eibi?dx.eibi_filter_tod:dx.stored_filter_tod)+' anti_clutter='+anti_clutter);kiwi_clearTimeout(dx.dx_refresh_timeout);if((eibi&&dx.eibi_filter_tod)||(!eibi&&dx.stored_filter_tod)){var _5_min_ms=5*60*1e3;var ms_delay=2e3;var ms_rem=_5_min_ms-(Date.now()%_5_min_ms);dx.dx_refresh_timeout=setTimeout(function(){dx_div.innerHTML="";dx_update();},ms_rem+ms_delay);}}
var dx_ibp_list,dx_ibp_interval,dx_ibp_server_time_ms,dx_ibp_local_time_epoch_ms=0;var dx_ibp_freqs={14:0,18:1,21:2,24:3,28:4};var dx_ibp_lastsec=0;var dx_ibp_stations={'4U1UN':{value:0,text:'New York'},'VE8AT':{value:1,text:'Nunavut'},'W6WX':{value:2,text:'California'},'KH6RS':{value:3,text:'Hawaii'},'ZL6B':{value:4,text:'New Zealand'},'VK6RBP':{value:5,text:'Australia'},'JA2IGY':{value:6,text:'Japan'},'RR9O':{value:7,text:'Siberia'},'VR2B':{value:8,text:'Hong Kong'},'4S7B':{value:9,text:'Sri Lanka'},'ZS6DN':{value:10,text:'South Africa'},'5Z4B':{value:11,text:'Kenya'},'4X6TU':{value:12,text:'Israel'},'OH2B':{value:13,text:'Finland'},'CS3B':{value:14,text:'Madeira'},'LU4AA':{value:15,text:'Argentina'},'OA4B':{value:16,text:'Peru'},'YV5B':{value:17,text:'Venezuela'}};function dx_label_cb(arr){var i,j,k;var hdr=arr[0];var obj;var eibi=(dx.db==dx.DB_EiBi);var gap=eibi?40:35;dx_color_init();if(hdr.t==dx.DX_STEP){var dl=arr[1];if(dl){var mode=kiwi.modes_l[dl.fl&dx.DX_MODE];freqmode_set_dsp_kHz(dl.f,mode);if(dl.lo!=0&&dl.hi!=0){ext_set_passband(dl.lo,dl.hi);}else{var dpb=cfg.passbands[mode];ext_set_passband(dpb.lo,dpb.hi);}}else{}
return;}
kiwi_clearInterval(dx_ibp_interval);dx_ibp_list=[];dx_ibp_server_time_ms=hdr.s*1000+(+hdr.m);dx_ibp_local_time_epoch_ms=Date.now();if(eibi)for(i=0;i<dx.DX_N_EiBi;i++){if(isDefined(hdr.ec[i]))
w3_innerHTML('eibi-cbox'+i+'-label',dx.eibi_svc_s[i]+' ('+hdr.ec[i]+')');}
dx.types_n=hdr.tc;dx_filter_field_err(+hdr.f);var dx_idx,dx_z=120;dx_div.innerHTML='';dx.displayed=[];dx.last_ident='';dx.last_freq=-1;dx.post_render=[];dx.color_fixup=[];dx.f_same=[];var same_x,lock_z=0;var s_a=[];var optimize_label_layout=function(x,z,f){var i,j,k;var spacing=6;var n=Math.ceil(gap / spacing);if(dx.font){dx.f_same.forEach(function(idx,i){idx++;var o=arr[idx];o.di=kiwi_decodeURIComponent('',o.i);o.tw=getTextWidth(o.di,dx.font);});dx.f_same.sort(function(i1,i2){var o1=arr[i1+1];var o2=arr[i2+1];return o2.tw-o1.tw;});}
for(i=0;i<=dx.f_same.length;i++){var idx=dx.f_same[i];j=i%(n+1);dx.post_render[idx]={top:dx_label_top+(j*spacing),ltop:dx_label_top,x:x+j*(spacing-1),z:z+i};if(j==n){x+=gap;}}};for(i=1;i<arr.length;i++){dx_idx=i-1;obj=arr[i];var flags=obj.fl;var type=flags&dx.DX_TYPE;var color_idx=dx_type2idx(type);var filtered=flags&dx.DX_FILTERED;var ident=obj.i;if(eibi&&ident==dx.last_ident){if(filtered==0)dx.color_fixup[dx.last_idx]=dx.eibi_colors[color_idx];continue;}
dx.last_idx=dx_idx;dx.last_ident=ident;var gid=obj.g;var freq=obj.f;var moff=obj.o;var notes=isDefined(obj.n)?obj.n:'';var params=isDefined(obj.p)?obj.p:'';if(eibi&&(flags&dx.DX_HAS_EXT))params=dx.eibi_ext[dx_type2idx(type)];var freqHz=freq*1000;var freq_label_Hz=freqHz+passband_offset_dxlabel(kiwi.modes_l[flags&dx.DX_MODE]);var x=scale_px_from_freq(freq_label_Hz,g_range)-1;var cmkr_x=0;var carrier=freqHz-moff;if(moff)cmkr_x=scale_px_from_freq(carrier,g_range);if(eibi&&freq_label_Hz==dx.last_freq){dx.f_same.push(dx_idx);same_x=x;if(lock_z==0)lock_z=dx_z;}else{if(eibi){if(dx.f_same.length>2)optimize_label_layout(same_x,lock_z,dx.last_freq/1e3);dx.f_same=[];dx.f_same.push(dx_idx);lock_z=0;}}
var top=dx_label_top+(gap*(dx_idx&1));dx.post_render[dx_idx]={top:top,ltop:top,x:x};dx.last_freq=freq_label_Hz;var color;if(eibi){color=filtered?dx.eibi_colors_light[color_idx]:dx.eibi_colors[color_idx];}else{var c=cfg.dx_type[color_idx].color;if(c=='')c='white';color=filtered?dx.stored_colors_light[color_idx]:c;}
if(!eibi&&color_idx==0&&(ident=='IBP'||ident=='IARU%2fNCDXF')){color='aquamarine';}
carrier /=1000;dx.list[gid]={"gid":gid,"carrier":carrier,"lo":obj.lo,"hi":obj.hi,"freq":freq,"moff":moff,"flags":flags,"begin":obj.b,"end":obj.e,"ident":ident,"notes":notes,"params":params};dx.displayed[dx_idx]=dx.list[gid];var has_ext=(params!='')?' id-has-ext':'';s_a[dx_idx]='<div id="'+dx_idx+'-id-dx-label" class="cl-dx-label '+(filtered?'cl-dx-label-filtered':'')+' '+gid+'-id-dx-gid'+has_ext+'" '+'style="left:'+(x-10)+'px; z-index:'+dx_z+'; '+'background-color:'+color+';" '+'onmouseenter="dx_enter(this,'+cmkr_x+', event)" onmouseleave="dx_leave(this,'+cmkr_x+')" '+'onmousedown="ignore(event)" onmousemove="ignore(event)" onmouseup="ignore(event)" ontouchmove="ignore(event)" ontouchend="ignore(event)" '+'onclick="dx_click(event,'+gid+')" ontouchstart="dx_click(event,'+gid+')" name="'+((params=='')?0:1)+'">'+'</div>'+'<div class="cl-dx-line" id="'+dx_idx+'-id-dx-line" style="left:'+x+'px; z-index:110"></div>';dx_z++;}
if(eibi&&dx.f_same.length>2)optimize_label_layout(same_x,lock_z,-1);var s='';for(i=0;i<arr.length-1;i++)
if(isDefined(s_a[i]))
s+=s_a[i];dx_div.innerHTML=s;dx.last_ident='';if(!dx.font){var el=w3_el('0-id-dx-label');if(el){dx.font=css_style(el,'font-size')+' '+css_style(el,'font-family');console_log_dbgUs('dx.font=<'+dx.font+'>');}}
var dx_title=function(obj,stored_db){var dow_s='';var dow=obj.fl&dx.DX_DOW;if(dow==dx.DX_DOW)dow=0;if(obj.b==0&&obj.e==0){obj.e=2400;kiwi_trace('$SHOULD NOT OCCUR');}
if(dow)for(var i=0;i<7;i++){var bit=dx.DX_MON>>i;var match=bit&dow;var c=match?'MTWTFSS'[i]:'_';dow_s+=c;}else
dow_s='7-days';var s=obj.b.leadingZeros(4)+'-'+obj.e.leadingZeros(4)+' '+dow_s;if(!stored_db){var lang='';if(obj.l!=''&&obj.l.charAt(0)!='-')lang=' lang: '+obj.l;s+=lang+' target: '+obj.t;}
return s;};for(i=1;i<arr.length;i++){dx_idx=i-1;obj=arr[i];var el;var ident=obj.i;var freq=obj.f;if(eibi){if(ident==dx.last_ident){el=w3_el(dx.last_dx_idx+'-id-dx-label');el.title+='\n'+dx_title(obj);continue;}}
dx.last_dx_idx=dx_idx;dx.last_ident=ident;var notes=(isDefined(obj.n))?obj.n:'';el=w3_el(dx_idx+'-id-dx-label');if(!el)continue;var _ident=kiwi_decodeURIComponent('dx_ident2',ident).trim();if(_ident=='')_ident='&nbsp;&nbsp;';el.innerHTML=_ident.replace(/\\n/g,'<br>');var idx=dx_type2idx(obj.fl&dx.DX_TYPE);var ex=(eibi&&dx.eibi_ext[idx]!='')?'\nshift-click to open extension':'';if(eibi){el.title=dx.eibi_svc_s[idx]+' // home country: '+obj.c+ex+'\n'+dx_title(obj);}else{var title=kiwi_decodeURIComponent('dx_notes',notes);title=title.replace(/<br>/g,'\n');title=title.replace(/\\n/g,'\n');var dow=obj.fl&dx.DX_DOW;if(dow!=dx.DX_DOW||(obj.b!=0||!(obj.b==0&&obj.e==2400)))
title=title+((title!='')?'\n':'')+dx_title(obj,true);el.title=title;}
if(dx_idx<dx.post_render.length){var sparse=dx.post_render[dx_idx];if(sparse){var top=sparse.top;var left=sparse.x-10;el.style.top=px(top);el.style.left=px(left);if(isDefined(sparse.z))el.style.zIndex=sparse.z;var el_line=w3_el(dx_idx+'-id-dx-line');top=sparse.ltop;el_line.style.top=px(top);el_line.style.height=px(dx_container_h-top);}}
if(dx_idx<dx.color_fixup.length){var sparse=dx.color_fixup[dx_idx];if(sparse){el.style.backgroundColor=sparse;}}
if(!eibi&&(ident=='IBP'||ident=='IARU%2fNCDXF')){var off=dx_ibp_freqs[Math.trunc(freq / 1000)];dx_ibp_list.push({idx:dx_idx,off:off});kiwi_clearInterval(dx_ibp_interval);dx_ibp_interval=setInterval(function(){var d=new Date(dx_ibp_server_time_ms+(Date.now()-dx_ibp_local_time_epoch_ms));var min=d.getMinutes();var sec=d.getSeconds();var rsec=sec%10;if((rsec==0)&&dx_ibp_lastsec==9){var slot=(min%3)*6+Math.trunc(sec/10);for(var i=0;i<dx_ibp_list.length;i++){var s=slot-dx_ibp_list[i].off;if(s<0)s=18+s;el=w3_el(dx_ibp_list[i].idx+'-id-dx-label');var call,location;w3_obj_enum(dx_ibp_stations,function(key,i,o){if(i==s){call=key;location=o.text;}});if(el)el.innerHTML='IBP: '+call+' '+location;}}
dx_ibp_lastsec=rsec;},500);}}}
function dx_filter(){var s=w3_div('w3-medium w3-text-aqua w3-bold','DX label filter')+
...
function dx_filter_cb(path,val,first,no_close){if(first)return;w3_string_cb(path,val);var filtered=(dx.filter_ident!=''||dx.filter_notes!='');wf_send('SET DX_FILTER i='+encodeURIComponent(dx.filter_ident+'x')+' n='+encodeURIComponent(dx.filter_notes+'x')+' c='+dx.filter_case+' w='+dx.filter_wild+' g='+dx.filter_grep);w3_remove_then_add('id-dx-container','whiteSmoke cl-dx-filtered',filtered?'cl-dx-filtered':'whiteSmoke');if(!filtered&&!no_close)dx_filter_panel_close();}
function dx_filter_opt_cb(path,val,first){if(first)return;val=+val;w3_bool_cb(path,val);dx_filter_cb('dx.filter_ident',dx.filter_ident,false,true);w3_checkbox_set(path,val);if(val&&path!='dx.filter_case'){dx_filter_opt_cb((path=='dx.filter_wild')?'dx.filter_grep':'dx.filter_wild',0);}
...
function dx_label_step(dir){var i,dl;if(dir==1){for(i=0;i<dx.displayed.length;i++){dl=dx.displayed[i];if(!dl)continue;var f=Math.round(dl.freq*1e3);if(f>freq_displayed_Hz)break;}
if(i==dx.displayed.length){wf_send('SET MARKER db='+dx.db+' dir=1 freq='+(freq_displayed_Hz/1e3).toFixed(3));return;}}else{for(i=dx.displayed.length-1;i>=0;i--){dl=dx.displayed[i];if(!dl)continue;var f=Math.round(dl.freq*1e3);if(f<freq_displayed_Hz)break;}
if(i<0){wf_send('SET MARKER db='+dx.db+' dir=-1 freq='+(freq_displayed_Hz/1e3).toFixed(3));return;}}
var mode=kiwi.modes_l[dl.flags&dx.DX_MODE];freqmode_set_dsp_kHz(dl.freq,mode);if(dl.lo!=0&&dl.hi!=0){ext_set_passband(dl.lo,dl.hi);}else{var dpb=cfg.passbands[mode];ext_set_passband(dpb.lo,dpb.hi);}}
var dx_keys;function dx_admin_pwd_cb(path,val){w3_string_cb(path,val);ext_valpwd('admin',val,ws_wf);}
function dx_show_edit_panel(ev,gid,from_shortcut){dx_keys=ev?{shift:ev.shiftKey,alt:ev.altKey,ctrl:ev.ctrlKey,meta:ev.metaKey}:{shift:0,alt:0,ctrl:0,meta:0};dx.o.gid=gid;if(dx.db==dx.DB_STORED){dx.badp=-1;ext_hasCredential('admin',function(badp){dx.badp=badp;if(badp){extint_panel_hide();var s=w3_inline('',w3_div('w3-medium w3-text-aqua w3-bold','DX label edit'),w3_select('w3-margin-L-64/w3-label-inline w3-text-white /w3-text-red','database','','dx.db',dx.db,dx.db_s,'dx_database_cb'))+
w3_div('',w3_input('w3-margin-T-16//w3-margin-T-8 w3-padding-small|width:80%','Admin password','dx.pwd','','dx_admin_pwd_cb'),w3_text('w3-margin-T-4','editing the stored DX labels of this Kiwi requires admin privileges'));ext_panel_set_name('dx');ext_panel_show(s,null,null,null,true);ext_set_controls_width_height(550,260);if(from_shortcut!=true)
w3_field_select('id-dx.pwd',{mobile:1});}else{dx_show_edit_panel2();}},null,ws_wf);if(dx.badp!=0)return;}else{dx_show_edit_panel2();}}
function dx_show_edit_panel2(){var gid=dx.o.gid;if(gid==-1){dx.o.fr=freq_displayed_kHz_str;dx.o.lo=dx.o.hi=dx.o.o=0;dx.o.fm=kiwi.modes_s[cur_mode];dx.o.ft=dx.DX_QUICK_0;dx.o.fd=dx.DX_DOW_BASE;dx.o.begin=0;dx.o.end=0;dx.o.i=dx.o.n=dx.o.p='';}else{var label=dx.list[gid];dx.o.fr=label.carrier.toFixed(2);dx.o.lo=label.lo;dx.o.hi=label.hi;dx.o.o=label.moff;dx.o.fm=(label.flags&dx.DX_MODE);dx.o.ft=((label.flags&dx.DX_TYPE)>>dx.DX_TYPE_SFT);dx.o.fd=((label.flags&dx.DX_DOW)>>dx.DX_DOW_SFT);if(dx.o.fd==0){dx.o.fd=dx.DX_DOW_BASE;kiwi_trace('$SHOULD NOT OCCUR');}
...
if(dx.db==dx.DB_STORED&&gid!=-1&&dx_keys.shift&&dx_keys.alt){var type=dx.o.ft;type=(type==dx.DX_QUICK_0)?dx.DX_QUICK_1:dx.DX_QUICK_0;dx.o.ft=type;var flags=(dx.o.fd<<dx.DX_DOW_SFT)|(type<<dx.DX_TYPE_SFT)|dx.o.fm;wf_send('SET DX_UPD g='+dx.o.gid+' f='+dx.o.fr+' lo='+dx.o.lo.toFixed(0)+' hi='+dx.o.hi.toFixed(0)+' o='+dx.o.o.toFixed(0)+' fl='+flags+' b='+dx.o.begin+' e='+dx.o.end+' i='+encodeURIComponent(dx.o.i+'x')+' n='+encodeURIComponent(dx.o.n+'x')+' p='+encodeURIComponent(dx.o.p+'x'));return;}
extint_panel_hide();dx.o.fr=(parseFloat(dx.o.fr)+kiwi.freq_offset_kHz).toFixed(2);dx.o.pb='';if(dx.o.lo||dx.o.hi){if(dx.o.lo==-dx.o.hi){dx.o.pb=(Math.abs(dx.o.hi)*2).toFixed(0);}else{dx.o.pb=dx.o.lo.toFixed(0)+', '+dx.o.hi.toFixed(0);}}
var s1=w3_inline('',w3_div('w3-medium w3-text-aqua w3-bold','DX label edit'),w3_select('w3-margin-L-64/w3-label-inline w3-text-white /w3-text-red','database','','dx.db',dx.db,dx.db_s,'dx_database_cb',0));var s2;if(dx.db==dx.DB_STORED){var dow_s='';var dow=dx.o.fd;if(dow==0)dow=dx.o.fd=dx.DX_DOW_BASE;console.log('DX g='+gid+' dow='+dow+'|'+dow.toHex(2)+' f='+(+dx.o.fr).toFixed(2)+' b='+dx.o.begin+' e='+dx.o.end+' '+dx.o.i);for(var i=0;i<7;i++){var day=dow&(1<<(6-i));var fg=day?'black':'white';var bg=day?dx.dow_colors[i]:'darkGrey';dow_s+=w3_button('w3-dummy//w3-round w3-padding-tiny'+
...
var type_menu=kiwi_deep_copy(cfg.dx_type);if(dx.types_n){for(i=0;i<dx.DX_N_STORED;i++){if(dx.types_n[i]!=0&&type_menu[i].name==''){type_menu[i].name='(T'+i+')';}}}
s2=w3_divs('w3-text-white/w3-margin-T-8',w3_inline('w3-halign-space-between/',w3_input('w3-padding-small||size=8','Freq','dx.o.fr',dx.o.fr,'dx_num_cb'),w3_select('w3-text-red','Mode','','dx.o.fm',dx.o.fm,kiwi.modes_u,'dx_sel_cb'),w3_input('w3-padding-small||size=10','Passband','dx.o.pb',dx.o.pb,'dx_passband_cb'),w3_select('w3-text-red','Type','','dx.o.ft',dx.o.ft,type_menu,'dx_sel_cb'),w3_input('w3-padding-small||size=8','Offset','dx.o.o',dx.o.o,'dx_num_cb')),w3_input('w3-label-inline/w3-padding-small','Ident','dx.o.i','','dx_string_cb'),w3_input('w3-label-inline/w3-padding-small','Notes','dx.o.n','','dx_string_cb'),w3_input('w3-label-inline/w3-padding-small','Extension','dx.o.p','','dx_string_cb'),w3_inline('w3-hspace-16',w3_text('w3-text-white w3-bold','Schedule (UTC)'),dow_s,w3_input('w3-label-inline/w3-padding-small','Begin','dx.o.begin',begin_s,'dx_sched_time_cb'),w3_input('w3-label-inline/w3-padding-small','End','dx.o.end',end_s,'dx_sched_time_cb')),w3_inline('w3-hspace-16',w3_button('w3-yellow','Modify','dx_modify_cb'),w3_button('w3-green','Add','dx_add_cb'),w3_button('w3-red','Delete','dx_delete_cb'),w3_div('w3-margin-T-4',w3_checkbox('/w3-label-inline w3-label-not-bold w3-text-white/','filter by time/day-of-week','dx.stored_filter_tod',dx.stored_filter_tod,'dx_time_dow_cb'),w3_text('w3-margin-T-4','Create new label with Add button'))));}else{var cbox_container='w3-halign-space-around/|flex-basis:130px';var cbox='/w3-label-inline w3-label-not-bold w3-round w3-padding-small/';var checkbox=function(type){var idx=dx_type2idx(type);var svc=(dx.eibi_types_mask&(1<<idx))?1:0;return w3_checkbox(cbox,dx.eibi_svc_s[idx],'eibi-cbox'+idx,svc,'dx_eibi_svc_cb',idx);};s2=w3_inline('',w3_checkbox('w3-margin-TB-16/w3-label-inline w3-label-not-bold w3-text-white/w3-hspace-32','select all','eibi-all',dx.eibi_types_mask==0xffffffff,'dx_eibi_all_cb'),w3_checkbox('/w3-label-inline w3-label-not-bold w3-text-white/w3-margin-L-32','filter by time/day-of-week','dx.eibi_filter_tod',dx.eibi_filter_tod,'dx_time_dow_cb'))+
w3_div('w3-valign-col w3-round w3-padding-TB-10 w3-gap-10|background:whiteSmoke',w3_inline(cbox_container,checkbox(dx.DX_BCAST),checkbox(dx.DX_UTIL),checkbox(dx.DX_TIME)),w3_inline(cbox_container,checkbox(dx.DX_ALE),checkbox(dx.DX_HFDL),checkbox(dx.DX_MILCOM)),w3_inline(cbox_container,checkbox(dx.DX_CW),checkbox(dx.DX_FSK),checkbox(dx.DX_FAX)),w3_inline(cbox_container,checkbox(dx.DX_AERO),checkbox(dx.DX_MARINE),checkbox(dx.DX_SPY)));}
ext_panel_set_name('dx');ext_panel_show(s1+s2,null,function(){if(dx.db==dx.DB_STORED){var el=w3_el('dx.o.i');el.value=dx.o.i;w3_el('dx.o.n').value=dx.o.n;w3_el('dx.o.p').value=dx.o.p;}else{for(var i=0;i<dx.DX_N_EiBi;i++){w3_color('eibi-cbox'+i+'-label','black',dx.eibi_colors[i]);}}},function(){},true);ext_set_controls_width_height(550,290);if(dx.help){extint_help_click_now();dx.help=false;}}
function dx_database_cb(path,idx,first,ctrlAlt,from_shortcut){if(first)return;dx.db=+idx;dx.list=[];console_log_dbgUs('DX DB-SWITCHED db='+dx.db);if(ctrlAlt)
dx_show_edit_panel(null,-1,from_shortcut);else{if(ext_panel_displayed('dx'))
dx_close_edit_panel();}
dx_schedule_update();}
...
function dx_time_dow_cb(path,checked,first){if(first)return;setVarFromString(path,checked?1:0);dx_schedule_update();}
...
if(audio_ext_adc_ovfl&&!sm_ovfl_showing){w3_hide('id-smeter-dbm-units');w3_show('id-smeter-ovfl');w3_call('S_meter_adc_ovfl',true);sm_ovfl_showing=true;}else
...
function keyboard_shortcut(key,key_mod,ctlAlt,keyCode){var action=true;var dir=ctlAlt?-1:1;shortcut.nav_click=false;switch(key){case'a':mode_button(null,w3_el('id-button-am'),dir);break;case'A':mode_button(null,w3_el('id-button-sam'),dir);break;case'd':ext_set_mode('drm',null,{open_ext:true});break;case'l':mode_button(null,w3_el('id-button-lsb'),dir);break;case'u':mode_button(null,w3_el('id-button-usb'),dir);break;case'c':mode_button(null,w3_el('id-button-cw'),dir);break;case'f':ext_set_mode('nbfm');break;case'q':ext_set_mode('iq');break;case'j':case'J':case'ArrowLeft':if(key_mod!=shortcut.SHIFT_PLUS_CTL_OR_ALT)
freqstep(owrx.wf_snap?key_mod:(2-key_mod));else
dx_label_step(-1);break;case'i':case'I':case'ArrowRight':if(key_mod!=shortcut.SHIFT_PLUS_CTL_OR_ALT)
freqstep(owrx.wf_snap?(5-key_mod):(3+key_mod));else
dx_label_step(+1);break;case'p':case'P':if(key_mod==shortcut.CTL_OR_ALT){restore_passband(cur_mode);demodulator_analog_replace(cur_mode);}else{passband_increment(key=='P');}
break;case'v':setvolume(1,kiwi.volume-5);toggle_or_set_mute(0);keyboard_shortcut_nav('audio');break;case'V':setvolume(1,kiwi.volume+5);toggle_or_set_mute(0);keyboard_shortcut_nav('audio');break;case' ':toggle_or_set_mute();shortcut.nav_click=true;break;case'g':case'=':freqset_select();break;case'm':freq_memory_menu_show(true);break;case'b':band_scroll(1);break;case'B':band_scroll(-1);break;case'n':freq_vfo_cb();break;case'N':vfo_update(true);break;case'<':page_scroll(-page_scroll_amount);break;case'>':page_scroll(+page_scroll_amount);break;case'z':zoom_step(ctlAlt?ext_zoom.NOM_IN:ext_zoom.IN);break;case'Z':zoom_step(ctlAlt?ext_zoom.MAX_OUT:ext_zoom.OUT);break;case'w':incr_mindb(1,ctlAlt?-10:-1);keyboard_shortcut_nav('wf');break;case'W':incr_mindb(1,ctlAlt?+10:+1);keyboard_shortcut_nav('wf');break;case's':toggle_or_set_spec();keyboard_shortcut_nav('wf');break;case'S':wf_autoscale_cb();keyboard_shortcut_nav('wf');break;case'D':toggle_or_set_slow_dev();keyboard_shortcut_nav('wf');break;case'!':keyboard_shortcut_nav('wf');wf_aper_cb('wf.aper',wf.aper^1,false);break;case'#':if(dbgUs)extint_open('prefs');break;case'o':keyboard_shortcut_nav(shortcut.nav_off?'status':'off');shortcut.nav_off^=1;break;case'r':toggle_or_set_rec();break;case'x':toggle_or_set_hide_panels();break;case'y':toggle_or_set_hide_bars();break;case'@':dx_filter();shortcut.nav_click=true;break;case'e':extension_scroll(1);break;case'E':extension_scroll(-1);break;case'\\':case'|':dx_database_cb('',(dx.db==dx.DB_STORED)?dx.DB_EiBi:dx.DB_STORED,false,key=='|',true);break;case'?':case'h':keyboard_shortcut_help();break;default:if(key.length==1&&keyCode!=shortcut.KEYCODE_ALT)console.log('no shortcut key <'+key+'>');action=false;break;}
ignore_next_keyup_event=true;}
function keyboard_shortcut_event(evt){if(evt.target){var k=evt.key;if(k=='Escape'||k.match(/F[1-9][12]?/)){return;}
var id=evt.target.id;var suffix='';if(id==''){w3_iterate_classList(evt.target,function(className,idx){if(className.startsWith('id-')){id=className;suffix=' (class)';}});}
var sft=evt.shiftKey;var ctl=evt.ctrlKey;var alt=evt.altKey;var meta=evt.metaKey;var ctlAlt=(ctl||alt);var key_mod=(sft&&!ctlAlt)?shortcut.SHIFT:((!sft&ctlAlt)?shortcut.CTL_OR_ALT:((sft&&ctlAlt)?shortcut.SHIFT_PLUS_CTL_OR_ALT:0));var field_input_key=((k>='0'&&k<='9'&&!ctl)||k=='.'||k==','||k=='/'||k==':'||k=='-'||k=='k'||k=='M'||k=='Enter'||k=='ArrowUp'||k=='ArrowDown'||k=='Backspace'||k=='Delete');if(evt.target.nodeName!='INPUT'||(id=='id-freq-input'&&!field_input_key)){if(kiwi_isMacOS()){if(meta){return;}}else{if(ctl){return;}}
if(alt&&!k.startsWith('Arrow')){k=String.fromCharCode(evt.keyCode).toLowerCase();if(sft)k=k.toUpperCase();}
keyboard_shortcut(k,key_mod,ctlAlt,evt.keyCode);cancelEvent(evt);return;}else{}}else{}}
...
w3_el('id-play-button-container').style.opacity=0;setTimeout(function(){w3_hide('id-play-button-container');},1100);audio_reset();freqset_select();}
...
w3_input('w3-custom-events w3-font-16px|margin: 2px 0 0 2px; padding:0 4px; width:'+freq_field_width()+'|type="text"'+mobile+' onchange="freqset_complete(1)" onkeyup="freqset_keyup(this, event)"','','id-freq-input')+'</form>')+'<select id="id-select-band" class="w3-pointer w3-select-menu" onchange="select_band(this.value)">'+
...
var WF_SPEED_OFF=0;var WF_SPEED_1HZ=1;var WF_SPEED_SLOW=2;var WF_SPEED_MED=3;var WF_SPEED_FAST=4;var wf_speed=WF_SPEED_FAST;var wf_speeds=['off','1 Hz','slow','med','fast'];var wf={no_wf:false,url_tstamp:0,ts_tz:0,scroll_multiple:3,no_sync:false,cmap:0,cmap_override:-1,custom_colormaps:[new Uint8Array(3*256),new Uint8Array(3*256),new Uint8Array(3*256),new Uint8Array(3*256)],aper:0,aper_algo:3,aper_param:0,sqrt:0,contr:0,last_zoom:-1,need_autoscale:0,auto_ceil:{min:0,val:5,max:30},auto_floor:{min:-30,val:0,max:0},auto_maxdb:0,auto_mindb:0,audioFFT_active:false,audioFFT_prev_mode:'',audioFFT_clear_wf:false,};var wf_contr_s={0:'normal',1:'scheme 1',2:'scheme 2',3:'scheme 3',4:'scheme 4'};function wf_contr_cb(path,idx,first){}
...
function toggle_or_set_spec_peak(set,val){if(isNumber(set))
spec.peak_show=kiwi_toggle(set,val,spec.peak_show,'last_spec_peak');else
spec.peak_show^=1;if(spec.peak_show)spec.peak_clear=true;w3_color('id-button-spec-peak',spec.peak_show?'lime':'white');freqset_select();writeCookie('last_spec_peak',spec.peak_show.toString());}
...
kiwi.muted=set;else
kiwi.muted^=1;console.log('$toggle_or_set_mute set='+set+' muted='+kiwi.muted);if(!owrx.squelched_overload){w3_show_hide('id-mute-no',!kiwi.muted);w3_show_hide('id-mute-yes',kiwi.muted);}
...
var de_emphasis=0;var de_emphasis_s=['off','on'];function de_emp_cb(path,idx,first){de_emphasis=+idx;snd_send('SET de_emp='+de_emphasis);writeCookie('last_de_emphasis',de_emphasis.toString());}
...
owrx.hide_bars=set;else{owrx.hide_bars=(owrx.hide_bars+1)&owrx.HIDE_ALLBARS;if((owrx.hide_bars&owrx.HIDE_TOPBAR)&&(extint.using_data_container||spec.source_wf||spec.source_audio))
...
recording=set;else
recording=!recording;w3_remove_then_add('id-rec1','fa-spin',recording?'fa-spin':'');w3_remove_then_add('id-rec2','fa-spin',recording?'fa-spin':'');w3_remove_then_add('id-rec1','w3-text-white','w3-text-pink');w3_remove_then_add('id-rec2','w3-text-white','w3-text-pink');if(recording){window.recording_meta={buffers:[],data:null,offset:0,total_size:0,filename:kiwi_host()+'_'+new Date().toISOString().replace(/:/g,'_').replace(/\.[0-9]+Z$/,'Z')+'_'+w3_el('id-freq-input').value+'_'+cur_mode+'.wav'};window.recording_meta.buffers.push(new ArrayBuffer(65536));window.recording_meta.data=new DataView(window.recording_meta.buffers[0]);}else
if(window.recording_meta){var wav_header=new ArrayBuffer(44);var wav_data=new DataView(wav_header);wav_data.setUint32(0,0x52494646);wav_data.setUint32(4,window.recording_meta.total_size+36,true);wav_data.setUint32(8,0x57415645);wav_data.setUint32(12,0x666d7420);wav_data.setUint32(16,16,true);wav_data.setUint16(20,1,true);var nch=ext_is_IQ_or_stereo_curmode()?2:1;wav_data.setUint16(22,nch,true);var srate=Math.round(audio_input_rate||12000);wav_data.setUint32(24,srate,true);var bpsa=16;var Bpsa=bpsa/8;wav_data.setUint32(28,srate*nch*Bpsa,true);wav_data.setUint16(32,nch*Bpsa,true);wav_data.setUint16(34,bpsa,true);wav_data.setUint32(36,0x64617461);wav_data.setUint32(40,window.recording_meta.total_size,true);window.recording_meta.buffers.unshift(wav_header);var wav_file=new Blob(window.recording_meta.buffers,{type:'octet/stream'});var a=document.createElement('a');a.style='display: none';a.href=window.URL.createObjectURL(wav_file);a.download=window.recording_meta.filename;document.body.appendChild(a);a.click();window.URL.revokeObjectURL(a.href);document.body.removeChild(a);delete window.recording_meta;}}
var squelched=0;var squelch=0;var squelch_tail=0;var squelch_tail_s=['0','.2s','.5s','1s','2s'];var squelch_tail_v=[0,0.2,0.5,1,2];function squelch_action(sq){squelched=sq;var mute_color=squelched?'white':kiwi.unmuted_color;var sq_color=mute_color;owrx.squelched_overload=(owrx.sMeter_dBm>=cfg.overload_mute);if(owrx.prev_squelched_overload!=owrx.squelched_overload){w3_show_hide('id-mute-exclaim',owrx.squelched_overload);w3_show_hide('id-mute-no',owrx.squelched_overload?false:!kiwi.muted);w3_show_hide('id-mute-yes',owrx.squelched_overload?false:kiwi.muted);owrx.prev_squelched_overload=owrx.squelched_overload;}
w3_color('id-squelch-label',sq_color);w3_color('id-mute-no',mute_color);if(recording){var stop=(squelched||owrx.squelched_overload);w3_remove_then_add('id-rec1','fa-spin',stop?'':'fa-spin');w3_remove_then_add('id-rec2','fa-spin',stop?'':'fa-spin');w3_remove_then_add('id-rec1','w3-text-white w3-text-pink',stop?'w3-text-white':'w3-text-pink');w3_remove_then_add('id-rec2','w3-text-white w3-text-pink',stop?'w3-text-white':'w3-text-pink');}}
function squelch_setup(flags){var nbfm=(cur_mode=='nbfm');if(flags&toggle_e.FROM_COOKIE){var sq=readCookie('last_squelch'+(nbfm?'':'_efm'));squelch=sq?+sq:0;w3_el('id-squelch-value').max=nbfm?99:40;w3_set_value('id-squelch-value',squelch);var tail=kiwi_storeGet('last_tail',0);squelch_tail=tail?+tail:0;w3_select_value('id-squelch-tail',squelch_tail);}
set_squelch_cb('',squelch,true,false,true);squelch_tail_cb('',squelch_tail,false,true);if(nbfm){squelch_action(squelched);}else{w3_color('id-mute-no',kiwi.unmuted_color);}
w3_hide2('id-squelch-tail',nbfm);w3_hide2('id-squelch',cur_mode=='drm');}
function send_squelch(){var nbfm=(cur_mode=='nbfm');snd_send("SET squelch="+squelch.toFixed(0)+' param='+(nbfm?squelch_threshold:squelch_tail_v[squelch_tail]).toFixed(2));}
function set_squelch_cb(path,str,done,first,no_write_cookie){var nbfm=(cur_mode=='nbfm');if(first)return;squelch=parseFloat(str);var sq_s=squelch.toFixed(0);w3_el('id-squelch-field').innerHTML=squelch?(sq_s+(nbfm?'':' dB')):'off';w3_set_value('id-squelch-value',squelch);w3_color('id-squelch-value',null,squelch?'':'rgba(255,0,0,0.3)');if(done){send_squelch();if(no_write_cookie!=true){writeCookie('last_squelch'+(nbfm?'':'_efm'),sq_s);}
freqset_select();}}
...
function squelch_tail_cb(path,val,first,no_write_cookie){if(first)return;squelch_tail=+val;send_squelch();if(no_write_cookie!=true&&cur_mode!='nbfm')
...
function set_agc(){var isCW=(cur_mode&&cur_mode.substr(0,2)=='cw');w3_color('label-threshold',(!isCW&&agc)?'orange':'white');w3_color('label-threshCW',(isCW&&agc)?'orange':'white');var thold=isCW?threshCW:thresh;snd_send('SET agc='+agc+' hang='+hang+' thresh='+thold+' slope='+slope+' decay='+decay+' manGain='+manGain);}
...
((i==rx_chan)?'w3-text-css-lime':optbar_prefix_color),'RX'+i),w3_div('id-optbar-user-'+i+' w3-show-inline-block'));}
...
function freq_vfo_cb(){owrx.vfo_ab^=1;w3_flip_colors('id-freq-vfo','w3-aqua w3-orange',owrx.vfo_ab);w3_innerHTML('id-freq-vfo',"AB"[owrx.vfo_ab]);var vfo=parse_freq_pb_mode_zoom(readCookie('last_vfo_'+"AB"[owrx.vfo_ab]));var mode=undefined,zoom=0;if(vfo[3])mode=vfo[3].toLowerCase();if(vfo[4])zoom=+vfo[4];ext_tune(vfo[1],mode,ext_zoom.ABS,zoom);}
function toggle_or_set_spec(set,val){console.log('toggle_or_set_spec set='+set+' val='+val);if(isNumber(set)&&(set&toggle_e.SET)){spec.source_wf=val;}else{spec.source_wf^=1;}
if(extint.using_data_container&&spec.source_wf){extint_panel_hide();}
html('id-button-spectrum').style.color=spec.source_wf?'lime':'white';w3_show_hide('id-spectrum-container',spec.source_wf);w3_show_hide('id-top-container',!spec.source_wf);freqset_select();}
...
function restore_passband(mode){owrx.last_lo[mode]=cfg.passbands[mode].lo;owrx.last_hi[mode]=cfg.passbands[mode].hi;}
var mode_buttons=[{s:['AM','AMN'],dis:0},{s:['SAM','SAL','SAU','SAS','QAM'],dis:0},{s:['DRM'],dis:0},{s:['LSB','LSN'],dis:0},{s:['USB','USN'],dis:0},{s:['CW','CWN'],dis:0},{s:['NBFM'],dis:0},{s:['IQ'],dis:0},];function mode_button(evt,el,dir){var mode=el.innerHTML.toLowerCase();if(isUndefined(dir))dir=1;if(any_alternate_click_event(evt)){if(evt.shiftKey||evt.button==mouse.middle){restore_passband(mode);ext_set_mode(mode,null,{open_ext:true});return;}
dir=-1;}
...
function place_panels(){var left_col=[];var right_col=[];var plist=html("id-panels-container").children;for(var i=0;i<plist.length;i++){var c=plist[i];if(c.classList.contains('class-panel')){var position=c.getAttribute('data-panel-pos');if(!position)continue;var newSize=c.getAttribute('data-panel-size').split(",");if(position=="left"){left_col.push(c);}
...
if(position=='bottom-left'){c.style.left=0;c.style.bottom=0;c.style.visibility="hidden";}}}
y=0;while(left_col.length>0){p=pop_bottommost_panel(left_col);p.style.left=0;p.style.bottom=px(y);p.style.visibility="visible";y+=p.uiHeight+3*panel_margin;w3_call('panel_setup_'+p.idName,p);}
...
w3_div('id-optbar-content w3-margin-T-6 w3-scroll-y|height:'+px(OPTBAR_CONTENT_HEIGHT),w3_div('id-optbar-wf w3-hide'),w3_div('id-optbar-audio w3-hide'),w3_div('id-optbar-agc w3-hide'),w3_div('id-optbar-users w3-hide w3-scroll-x'),w3_div('id-optbar-status w3-hide'))+
...
function set_gen(freq,attn){snd_send("SET genattn="+attn.toFixed(0));snd_send("SET gen="+freq+" mix=-1");}
var bin_server=0;var zoom_server=0;function owrx_msg_cb(param,ws){switch(param[0]){case"wf_setup":wf_init();break;case"extint_list_json":extint_list_json(param[1]);if(override_ext)
extint_open(override_ext,3000);break;case"bandwidth":bandwidth=parseInt(param[1]);break;case"center_freq":center_freq=parseInt(param[1]);break;case"wf_fft_size":wf_fft_size=parseInt(param[1]);break;case"wf_fps_max":wf_fps_max=parseInt(param[1]);break;case"wf_fps":wf_fps=parseInt(param[1]);break;case"wf_cal":break;case"start":bin_server=parseInt(param[1]);break;case"zoom":zoom_server=parseInt(param[1]);break;case"zoom_max":zoom_levels_max=parseInt(param[1]);zoom_nom=Math.min(ZOOM_NOMINAL,zoom_levels_max);break;case"audio_init":toggle_or_set_audio(toggle_e.FROM_COOKIE|toggle_e.SET,1);audio_init(+param[1],btn_less_buffering,kiwi_toggle(toggle_e.FROM_COOKIE|toggle_e.SET,1,1,'last_compression'));break;case"audio_camp":var p=param[1].split(',');audio_camp(+p[0],+p[1],btn_less_buffering,kiwi_toggle(toggle_e.FROM_COOKIE|toggle_e.SET,1,1,'last_compression'));break;case"audio_rate":audio_rate(parseFloat(param[1]));break;case"audio_adpcm_state":var p=param[1].split(',');audio_adpcm_state(+p[0],+p[1]);break;case"audio_passband":var p=param[1].split(',');audio_recompute_LPF(1,+p[0],+p[1]);break;case"audio_flags2":audio_recv_flags2(param[1]);break;case"kiwi_up":kiwi_up(parseInt(param[1]));break;case"gps":toggle_or_set_spec(toggle_e.SET,1);break;case"fft_mode":kiwi_fft_mode();break;case"maxdb":wf.auto_maxdb=+param[1];setmaxdb(1,wf.auto_maxdb,true);break;case"mindb":wf.auto_mindb=+param[1];setmindb(1,wf.auto_mindb,true);w3_call('waterfall_maxmin_cb');update_maxmindb_sliders(true);break;case"max_thr":owrx.overload_mute=Math.round(+param[1]);break;default:return false;}
return true;}
