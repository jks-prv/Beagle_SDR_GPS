...
// 	},
    ...
    __draw: function(label) {
        ...
        function _text(txt, x, y) {
            if (1) {
                var sop = ctx.opacity;
                ctx.opacity = 1;
                var sfs = ctx.fillStyle;
                ctx.fillStyle = "black";
                var w = Math.ceil(txtWidth);
                var h = Math.ceil(txtHeight);
        ...
        if (L.Browser.canvas && map) {
            ...
            function __draw_lon_line(self, lon_tick) {
                ...
                if (curvedLon) {
                    ...
                    for (var j=_lat_b; j<_lat_t; j+=_lat_delta) {
                        ...
                        if (self.options.showLabel && label && _prev_p != null) {
                            if (_prev_p.y > 8 && tt.y <= 8) {
                                _text(lngstr, tt.x - (txtWidth/2), txtHeight);
                            }
                            else if (_prev_p.y >= hh && tt.y < hh) {
                                _text(lngstr, tt.x - (txtWidth/2), hh-2);
                            }
                        }
                    ...
                    if (self.options.showLabel && label) {
                        _text(lngstr, tt.x - (txtWidth/2), txtHeight+1);
                        _text(lngstr, bb.x - (txtWidth/2), hh-3);
                    }
                }
            };
