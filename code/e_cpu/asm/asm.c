...
int main(int argc, char *argv[])
{
	...
	char *bfs = FN_PREFIX ".aout";				// loaded into FPGA via SPI
	char *ofs = "ecode.aout.h";					// included by simulator
		  hfs = "../" FN_PREFIX ".gen.h";		// included by .cpp / .c
		  vfs = "../wrx_v/" FN_PREFIX ".gen.vh";	// included by verilog

	int ifn;
	FILE *ifp[NIFILES_NEST], *ofp, *hfp, *vfp;
	...
	if (gen) {
		...
		fprintf(hfp, "%s", warn);
		fprintf(hfp, "%s", "#ifndef _WRX_GEN_H_\n#define _WRX_GEN_H_\n\n// from assembler DEF directives:\n\n");
		fprintf(vfp, "%s`ifndef _WRX_GEN_VH_\n`define _WRX_GEN_VH_\n\n// from assembler DEF directives:\n\n", warn);
	}
		...
		if (tp->ttype == TT_PRE && tp->num == PP_REPEAT) {
			...
			for (i=0; i<val; i++) {
				...
				while (tp->ttype != TT_PRE && tp != ep2) {
					if (tp->ttype == TT_ITER) to->ttype = TT_NUM, to->num = i, to++, tp++; else *to++ = *tp++;
				}
	...
	for (tp=pass4; tp != ep4; tp++) {
		...
		if (tp->ttype == TT_OPC) {
			...
			// have to do this in a later pass depending on when constant was resolved, use in macros etc.
			if ((strcmp(tp->str, "push") == 0) && ((tn->ttype == TT_NUM) && (tn->num >= 0x8000))) {
				...
				t = tn+1; insert(t, &ep4); t->ttype = TT_OPC; t->str = "shl"; t->num = OC_SHL;
				tp += 2; a += 2;
				
				if (odd) {
					if (debug) printf("push 1; or; ");
					t++; insert(t, &ep4); t->ttype = TT_OPC; t->str = "push"; t->num = OC_PUSH + 0;
					t++; insert(t, &ep4); t->ttype = TT_NUM; t->num = 1;
					t++; insert(t, &ep4); t->ttype = TT_OPC; t->str = "or"; t->num = OC_OR;
					tp += 3; a += 4;
				}
	...
	for (tp=pass4; tp != ep4; tp++) {
		...
		if (tp->ttype == TT_OPC) {
			...
			// opcode & operand
			if (t->ttype == TT_NUM) {
				op += t->num, operand_type=1;
			}
			if ((t->ttype == TT_SYM) && ((st = string_find(t->str)) && (st->flags & SF_DEFINED))) {
				op += st->val, operand_type=2;
			}
