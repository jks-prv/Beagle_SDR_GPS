...
function wspr_div()
{
	...
	"<div id='wspr_div' style='width:auto; display:none'>"+
		...
		"</table><table>"+
			"<tr>"+
				'<td style="width:20%">'+ wrx_button('demo', 'wspr_freq('+wspr_fbn+');') + '</td>'+
...
function wspr_visible(v)
{
   ...
   waterfallapplet[0].app_visible(v);
	...
	if (v) {
		wrx_waterfallmode(WF_NORM, WF_SMALL);
		setVBW(VBW_NONLIN);
   	...
   	wrx_clearTimeout(wspr_upload_timeout);
   	wrx_clearTimeout(wspr_pie_timeout);
...
function wspr_upload(type, s)
{
	...
	var request = wrx_GETrequest(spot? "spot":"stat", url);
	wrx_GETrequest_param(request, "function", spot? "wspr":"wsprstat");
	wrx_GETrequest_param(request, "rcall", wspr_calls[rcall]);
	wrx_GETrequest_param(request, "rgrid", wspr_grids[rgrid]);
	wrx_GETrequest_param(request, "rqrg", wspr_rfreq.toFixed(6));
	
	if (spot) {
		var d = new Date();
		wrx_GETrequest_param(request, "date",
			d.getUTCFullYear().toString().substr(2,2)+(d.getUTCMonth()+1).leadingZeros(2)+d.getUTCDate().leadingZeros(2));
		wrx_GETrequest_param(request, "time", decode[0]);
		wrx_GETrequest_param(request, "sig", decode[1]);
		wrx_GETrequest_param(request, "dt", decode[2]);
		wrx_GETrequest_param(request, "drift", decode[4]);
		tqrg = decode[3];
		dbm = decode[7];
	} else {
		wrx_GETrequest_param(request, "tpct", "0");
		tqrg = wspr_tfreq.toFixed(6);
		dbm = "0";
	}
	
	wrx_GETrequest_param(request, "tqrg", tqrg);

	if (spot) {
		wrx_GETrequest_param(request, "tcall", decode[5]);
		wrx_GETrequest_param(request, "tgrid", decode[6]);
	}
	
	wrx_GETrequest_param(request, "dbm", dbm);
	var version = "0.1_wrx";
	//var version = "0.8_r3058";
	wrx_GETrequest_param(request, "version", version);
	wrx_GETrequest_submit(request);
...
{
	var s = "<td>"+wrx_button(v, 'wspr_freq('+wspr_fbn+');')+"</td>";
	wspr_fbn++;
	return s;
}
