...
static void update_task(void *param)
{
	...
	int status = child_task("kiwi.upd", curl_makefile_ctask, POLL_MSEC(1000));
	...
	if (ver_changed || force_build) {
		...
		status = child_task("kiwi.bld", update_build_ctask, POLL_MSEC(1000), TO_VOID_PARAM(force_build));
