...
std::string NML::Dump(void) const
{
  ...
  for(unsigned int i=0; i<nr_of_items; i++)
  {
    ...
    {
         kiwi_snprintf_buf(buf, "Item #%d is\n", i);
    }
    items += buf;
...
NML* NMLFactory::CreateNML( const NML::RawNewsObject_t& rno, const NMLEscapeCodeHandler* EscapeCodeHandler )
{
	...
	if( rno.nml_len < 4 )
	{
	    kiwi_snprintf_buf( error, "Error: NML has only %u bytes", rno.nml_len );
	...
	{
	    kiwi_snprintf_buf( error, "Error: NML has illegal object type 0x%02x", rno.nml[2] >> 5 );
	...
	{
	    kiwi_snprintf_buf( error, "Error: NML extended header too big (%u>%u)", rno.extended_header_len, len );
		n->SetErrorDump( n->_news.object_id, rno, error );
		return n;
	}
	...
	if( compressed_flag )
	{
		...
		if( *p != 0x08 )
		{
		    ...
		    kiwi_snprintf_buf( error, "Error: NML with unknown compression 0x%02x", *p );
		...
		if( !rv )
		{
		    kiwi_snprintf_buf( error, "Error: could not uncompress NML body (%d)", rv );
			n->SetErrorDump(n->_news.object_id, rno, error);
			return n;
		}
		...
		*/
		...
		{
		    kiwi_snprintf_buf( error, "Error: uncompressed NML body too big (%lu bytes)",
	...
	if( *p != 0x01 )
	{
	    kiwi_snprintf_buf( error, "Error: expected NML Code 0x01, got 0x%02x", *p );
	...
	if( !pure_text )
	{
	    kiwi_snprintf_buf( error, "Error: ignoreDataSections() returned illegal data section" );
		n->SetErrorDump( n->_news.object_id, uncompressed, error );
		return n;
	}
	...
	if( n->_news.object_type == NML::PLAIN )
	{
		if( *p != 0x03 )
		{
		    kiwi_snprintf_buf( error, "Error: expected NML Code 0x03, got 0x%02x", *p );
			n->SetErrorDump( n->_news.object_id, uncompressed, error );
			return n;
		}
		len--;
	...
	else if( n->_news.object_type == NML::MENU )
	{
		while( len > NML::NML_MIN_NR_OF_ITEM_BYTES )
		{
			if( *p != 0x02 )
			{
			    kiwi_snprintf_buf( error, "Error: expected NML Code 0x02, got 0x%02x", *p );
				n->SetErrorDump( n->_news.object_id, uncompressed, error );
				return n;
			}
			len--;
	...
	else if( n->_news.object_type == NML::LIST )
	{
		...
		while( len > NML::NML_MIN_NR_OF_ITEM_BYTES )
		{
			if( *p != 0x04 && *p != 0x05 )
			{
			    kiwi_snprintf_buf( error, "Error: expected NML Code 0x04 or 0x05, got 0x%02x", *p );
				n->SetErrorDump( n->_news.object_id, uncompressed, error );
				return n;
			}
//			bool newRow = (*p == 0x04);
