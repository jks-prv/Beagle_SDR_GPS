...
public:
    ...
    std::complex<float> process_sample(TYPECPX *s) {
        ...
        std::complex<float> exp_sample = std::pow(sample, _exponent);
        ...
        sample *= std::exp(std::complex<float>(0.0f, -_phase/_exponent));
    ...
    void display_data(int ch, int nsamps, TYPECPX *samps) {
        ...
        _nsamp += nsamps;
        if (_nsamp > _maNsend) {
            ext_send_msg(_rx_chan, IQ_DISPLAY_DEBUG_MSG, "EXT cmaI=%e cmaQ=%e df=%e", _cma.real(), _cma.imag(), _df/(2*M_PI*_exponent));
            _nsamp -= _maNsend;
        }
    }
