...
void sstv_get_fsk(sstv_chan_t *e, char *dest)
{
    ...
    int i=0, LoBin, HiBin, MidBin, TestNum=0, TestPtr=0;
  ...
  for (i = 0; i < samps_22ms; i++) Hann[i] = 0.5 * (1 - SSTV_MCOS( 2 * M_PI * i / (SSTV_REAL) (samps_22ms-1) ) );
  ...
  while ( TRUE ) {
    ...
    // Apply Hann window
    for (i = 0; i < samps_22ms; i++) e->fft.in[i] = e->pcm.Buffer[e->pcm.WindowPtr+i- half_samps_22ms] * Hann[i];
    ...
    LoBin  = GET_BIN(1900+e->pic.HedrShift, FFTLen)-1;
    MidBin = GET_BIN(2000+e->pic.HedrShift, FFTLen);
    HiBin  = GET_BIN(2100+e->pic.HedrShift, FFTLen)+1;
