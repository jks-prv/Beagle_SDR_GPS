...
// data types
typedef struct FComplex_s {float re,im;} FComplex;
typedef struct FDemodulate_s {float data; FComplex rx_symbol; FComplex dx_symbol; FComplex error;} FDemodulate;

// external API
typedef enum{ B75N, B75S, B75L, B150N, B150S, B150L, B300N, B300S, B300L, B600N, B600S, B600L, B600U, B1200N, B1200S, B1200L, B1200U, B1800U, B2400N, B2400S, B2400L, B2400U, B3600U } Kmode;
...
// This class is exported from the St4285.dll
...
private:
	...
	enum{ KN = FF_EQ_LENGTH + FB_EQ_LENGTH };
	...
	enum{ RX_BLOCK_LENGTH = 1024 };
	enum{ SYMBOLS_PER_FRAME = 256 };
	enum{ SCRAMBLER_TABLE_LENGTH = 176 };	// i.e. SYMBOLS_PER_FRAME - PREAMBLE_LENGTH since probes are also scrambled
	enum{ SAMPLE_BLOCK_SIZE = (SYMBOLS_PER_FRAME*4)};
	...
	enum{ DATA_LENGTH =  32 };
	enum{ DATA_SYMBOLS_PER_FRAME = (SYMBOLS_PER_FRAME-PREAMBLE_LENGTH)};
...
private:
	...
	FComplex in_a[RX_CHANNELS][SAMPLE_BLOCK_SIZE+RX_FILTER_SIZE];
	FComplex in_b[RX_CHANNELS][3*HALF_SAMPLE_BLOCK_SIZE];
...
private:
	...
	float initial_doppler_correct( FComplex *in , float *delta );
	float doppler_correct( FComplex *in , float *delta );
