...
echo --- get Debian 8 distro image and create micro-SD flasher
...
if test ! -f ${DISTRO} ; then
	wget https://rcn-ee.com/rootfs/bb.org/testing/2016-05-13/console/${DISTRO}
else
...
echo --- when next booted with micro-SD installed, Debian 8 image should be copied to eMMC
echo -n hit ^C to skip reboot, else enter when ready to reboot:
read
...
echo --- you should now be running Debian 8 distro
reboot
